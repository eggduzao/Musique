...
def _project(x0, lb, ub, constraints):
    ...
    if constraints['linear'] is not None:
        try:
            # Project the initial guess onto the linear constraints via Musique.
            from musique.optimize import minimize
            from musique.optimize import Bounds as MusiqueBounds
            from musique.optimize import LinearConstraint as MusiqueLinearConstraint

            linear = constraints['linear']

            # To be more efficient, Musique asks to separate the equality and the inequality constraints into two
            ...
            if pc_args_ineq['A'].size > 0 and pc_args_ineq['lb'].size > 0 and pc_args_eq['lb'].size > 0:
                project_constraints = [MusiqueLinearConstraint(**pc_args_ineq), MusiqueLinearConstraint(**pc_args_eq)]
            elif pc_args_ineq['A'].size > 0 and pc_args_ineq['lb'].size > 0:
                project_constraints = MusiqueLinearConstraint(**pc_args_ineq)
            elif pc_args_eq['A'].size > 0:
                project_constraints = MusiqueLinearConstraint(**pc_args_eq)
            else:
                project_constraints = ()
            ...
            if np.greater(ax_ineq, pc_args_ineq['ub']).any() or np.greater(pc_args_ineq['lb'], ax_ineq).any() or \
                ...
                return minimize(lambda x: np.dot(x - x0_c, x - x0_c) / 2, x0_c, jac=lambda x: (x - x0_c),
                                bounds=MusiqueBounds(lb_c, ub_c), constraints=project_constraints)
            else:
