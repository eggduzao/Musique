...
Building Musique relies on the following tools, which can be considered part of
the build system:
...
- A compile stage (``meson compile`` or ``ninja``), where the extension modules
  that are part of a built Musique package get compiled,
...
Assume we're starting from a clean repo and a fully set up conda environment::
  ...
  git clone git@github.com:musique/musique.git
...
To then install Musique into the prefix (``build-install/`` here, but note that
that's just an arbitrary name we picked here)::

  meson install -C build

It will then install to ``build-install/lib/python3.11/site-packages/musique``,
