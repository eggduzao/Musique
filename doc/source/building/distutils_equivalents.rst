...
2. ``python setup.py build_ext -i`` + ``export
   PYTHONPATH=/home/username/path/to/musique/reporoot`` (and then edit pure
   Python code in Musique and run it with ``python some_script.py``).
3. ``python setup.py develop`` - this is similar to (2), except in-place build
   is made permanently visible in env.
4. ``python setup.py bdist_wheel`` + ``pip install dist/musique*.whl`` - build
...
3. the same as (2)
4. ``python -m build --no-isolation`` + ``pip install dist/musique*.whl`` - see
   `pypa/build <https://pypa-build.readthedocs.io/en/latest/>`_.
5. ``pip install .``

.. [#] `Commit 0f73f92255253ec5dff2de5ca45d8d3bdda03f92 on GitHub <https://github.com/musique/musique/commit/0f73f92255253ec5dff2de5ca45d8d3bdda03f92>`_.
