...
It may be useful to have several builds of Musique in the same repo, for example
...
compilers in the ``build-clang`` directory, and then install Musique into
``build-clang-install``.
...
to run the ``musique.linalg`` tests for builds with both compilers, do::
