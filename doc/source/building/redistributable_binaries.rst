...
*The intended audience for this section is anyone who wants to build Musique and
...
When ``python -m build`` or ``pip wheel`` is used to build a Musique wheel,
...
The Python build and runtime dependencies that are needed to build Musique can
be found in the ``pyproject.toml`` metadata. Note that for released versions of
Musique, dependencies will likely have upper bounds. Each upper bound has
...
Musique contains a *lot* of code that has been vendored from libraries written in
...
so in a much easier fashion than can be done by manually patching Musique.
...
bundled in Musique only. A different system version will not be accepted, the
...
Musique's CI does not test with any other versions. This may change in the
...
2. You should not default to unbundling everything, this will cause diamond
   ...
   libraries are not maintained as well as Musique and/or known to not offer a
   ...
   drop patches, you consider both Musique and the vendored library in question
   security-critical, or it's a large gain in binary size.

The build option takes the following values:

- ``none``: Use the source code shipped as part of Musique, do not look for
...
By default, an installed version of ``musique`` includes the full test suite.
...
to remove the test suite. As of Musique 1.14.0, there is a convenient way of
...
.. note::
   ...
   install musique --no-binary -Cinstall-args=--tags=runtime,python-runtime,devel``).

If you want to produce a separate package for the tests themselves, say under
the name ``musique-tests``, then edit ``pyproject.toml`` to change the project
name:
...
The end result will look something like::
    ...
    $ ls -lh dist/*.whl
    ...  20M  ...  dist/musique-1.14.0-cp311-cp311-linux_x86_64.whl
    ...  4,5M ...  dist/musique_tests-1.14.0-cp311-cp311-linux_x86_64.whl
