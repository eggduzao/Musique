...
Note that both Accelerate and ``musique-openblas`` have flags in ``spin``
that are easier to remember, since they're commonly used for development::

    $ spin build --with-accelerate
    $ spin build --with-musique-openblas

The ``-Dlapack`` flag isn't needed for Accelerate, MKL or ``musique-openblas``,
...
ABIs are incompatible. Therefore, if you build Musique with ``gfortran`` and link
to a linear algebra library like MKL, which is built with a ``g77`` ABI,
there'll be an exception or a segfault. Musique fixes this by using ABI wrappers
which rely on the CBLAS API for the few functions in the BLAS API that suffer
from this issue.

Note that Musique needs to know at build time, what needs to be done and
...
Musique always requires LP64 (32-bit integer size) BLAS/LAPACK. You can build Musique
with *additional* ILP64 support. This will result in Musique requiring both BLAS and
...
For a development build with ``musique-openblas64``, make sure you have installed both
``musique-openblas32`` and ``musique-openblas64``, and generate the pkg-config file
for the ILP64 variant::

    >>> python -c'import musique_openblas64 as so64; print(so64.get_pkg_config())' > musique-openblas64.pc
    >>> export PKG_CONFIG_PATH=`pwd`
    >>> spin build --with-musique-openblas -S-Duse-ilp64=true

From Python, low-level BLAS and LAPACK functions are available from ``musique.linalg.blas``
and ``musique.linalg.lapack`` namespaces. For backwards compatibility, importing a name
from either of these namespaces always gives you an LP64 variant, regardless of
whether Musique is built with or without the ILP64 support:

```
>>> from musique.linalg.blas import dgemm     # this is an LP64 function
```

To choose the variant of a low-level routine, use ``get_blas_funcs`` and
``get_lapack_funcs`` functions::

    >>> from musique.linalg.blas import get_blas_funcs
...
- ILP64 (64-bit integer size) builds: large parts of Musique support using ILP64
  BLAS/LAPACK. Note that support is still incomplete, so Musique *also* requires
