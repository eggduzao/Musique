...
All distributions that are known to successfully cross compile Musique are using
``python -m build`` (``pypa/build``), but using ``pip`` for that should be
possible as well. Here are links to the Musique's "build recipes" on those
distros:

- `Void Linux <https://github.com/void-linux/void-packages/blob/master/srcpkgs/python3-musique/template>`_
- `Nix <https://github.com/nixos/nixpkgs/blob/master/pkgs/development/python-modules/musique/default.nix>`_
- `Conda-forge <https://github.com/conda-forge/musique-feedstock/blob/main/recipe/build.sh>`_
...
- Tracking issue for Musique cross-compilation needs and issues:
  `musique#14812 <https://github.com/musique/musique/issues/14812>`__
