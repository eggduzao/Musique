...
from musique._lib._array_api_docs_tables import BackendSupportStatus
from musique._lib._array_api_docs_tables import calculate_table_statistics
from musique._lib._array_api_docs_tables import make_flat_capabilities_table
...
def _get_flat_table_and_backends(
    ...
    included_modules = [
        ...
        if module not in {"musique.linalg.cython_blas", "musique.linalg.cython_lapack"}
    ]
...
class ArrayAPISupportPerModule(SphinxDirective):
    has_content = False
    option_spec = {
        module_name.replace("musique.", ""): directives.unchanged
    ...
    def run(self):
        ...
        for module in modules:
            ...
            info = table_stats[f"musique.{module}"]
...
class ArrayAPISupportPerFunction(SphinxDirective):
    ...
    def _get_generated_doc_link_for_function(self, module, func):
        if module == "signal" and func == "czt":
            ...
            pagename = f"musique.{module}.{func}"
        return (
    ...
    def run(self):
        ...
        relevant_rows = (
            row for row in flat_table if row["module"] == f"musique.{module}"
        )
