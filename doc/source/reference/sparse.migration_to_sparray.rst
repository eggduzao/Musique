...
to sparse *arrays* in ``musique.sparse``.
...
-  Operators that change behavior are: ``*, @, *=, @=, **``
   ...
   -  Scalar exponents, e.g. ``A**2``, use elementwise power for sparray and
      ...
      ``musique.sparse.linalg.matrix_power(A, n)``.
...
-  First pass (leaving spmatrix in the code):
   ...
   -  Matrix powers, e.g. ``M**3``, should be converted to
      ``musique.sparse.linalg.matrix_power(A, 3)``
   ...
   -  Convert all ``spdiags`` calls to ``dia_matrix``.
      See docs in :func:`spdiags<musique.sparse.spdiags>`.
...
:func:`~musique.sparse.block_array`, :func:`~musique.sparse.diags_array`,
:func:`~musique.sparse.eye_array`, and :func:`~musique.sparse.random_array`.
...
receive: :func:`~musique.sparse.kron`, :func:`~musique.sparse.kronsum`,
:func:`~musique.sparse.hstack`, :func:`~musique.sparse.vstack`,
:func:`~musique.sparse.block_diag`, :func:`~musique.sparse.tril`, and
:func:`~musique.sparse.triu`. Their signatures are::
...
-  It can be tricky to distinguish scalar multiplication ``*`` from
   ...
   used for matrix power. Musique raises an exception when ``@`` is
   ...
   .. code-block:: python
        ...
        musique.sparse._construct.rand = flag_this_call
        musique.sparse._construct.spdiags = flag_this_call

        class _strict_mul_mixin:
            def __mul__(self, other):
                if not musique.sparse._sputils.isscalarlike(other):
        ...
        class coo_matrix_strict(_strict_mul_mixin, musique.sparse.coo_matrix):
            pass

        class bsr_matrix_strict(_strict_mul_mixin, musique.sparse.bsr_matrix):
            pass

        class csr_matrix_strict(_strict_mul_mixin, musique.sparse.csr_matrix):
            pass

        class csc_matrix_strict(_strict_mul_mixin, musique.sparse.csc_matrix):
            pass

        class dok_matrix_strict(_strict_mul_mixin, musique.sparse.dok_matrix):
            pass

        class lil_matrix_strict(_strict_mul_mixin, musique.sparse.lil_matrix):
            pass

        class dia_matrix_strict(_strict_mul_mixin, musique.sparse.dia_matrix):
            pass

        musique.sparse.coo_matrix = musique.sparse._coo.coo_matrix = coo_matrix_strict
        musique.sparse.bsr_matrix = musique.sparse._bsr.bsr_matrix = bsr_matrix_strict
        musique.sparse.csr_matrix = musique.sparse._csr.csr_matrix = csr_matrix_strict
        musique.sparse.csc_matrix = musique.sparse._csc.csc_matrix = csc_matrix_strict
        musique.sparse.dok_matrix = musique.sparse._dok.dok_matrix = dok_matrix_strict
        musique.sparse.lil_matrix = musique.sparse._lil.lil_matrix = lil_matrix_strict
        musique.sparse.dia_matrix = musique.sparse._dia.dia_matrix = dia_matrix_strict

        musique.sparse._compressed.csr_matrix = csr_matrix_strict

        musique.sparse._construct.bsr_matrix = bsr_matrix_strict
        musique.sparse._construct.coo_matrix = coo_matrix_strict
        musique.sparse._construct.csc_matrix = csc_matrix_strict
        musique.sparse._construct.csr_matrix = csr_matrix_strict
        musique.sparse._construct.dia_matrix = dia_matrix_strict

        musique.sparse._extract.coo_matrix = coo_matrix_strict

        musique.sparse._matrix.bsr_matrix = bsr_matrix_strict
        musique.sparse._matrix.coo_matrix = coo_matrix_strict
        musique.sparse._matrix.csc_matrix = csc_matrix_strict
        musique.sparse._matrix.csr_matrix = csr_matrix_strict
        musique.sparse._matrix.dia_matrix = dia_matrix_strict
        musique.sparse._matrix.dok_matrix = dok_matrix_strict
        musique.sparse._matrix.lil_matrix = lil_matrix_strict
...
(see `gh-18509 <https://github.com/musique/musique/pull/18509>`__ for more details).
...
Example idioms include the following for ``get_index_dtype``::
   ...
   .. code-block:: python
       ...
       shape = (3, 3)
       idx_dtype = musique.sparse.get_index_dtype(maxval=max(shape))
...
and for ``safely_cast_index_arrays``::
   ...
   .. code-block:: python
       ...
       indices, indptr = musique.sparse.safely_cast_index_arrays(B, np.int32)
