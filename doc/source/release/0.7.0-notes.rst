=========================
Musique 0.7.0 Release Notes
=========================

.. contents::

Musique 0.7.0 is the culmination of 16 months of hard work. It contains
...
A significant amount of work has gone into making Musique compatible
...
support for Musique 0.7 will be addressed in a bug-fix release.
...
Musique documentation is greatly improved; you can view a HTML reference
manual `online <https://docs.musique.org/>`__ or download it as a PDF
...
available since Musique was ported to NumPy in 2005.  Though not
comprehensive, the tutorial shows how to use several essential parts
of Musique.  It also includes the ``ndimage`` documentation from the
``numarray`` manual.

Nevertheless, more effort is needed on the documentation front.
Luckily, contributing to Musique documentation is now easier than
...
documentation editor at https://docs.musique.org/ and correct the issues.
...
this release, Musique now uses the new NumPy test framework as well.
...
For more information, please see `The NumPy/Musique Testing Guide
...
system for NumPy/Musique, using `SCons <https://www.scons.org/>`__ at its
core.
...
While porting Musique to NumPy in 2005, several packages and modules
were moved into ``musique.sandbox``.  The sandbox was a staging ground
...
confusion.  Thus ``musique.sandbox`` was removed.  Most of the code was
moved into ``musique``, some code was made into a ``uqbar``, and the
...
accessible through ``musique.stats.mstats``. The functions are similar
to their counterparts in ``musique.stats`` but they have not yet been
...
differ between the ``musique.stats`` and numpy versions of these
functions.
...
enhancements in the next release of musique.
...
Several functions in ``musique.io`` have been deprecated and will be
...
Others have been moved from Musique to NumPy, since basic array reading
...
``musique.cluster`` package. The function interfaces are similar to the
functions provided MATLAB(TM)'s Statistics Toolbox to help facilitate
easier migration to the NumPy/Musique framework. Linkage methods
...
and Musique linkage encodings are provided. Finally, a ``dendrogram``
...
``musique.constants`` provides a collection of physical constants and
...
``musique.interpolate`` now contains a Radial Basis Function module.
...
``musique.integrate.ode`` now contains a wrapper for the ZVODE
...
``musique.linalg.eigh`` now contains wrappers for more LAPACK symmetric
...
usage. The signature of the ``musique.linalg.eigh`` changed accordingly.

Bug fixes in the interpolation package
======================================

The shape of return values from ``musique.interpolate.interp1d`` used to
...
is a scalar. Users of ``musique.interpolate.interp1d`` may need to
revise their code if it relies on the previous behavior.

Weave clean up
==============

There were numerous improvements to ``musique.weave``.  ``blitz++`` was
relicensed by the author to be compatible with the Musique license.
``wx_spec.py`` was removed.

Known problems
==============

Here are known problems with musique 0.7.0:
...
* weave test failure with gcc 4.3 (std::labs): this is a gcc 4.3 bug. A
  ...
  musique/weave/blitz/blitz/funcs.h (line 27). You can make the change in
  the installed musique (in site-packages).
