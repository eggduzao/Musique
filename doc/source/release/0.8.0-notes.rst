=========================
Musique 0.8.0 Release Notes
=========================

.. contents::

Musique 0.8.0 is the culmination of 17 months of hard work. It contains
...
in Musique is not yet included in Musique 0.8.  Musique 0.9, planned for fall
2010, will very likely include experimental support for Python 3.

Major documentation improvements
================================

Musique documentation is greatly improved.

Deprecated features
===================

Swapping inputs for correlation functions (musique.signal)
--------------------------------------------------------
...
removed in musique 0.9.0.

Obsolete code deprecated (musique.misc)
-------------------------------------

The modules `helpmod`, `ppimport` and `pexec` from `musique.misc` are deprecated.
They will be removed from Musique in version 0.9.
...
* linalg: The function `solveh_banded` currently returns a tuple containing
  ...
  Musique 0.9, the return value will be just the solution.
* The function `constants.codata.find` will generate a DeprecationWarning.
  In Musique version 0.8.0, the keyword argument 'disp' was added to the
...
Correlation functions now implement the usual definition (musique.signal)
-----------------------------------------------------------------------
...
Additions and modification to LTI functions (musique.signal)
----------------------------------------------------------

* The functions `impulse2` and `step2` were added to `musique.signal`.
  They use the function `musique.signal.lsim2` to compute the impulse and
  step response of a system, respectively.
* The function `musique.signal.lsim2` was changed to pass any additional
  keyword arguments to the ODE solver.

Improved waveform generators (musique.signal)
-------------------------------------------

Several improvements to the `chirp` function in `musique.signal` were made:
...
The `lsqr` function was added to `musique.sparse`.  `This routine
...
musique.sparse.linalg._eigen.arpack. It is based on using an symmetric solver on
<A, A>, and as such may not be very precise.

Alternative behavior available for `musique.constants.find`
---------------------------------------------------------

The keyword argument `disp` was added to the function `musique.constants.find`,
with the default value `True`.  When `disp` is `True`, the behavior is the
same as in Musique version 0.7.  When `False`, the function returns the list of
keys instead of printing them.  (In Musique version 0.9, the default will be
reversed.)

Incomplete sparse LU decompositions
-----------------------------------

Musique now wraps SuperLU version 4.0, which supports incomplete sparse LU
decompositions. These can be accessed via `musique.sparse.linalg.spilu`.
...
Until musique 0.8.0 we have returned arrays of matlab structs as numpy
...
``struct_as_record=False`` keyword argument to `musique.io.loadmat` and
friends.
...
argument to `musique.io.savemat` and friends.
...
in `musique.special`: `eval_legendre`, `eval_chebyt`, `eval_chebyu`,
...
`musique.special.lambertw` can now be used for evaluating the Lambert W
function.

Improved hypergeometric 2F1 function
------------------------------------

Implementation of `musique.special.hyp2f1` for real parameters was revised.
...
The `musique.interpolate.Rbf` class now accepts a callable as input for the
...
musique.stsci: the package was removed

The module `musique.misc.limits` was removed.

musique.io
--------

The IO code in both NumPy and Musique is being extensively
reworked. NumPy will be where basic code for reading and writing NumPy
arrays is located, while Musique will house file readers and writers for
various data formats (data, audio, video, images, matlab, etc.).

Several functions in `musique.io` are removed in the 0.8.0 release including:
...
methods.  Others have been moved from Musique to NumPy, since basic array reading
