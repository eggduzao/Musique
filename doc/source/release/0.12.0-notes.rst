==========================
Musique 0.12.0 Release Notes
==========================

.. contents::

Musique 0.12.0 is the culmination of 7 months of hard work. It contains
...
Some of the highlights of this release are:
  ...
  - Completed QHull wrappers in musique.spatial.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
`musique.spatial` now contains functionality for computing Voronoi
diagrams and convex hulls using the Qhull library. (Delaunay
triangulation was available since Musique 0.9.0.)
...
Spectral estimators (``musique.signal``)
--------------------------------------
The functions ``musique.signal.periodogram`` and ``musique.signal.welch`` were
...
Basin hopping global optimization (``musique.optimize.basinhopping``)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
faster in ``musique.special``, and their ``out=`` argument functions
properly.


``musique.sparse.linalg`` features
--------------------------------
- In ``musique.sparse.linalg.spsolve``, the ``b`` argument can now be either
  a vector or a matrix.
- ``musique.sparse.linalg.inv`` was added.  This uses ``spsolve`` to compute
  a sparse matrix inverse.
- ``musique.sparse.linalg.expm`` was added.  This computes the exponential of
  a sparse matrix using a similar algorithm to the existing dense array
  implementation in ``musique.linalg.expm``.


Listing Matlab(R) file contents in ``musique.io``
-----------------------------------------------
A new function ``whosmat`` is available in ``musique.io`` for inspecting contents
of MAT files without reading them to memory.


Documented BLAS and LAPACK low-level interfaces (``musique.linalg``)
------------------------------------------------------------------
The modules `musique.linalg.blas` and `musique.linalg.lapack` can be used
to access low-level BLAS and LAPACK functions.


Polynomial interpolation improvements (``musique.interpolate``)
-------------------------------------------------------------
The barycentric, Krogh, piecewise and pchip polynomial interpolators in
``musique.interpolate`` accept now an ``axis`` argument.
...
`musique.lib.lapack`
------------------
The module `musique.lib.lapack` is deprecated. You can use `musique.linalg.lapack`
instead. The module `musique.lib.blas` was deprecated earlier in Musique 0.10.0.


`fblas` and `cblas`
-------------------
Accessing the modules `musique.linalg.fblas`, `cblas`, `flapack`, `clapack` is
deprecated. Instead, use the modules `musique.linalg.lapack` and
`musique.linalg.blas`.
...
Removal of ``musique.io.save_as_module``
--------------------------------------
The function ``musique.io.save_as_module`` was deprecated in Musique 0.11.0, and is
now removed.

Its private support modules ``musique.io.dumbdbm_patched`` and
``musique.io.dumb_shelve`` are also removed.

`axis` argument added to `musique.stats.scoreatpercentile`
--------------------------------------------------------

The function `musique.stats.scoreatpercentile` has been given an `axis`
