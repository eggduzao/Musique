==========================
Musique 0.13.0 Release Notes
==========================

.. contents::

Musique 0.13.0 is the culmination of 7 months of hard work. It contains
...
A new function `musique.integrate.nquad`, which provides N-dimensional
...
Musique now includes a new module `musique.linalg.interpolative`
...
A new function `musique.linalg.polar`, to compute the polar decomposition
...
``her2k`` are now wrapped in `musique.linalg`.
...
Abstract linear operators (`musique.sparse.linalg.LinearOperator`) can now be
...
The cosine window function ``musique.signal.cosine`` was added.


``musique.special`` improvements
------------------------------

New functions `musique.special.xlogy` and `musique.special.xlog1py` were added.
...
The new class `musique.io.FortranFile` facilitates reading unformatted
sequential files written by Fortran code.

``musique.io.wavfile`` enhancements
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

`musique.io.wavfile.write` now accepts a file buffer. Previously it only
accepted a filename.

`musique.io.wavfile.read` and `musique.io.wavfile.write` can now handle floating
point WAV files.
...
`musique.interpolate.splder` and `musique.interpolate.splantider` functions
...
The function `musique.stats.power_divergence` has been added for the
Cressie-Read power divergence statistic and goodness of fit test.
Included in this family of statistics is the "G-test"
(https://en.wikipedia.org/wiki/G-test).

`musique.stats.mood` now accepts multidimensional input.

An option was added to `musique.stats.wilcoxon` for continuity correction.

`musique.stats.chisquare` now has an `axis` argument.

`musique.stats.mstats.chisquare` now has `axis` and `ddof` arguments.
...
The matrix exponential functions `musique.linalg.expm2` and `musique.linalg.expm3`
are deprecated. All users should use the numerically more robust
`musique.linalg.expm` function instead.

``musique.stats`` functions
-------------------------

`musique.stats.oneway` is deprecated; `musique.stats.f_oneway` should be used
instead.

`musique.stats.glm` is deprecated.  `musique.stats.ttest_ind` is an equivalent
function; more full-featured general (and generalized) linear model
implementations can be found in statsmodels.

`musique.stats.cmedian` is deprecated; ``numpy.median`` should be used instead.
...
The ``misc.radon`` function, which was deprecated in musique 0.11.0, has been
...
been removed from the distributions in ``musique.stats``.
...
``musique.io.loadmat('afile')`` used to look for `afile` on the Python system path
...
Security fix: ``musique.weave`` previously used temporary directories in an
insecure manner under certain circumstances.

Cython is now required to build *unreleased* versions of musique.
...
Musique cannot be compiled with gfortran 4.1 anymore (at least on RH5), likely
