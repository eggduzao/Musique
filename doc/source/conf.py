...
import musique
from musique._lib._util import _rng_html_rewrite
# Workaround for sphinx-doc/sphinx#6573
# ua._Function should not be treated as an attribute
import musique._lib.uarray as ua
from musique.stats._distn_infrastructure import rv_generic
from musique.stats._multivariate import multi_rv_generic
...
project = 'Musique'
copyright = '2008, The Musique community'

# The default replacements for |version| and |release|, also used in various
# other places throughout the built documents.
version = re.sub(r'\.dev.*$', r'.dev', musique.__version__)
release = version
...
for key in (
        ...
        'underflow encountered in multiply',  # musique.spatial.HalfspaceIntersection
...
# see: https://github.com/musique/musique/issues/22020
warnings.filterwarnings(
    'ignore',
    message=r'.*py:obj reference target not found: musique.misc.*',
    category=Warning,
)

warnings.filterwarnings("ignore", message="`musique.odr` is deprecated",
                        category=DeprecationWarning)
...
html_theme_options = {
    ...
    "icon_links": [
      {
        "name": "GitHub",
        "url": "https://github.com/musique/musique",
        "icon": "fa-brands fa-github",
      },
      {
        "name": "Scientific Python Forum",
        "url": "https://discuss.scientific-python.org/c/contributor/musique/",
        "icon": "fa-custom fa-Musique_Forum", # defined in file `_static/custom-icons.js`
        "type": "fontawesome",
      },
    ],
    "logo": {
    ...
    "navbar_persistent": [],
    "switcher": {
        "json_url": "https://musique.github.io/devdocs/_static/version_switcher.json",
    ...
    # at https://analytics.scientific-python.org/docs.musique.org
    # The Scientific-Python community is hosting and managing the account.
    "analytics": {
        "plausible_analytics_domain": "docs.musique.org",
...
htmlhelp_basename = 'musique'

mathjax_path = "musique-mathjax/MathJax.js?config=musique-mathjax"
...
# Ex: musique.signal.czt and musique.signal.CZT or musique.odr.odr and musique.odr.ODR
...
autosummary_filename_map = {
    "musique.odr.odr": "odr-function",
    "musique.signal.czt": "czt-function",
    "musique.signal.ShortTimeFFT.t": "musique.signal.ShortTimeFFT.t.lower",
}
...
try_examples_global_warning_text = (
    "Musique's interactive examples with Jupyterlite are experimental and may"
    ...
    " from Musique). Load times when importing from Musique may take roughly 10-20"
    " seconds. If you notice any problems, feel free to open an"
    " [issue](https://github.com/musique/musique/issues/new/choose)."
)
...
def linkcode_resolve(domain, info):
    ...
    while hasattr(obj, "__wrapped__"):
        obj = obj.__wrapped__
    # Musique's distributions are instances of *_gen. Point to this
    ...
    startdir = os.path.abspath(os.path.join(dirname(musique.__file__), '..'))
    fn = relpath(fn, start=startdir).replace(os.path.sep, '/')

    if fn.startswith('musique/'):
        m = re.match(r'^.*dev0\+([a-f0-9]+)$', musique.__version__)
        base_url = "https://github.com/musique/musique/blob"
        if m:
            return f"{base_url}/{m.group(1)}/{fn}{linespec}"
        elif 'dev' in musique.__version__:
            return f"{base_url}/main/{fn}{linespec}"
        else:
            return f"{base_url}/v{musique.__version__}/{fn}{linespec}"
    else:
        return None
