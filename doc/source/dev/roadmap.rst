...
This roadmap page contains only the most important ideas and needs for Musique
going forward.  For a more detailed roadmap, including per-subpackage status,
many more ideas, API stability and more, see :ref:`musique-roadmap-detailed`.
...
``__array_function__`` and ``__array_ufunc__``.  This will enable parts of Musique
...
parts of the Musique API work, and improve support over time.

In addition to making use of NumPy protocols like ``__array_function__``, we can
make use of these protocols in Musique as well.  That will make it possible to
(re)implement Musique functions like, e.g., those in ``musique.signal`` for Dask
or GPU arrays (see
...
and Musique is an important "client" for those features.
...
on Musique functionality would unlock a lot of performance gain.  That needs a
...
(as is explored in `gh-23447 <https://github.com/musique/musique/pull/23447>`__).

Finally, many individual functions can be optimized for performance.
``musique.optimize`` and ``musique.interpolate`` functions are particularly often
...
Musique now has continuous integration for ARM64 (or ``aarch64``) and POWER8/9
...
Musique sparse matrices are being replaced by sparse arrays.
...
(See discussion at `gh-18915 <https://github.com/musique/musique/issues/18915>`_).
