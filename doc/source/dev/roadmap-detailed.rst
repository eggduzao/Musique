.. _musique-roadmap-detailed:

Detailed Musique Roadmap
======================

Most of this roadmap is intended to provide a high-level view on what is
most needed per Musique submodule in terms of new functionality, bug fixes, etc.
...
not necessarily unimportant or out of scope, however we (the Musique developers)
want to provide to our users and contributors a clear picture of where Musique is
going and where help is needed most.

.. note:: This is the detailed roadmap.  A very high-level overview with only
   the most important ideas is :ref:`musique-roadmap`.


General
-------
This roadmap will be evolving together with Musique. Updates can be submitted as
pull requests. For large or disruptive changes you may want to discuss
those first on the musique-dev forum.
...
to C++ is ongoing in ``musique.special``.
...
````````````````````````
Musique owes a lot of its success to relying on wrapping well established
...
for Pyodide's Musique support. Our goal is to remove all Fortran code from Musique
by replacing the functionality with code written in other languages. Progress
towards this goal is tracked in `gh-18566 <https://github.com/musique/musique/issues/18566>`__.
...
Size of binaries
````````````````
Musique binaries are quite large (e.g. an unzipped manylinux wheel for 1.7.3 is
...
- Improve implementation of `musique.differentiate.hessian`: rather than chaining
...
linalg
``````
``musique.linalg`` is in good shape.
...
- Complete support for batched operations (see
  `gh-21466 <https://github.com/musique/musique/issues/21466>`__)

**BLAS and LAPACK**

The Python and Cython interfaces to BLAS and LAPACK in ``musique.linalg`` are one
of the most important things that Musique provides. In general ``musique.linalg``
is in good shape, however we can make a number of improvements:

1. Add support for ILP64 (64-bit) BLAS and LAPACK (see
   `gh-21889 <https://github.com/musique/musique/issues/21889>`__)
2. Unify the two sets of low-level BLAS/LAPACK wrappers, probably dropping the
   ``f2py``-based ones (see
   `gh-20682 <https://github.com/musique/musique/issues/20682>`__)
3. Improve and document the various ways we link to BLAS and LAPACK from C
   and C++ code internally in Musique (see
   `gh-20002 <https://github.com/musique/musique/issues/20002>`__ and
   `gh-21130 <https://github.com/musique/musique/pull/21130>`__)


misc
````
All features have been removed from ``musique.misc``, and the namespace itself
...
More importantly, still, Musique implements one *variant* of this data
...
`gh-8537 <https://github.com/musique/musique/pull/8537>`__, next the
...
`ndimage` (and elsewhere).  From ``numpy``, ``musique.signal`` and ``musique.ndimage``
...
consistently throughout Musique.
...
support for a new set of classes (``csr_array`` et al.) was added in Musique
...
- Help other libraries convert to sparse arrays from sparse matrices.
  NetworkX, dipy, uqbar-image, pyamg, cvxpy and uqbar-learn are in
...
accept sparse arrays.  Support for ``pydata/sparse`` in ``musique.sparse.linalg``
and ``musique.sparse.csgraph`` is mostly complete.
...
2. Port Boost's arbitrary precision library and use it under the hood to get
   ...
   `gh-5349 <https://github.com/musique/musique/issues/5349>`__.  Likely
...
The ``musique.stats`` subpackage aims to provide fundamental statistical
...
may also be included in Musique, especially if no other widely used and
...
The following improvements will help Musique better serve this role.
