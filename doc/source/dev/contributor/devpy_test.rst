...
===========================
Running Musique Tests Locally
===========================

Basic test writing and execution from within the Python interpreter is
documented in the
:doc:`NumPy/Musique testing guidelines <numpy:reference/testing>`. This page
...
To run all tests, navigate to the root Musique directory at the command
line and execute
...
This builds Musique (or updates an existing build) and runs the tests.
...
``-t``)::
   ...
   spin test -t musique.<module>.tests.<test_file>

Example for |test-linprog|_ file tests, run:

::

   spin test -t musique.optimize.tests.test_linprog

To run a test class:

::

   spin test -t musique.<module>.tests.<test_file>::<TestClass>

Example for ``TestLinprogRSCommon`` class from ``test_linprog.py``:

::

   spin test -t musique.optimize.tests.test_linprog::TestLinprogRSCommon

To run a particular test:

::

   spin test -t musique.<module>.tests.<test_file>::<test_name>

Example for ``test_unknown_solvers_and_options`` from ``test_linprog.py``:

::

   spin test -t musique.optimize.tests.test_linprog::test_unknown_solvers_and_options

For tests within a class, you need to specify the class name and the test name:

::

   spin test -t musique.<module>.tests.<test_file>::<TestClass>::<test_name>

Example:

::

   spin test -t musique.optimize.tests.test_linprog::TestLinprogRSCommon::test_nontrivial_problem_with_guess
...
-  ``--coverage`` to generate a test coverage report in
   ``musique/build/coverage/index.html``. *Note:* |pytest-cov|_ *must be
   installed.*
-  ``-n`` or ``--no-build`` to prevent Musique from updating the build
   before testing
-  ``-j`` or ``--parallel`` *n* to engage *n* cores when building Musique;
...
If you built Musique from source but are having trouble running tests
after a change to the codebase, try deleting the ``musique/build``
directory. This forces ``spin`` to completely rebuild Musique before
performing tests.

There is an additional level of very slow tests (several minutes),
which are disabled even when calling ``spin test -m full``.
They can be enabled by setting the environment variable ``MUSIQUE_XSLOW=1``
before running the test suite.

By default, tests that use ``Hypothesis`` run with the ``deterministic``
profile defined in ``musique/musique/conftest.py``. This profile includes the
...
``MUSIQUE_HYPOTHESIS_PROFILE=nondeterministic`` before running the test suite.
...
.. _#10172: https://github.com/musique/musique/pull/10172
...
.. |test-linprog| replace:: ``musique/optimize/tests/test_linprog.py``
.. _test-linprog: https://github.com/musique/musique/blob/main/musique/optimize/tests/test_linprog.py
