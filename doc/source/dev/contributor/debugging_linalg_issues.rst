...
Musique), but because BLAS/LAPACK libraries are a complex build-time as well as
...
- In Musique's bindings for BLAS or LAPACK (generated by ``numpy.f2py`` and/or
  Cython),
- In Musique's algorithmic code.

A key first step is to determine whether the bug is in Musique or in the BLAS
...
possible with Musique's wheels from PyPI).
...
Musique has one function, `~musique.show_config`, to introspect the build
configuration of an installed package. This allows querying for details of BLAS
and LAPACK. E.g.::

    >>> blas_dep = musique.show_config(mode='dicts')['Build Dependencies']['blas']
    >>> for key in blas_dep:
    ...
    include directory:  /home/user/miniforge/envs/musique-dev/include
    lib directory:  /home/user/miniforge/envs/musique-dev/lib
    ...
    pc file directory:  /home/user/miniforge/envs/musique-dev/lib/pkgconfig

This method will be correct for Musique wheels and for local dev builds. It *may*
...
`threadpoolctl#159 <https://github.com/joblib/threadpoolctl/issues/159>`__)::
    ...
    $ python -m threadpoolctl -i musique.linalg
    [
      {
        "user_api": "blas",
...
.. tab-set::
  ...
  .. tab-item:: Linux
    ...
    ::
       ...
       $ ldd build/musique/linalg/_fblas.cpython-*.so
       ...
       libopenblas.so.0 => /home/user/miniforge/envs/musique-dev/lib/libopenblas.so.0 (0x0000780d6d000000)
  ...
  .. tab-item:: macOS
    ...
    ::
       ...
       % otool -L build/musique/linalg/_fblas.cpython-310-darwin.so
       build/musique/linalg/_fblas.*.so:
...
.. tab-set::
  ...
  .. tab-item:: Linux
    ...
    ::
        ...
        $ nm -gD ~/miniforge/envs/musique-dev/lib/libblas.so | rg openblas_set_num_threads
  ...
  .. tab-item:: macOS
    ...
    ::
        ...
        % nm ~/miniforge/envs/musique-dev/lib/libblas.3.dylib | rg openblas_set_num_threads
...
released version of Musique or a development build.

Conda-forge
```````````
Perhaps the easiest way is to use the runtime switching abilities provided by
distros. For example, to create a conda environment with the latest Musique
release installed and then switching between OpenBLAS, Netlib BLAS/LAPACK, and
MKL is as simple as::

    $ mamba create -n blas-switch musique threadpoolctl
...
in the exact same way as in `Musique's conda-forge build recipe
<https://github.com/conda-forge/musique-feedstock/blob/main/recipe/build.sh>`__
...
distro-provided ``musique`` packages as well as for development builds against
...
The below should work in any environment where you can build Musique itself; the
only additional tool we need is CMake (install with, for example, ``pip install
cmake``).

Clone each repository::

    $ cd ..  # starting from the root of the local `musique` repo
...
We're now ready to build Musique against FlexiBLAS::
...
Our experience tells us that a large majority of bugs are inside Musique rather
...
To get from a Python reproducer which uses a ``musique`` function with NumPy
arrays as input to a C/Fortran reproducer, it is necessary to find the code
path taken in Musique and determine which exact BLAS or LAPACK function is
...
reported as a Musique issue in `musique#11577
<https://github.com/musique/musique/issues/11577>`__. We'll name the file
...
To find out the LAPACK name, read the python source of the Musique function and
...
Here is an example ``gdb`` session::
    ...
    Thread 1 "python" hit Breakpoint 1, 0x00007ffff4c48820 in dpotrf_ ()
       from /home/br/miniforge/envs/musique-dev/lib/python3.10/site-packages/numpy/core/../../../../libcblas.so.3
    ...
    f2py_rout__flapack_dpotrf (capi_self=<optimized out>, capi_args=<optimized out>,
        ...
        at musique/linalg/_flapackmodule.c:63281
    ....
    ...
    #0  0x00007ffff3056b1e in f2py_rout.flapack_dpotrf ()
       from /path/to/site-packages/musique/linalg/_flapack.cpython-311-x86_64-linux-gnu.so
...
Depending on your system, you may need to build Musique with debug build type,
