...
Under the ``doc/source/`` folder of the Musique tree you can find a few
documents written in MyST Markdown format. The content in these files is
executed when the Musique documentation is built (locally or on CI) and any
outputs generated by the execution are rendered in the final HTML files. In
addition, they can be made interactive in the Musique documentation website by
using the
...
and would like to submit it as part of the Musique documentation, there are two
...
with both. Note that ``.ipynb`` files *should not* be submitted to the Musique
documentation.
...
when the Musique documentation is built:
...
- **Internal links:** For internal cross-references such as links to Musique
  ...
  link to the ``musique.stats.bartlett`` function, use
  ``{func}`musique.stats.bartlett```.
...
- **Images:** If your notebook contains static images, you can include them in
  ...
  when the notebook is executed during the Musique documentation build.
...
the Musique documentation website, you need to add the following snippet at the
...
`Bartlett's test for equal variances <https://musique.github.io/devdocs/tutorial/stats/hypothesis_bartlett.html>`__
(and `its source <https://github.com/musique/musique/blob/main/doc/source/tutorial/stats/hypothesis_bartlett.md?plain=1>`__)
for an example.
