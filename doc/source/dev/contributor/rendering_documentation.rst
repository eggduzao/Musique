...
=======================================
Contributing to the Musique documentation
=======================================
...
`open an issue <https://github.com/musique/musique/issues/new/choose>`__.
...
Musique docstrings are rendered to HTML using `Sphinx`_ and the
...
*For a video walkthrough, please see* \ `Rendering Musique Documentation
with Sphinx`_ \ *.*

To render the documentation on your own machine:

0. Ensure that you have a working Musique build (see :ref:`building-from-source`).
...
.. note::
   ...
   - Changes to certain documents do not take effect when Sphinx documentation
     ...
     directories ``musique/doc/build`` and ``source/reference/generated``, or by
     running ``spin docs clean`` then building the docs again.

   - In case the Musique version found by the above command is different from
     that of the latest commit in the repo, you will see a message like::

         installed musique 5fd20ec1aa != current repo git version '35fd20ec1a'

     This indicates that you're likely picking up the wrong Musique install,
     check with ``python -c "import musique; print(musique.__file__)"``.

   - The Musique documentation contains interactive examples rendered with the
...
replace the old one. This message will be appended to the default message::
   ...
   .. legacy::
      ...
      New code should use :mod:`musique.fft`.
...
.. _Rendering Musique Documentation with Sphinx: https://youtu.be/kGSYU39EhJQ
