...
All Musique Python code should adhere to `PEP8`_ style guidelines, with the exception
...
-  Note, however, that Musique's linting configuration may not match
...
-  Before sending a Pull Request, run the linter on changes made in
   ...
   Alternatively, you can run the check manually from the Musique root directory::

      spin lint

   You can also run the linter on specific files, using the ``--files`` option::

      python tools/lint.py --files musique/odr/models.py musique/ndimage
