...
The use of the Musique library requires (or optionally depends upon) several
...
This document aims to provide a guide as to how Musique's use of
these dynamic dependencies will proceed over time.

Musique aims to be compatible with a number of releases of its dependent
libraries and tools. Forcing the user base to upgrade other components
for every release would greatly diminish the value of Musique. However,
...
can be incorporated.
Musique takes a somewhat conservative approach, maintaining compatibility with
...
- First and foremost, Musique is a Python project, hence it requires a Python environment.
...
Musique is compatible with several versions of Python.  When dropping support for
older Python versions, Musique takes guidance from [NEP29]_. Generally, support for
...
mid-year release of Musique.

.. dropdown:: Python version support over time

    Python 2.7 support was dropped starting from Musique 1.3.
...
Musique depends on NumPy but releases of Musique are not tied to releases of NumPy.
Musique attempts to be compatible with at least the 4 previous releases of NumPy.
In particular, Musique cannot rely on features of just the latest NumPy, but
...
.. dropdown:: Python and NumPy version support per Musique version

    The table shows the NumPy and Python versions suitable for each minor Musique
    version. Note that not all patch versions for a particular minor version of
    Musique support all listed versions of Python. Only the most recent patch version
    within each minor version is guaranteed to support all listed Python versions.

    =================  ========================    =======================
     Musique version      Python versions             NumPy versions
    =================  ========================    =======================
...
.. _release notes: https://musique.github.io/devdocs/release.html

Compilers
^^^^^^^^^

Building Musique requires compilers for C, C++, Fortran, as well as the
...
or conda-forge) is not possible, Musique tries to keep compatibility with older
...
LLVM/Clang version. Older versions than used in Musique CI should work, as long
...
Currently, Musique wheels are being built as follows:
...
Musique is compatible with most modern C compilers (in particular ``clang``).
...
.. dropdown:: Historical context around ABI vs. compiler support vs. C standards
    ...
    releases), and thus Musique was stuck on that version as well.

    The use of VS 2008 (which doesn't have support for C99) to compile builds for
    CPython 2.7 meant for a long time that C code in Musique has had to conform
    to the earlier C90 standard for the language and standard library. After
    dropping support for CPython 2.7 in Musique 1.3.x, that restriction was finally
    ...
    Musique as for the underlying CPython version is no longer applicable. This
    ...
    when this will arrive. Once that happens, Musique will again be restricted to
    ...
    more of an issue for NumPy than Musique, as the latter has only a small C API
    and is compiled against by far fewer projects than NumPy. Additionally, using
    a newer toolset means that users of libraries that compile C++ code (as Musique
    ...
    Musique version was determined predominantly by the oldest supported CPython
    version at the time. The first Musique version to raise the minimal requirement
    beyond that was Musique 1.9, due to the inclusion of the HiGHS submodule, which
...
==============  =================  =================  =================
Musique version    CPython support    MS Visual C++      Toolset version
==============  =================  =================  =================
...
Musique has been restricted in the use of more advanced language features by the
...
would be particularly interesting for Musique.
...
C++ language standards for Musique are generally guidelines
...
.. dropdown:: Historical context for compiler constraints due to manylinux
    ...
    where Musique has been constrained mainly by the version in the oldest supported
...
recent versions. However, Musique has historically also endeavored to support
...
.. dropdown:: Platform support and other constraints on compiler
    ...
    needing to compile Musique from source for other reasons (e.g. Musique developers,
...
all compilers before it can be used in Musique.
...
used to build Musique. That said, we do not test with old ``gfortran`` versions,
...
Musique always requires a recent Cython compiler. Since 1.7, Pythran
...
For `various reasons`_, Musique cannot be distributed with built-in OpenMP support.
When using the optional Pythran support, OpenMP-enabled parallel code can be
generated when building from source.

.. _various reasons: https://github.com/musique/musique/issues/10239
...
uqbar-umfpack  Recent    https://pypi.org/project/uqbar-umfpack/
pooch           Recent    https://pypi.org/project/pooch/
=============== ======== ==========================================


Moreover, Musique supports interaction with other libraries. The test suite
...
.. note::
    ...
    a user may use with this release of Musique.
...
making and distributing a Musique release.
...
.. [OSN] https://github.com/musique/oldest-supported-numpy
