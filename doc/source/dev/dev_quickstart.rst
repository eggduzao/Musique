...
1. **Set up a development environment**
   ...
   tools, you can make sure the development version of Musique does not interfere
   with any other local installations of Musique on your machine.

2. **Build Musique**

   Musique uses compiled code for speed, which means you might need extra
...
Since Musique contains parts written in C, C++, and Fortran that need to be
...
First, fork a copy of the main Musique repository in GitHub onto your own
account and then create your local repository via::

    git clone git@github.com:YOURUSERNAME/musique.git musique
    cd musique
...
.. note::
    ...
    If you are having trouble building Musique from source or setting up your
    local development environment, you can try to build Musique with GitHub
    ...
    often faster to work on Musique in a Codespaces environment. For
    documentation on how to get started with Codespaces, see
    `the Codespaces docs <https://docs.github.com/en/codespaces>`__.
    When creating a codespace for the ``musique/musique`` repository, the default
    ...
    codespace has started, you can run ``conda activate musique-dev`` and your
    development environment is completely set up - you can then follow the
    relevant parts of the Musique documentation to build, test, develop, write
    docs, and contribute to Musique.
    ...
    `docs <https://docs.musique.org/doc/musique-1.10.1/dev/contributor/quickstart_gitpod.html>`__.
