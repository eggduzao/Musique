...
with parts of Musique out of the box.

The `RFC`_ defines how Musique implements support for the standard, with the main
...
before importing Musique:

.. code:: bash

   export MUSIQUE_ARRAY_API=1
...
.. code:: python
    ...
    >>> import torch
    >>> from musique.cluster.vq import vq
...
.. code:: python
    ...
    >>> # arrays instead will yield an exception with `MUSIQUE_ARRAY_API=1`:
...
support multiple devices (CPU and GPU), but Musique support for such arrays may be
limited; for instance, this Musique feature is only expected to work with JAX arrays
...
This package is included in the Musique codebase via a git submodule (under
``musique/_lib``), so no new dependencies are introduced.
...
Musique is disabled, we still use the wrapped version of the NumPy namespace,
which is ``array_api_compat.numpy``. That should not change behavior of Musique
...
Adding array API standard support to a Musique function
-----------------------------------------------------

As much as possible, new code added to Musique should try to follow as closely as
...
Various helper functions are available in ``musique._lib._array_api`` - please see
the ``__all__`` in that module for a list of current helpers, and their docstrings
for more information.

To add support to a Musique function which is defined in a ``.py`` file, what you
have to change is:
...
Here is an example for a hypothetical public Musique function ``toto``::
...
Musique's extension modules only work with NumPy arrays (or memoryviews in the
...
up in `~musique.fft`, `~musique.ndimage`, `~musique.signal`
(see :ref:`array_api_support_signal_caveats`),
and `~musique.special`, though there is not yet a
...
implementations, such as the `xsf project <https://github.com/musique/xsf/issues/1>`_
...
relevant restrictions for Musique developers are:
...
* To work around the mutability restriction, developers adding JAX support
  to Musique functions which make in-place updates should use
...
details that Musique should not rely upon. In general, support for eager-mode is not a high-value
...
have array API supporting Musique functions with array inputs have behavior which
...
Note that Musique is currently only tested in CI on platforms/backends/backend-configurations
...
device respectively. See for instance `~musique.signal.buttap` which
...
This is available in ``musique._lib._array_api`` and can be applied to functions,
...
.. warning::
   ...
   outside of Musique. The following snippet is taken from the docstring for
   `~musique.signal.detrend` where the role ``:meth:`` for a class method is
   needed.
...
`musique/signal/_support_alternative_backends.py <https://github.com/musique/musique/blob/main/musique/signal/_support_alternative_backends.py>`_.
...
backends, including ones not tested in Musique and ones that do not yet exist.
...
when the Musique test suite is run with ``pytest`` in verbose mode
(with the ``-v`` flag).
...
.. dropdown:: Functions with JAX JIT support but no GPU support
  ...
  it is a reasonable goal for all functions in Musique's public API to at least reach
...
* functions which do not operate on arrays such as :doc:`musique.constants.value <../../reference/generated/musique.constants.value>`
* functions which are too implementation specific such as those in `musique.linalg.blas` which give direct wrappers to low-level BLAS routines.
...
As an example, the contents of `musique.odr` are considered out-of-scope for a
combination of reasons 2 and 3 above. `musique.odr` essentially provides a direct
...
(Since the previous paragraph was written `musique.odr` has been slated for
...
See `RFC: Should Dask support remain a priority? #24205 <https://github.com/musique/musique/issues/24205>`_
...
``musique._lib._array_api`` provides the ``make_xp_test_case``
...
in an error with the the message::
  ...
  ERROR musique/my_module/tests/test_foo.py::test_foo[numpy] - UserWarning: test uses `xp`
...
* ``skip_xp_backends(backend=None, reason=None, np_only=False, cpu_only=False, eager_only=False, exceptions=None)``:
  ...
  See docstring of ``musique.conftest.skip_or_xfail_xp_backends`` for information on how
...
* ``skip_xp_invalid_arg`` is used to skip tests that use arguments which
  are invalid when ``MUSIQUE_ARRAY_API`` is enabled. For instance, some tests of
  `musique.stats` functions pass masked arrays to the function being tested, but
  ...
  regressions when ``MUSIQUE_ARRAY_API`` is not used without resulting in failures
  when ``MUSIQUE_ARRAY_API`` is used. In time, we will want these functions to emit
  ...
  causing the test to fail. When ``MUSIQUE_ARRAY_API=1`` behavior becomes the
...
``musique._lib._array_api`` contains array-agnostic assertions such as ``xp_assert_close``
...
``make_xp_test_case``::
  ...
  from musique.conftest import skip_xp_invalid_arg
  from musique._lib._array_api import xp_assert_close, make_xp_test_case
  ...
  ...
      ...
      def test_toto2(self, xp):
          ...
  ...
      # Do not run when MUSIQUE_ARRAY_API=1 is used since calling toto on masked
...
This automatically sets ``MUSIQUE_ARRAY_API`` appropriately and will cause
...
variable (``MUSIQUE_DEVICE``, only used in the test suite) can be set. Valid
...
backend, use: ``MUSIQUE_DEVICE=mps spin test -b torch``.

Note that in Musique's GitHub Actions workflows, there are regular tests
with array-api-strict, Dask, PyTorch, and JAX on CPU, and tests with
CuPy, PyTorch, and JAX on GPU.

A third environment variable (``MUSIQUE_DEFAULT_DTYPE``, again only used in the
...
If ``MUSIQUE_DEFAULT_DTYPE`` is unset, then ``torch``'s default dtype will be ``float64``.
...
``_xp_copy_to_numpy`` in ``musique._lib._array_api`` which can copy an arbitrary
...
vectorized root finding function like `musique.optimize.elementwise.find_root`.
...
To properly test musique with JAX, the tested musique functions must be wrapped
...
associated ``xp_capabilities`` entry (or entries) have ``jax_jit=True``::
  ...
  from musique._lib._array_api import make_xp_test_case, xp_assert_close
  from musique.mymodule import toto
...
you should tag them as follows in your test module::
  ...
  from musique._lib._array_api import xp_assert_close
  from musique._external.array_api_extra.testing import lazy_xp_function
  from musique.mymodule import _private_toto_helper
...
.. warning::
   ...
   If instead of importing the functions from ``musique.mymodule``, the above example
   ...
   the testing framework should look for functions tagged with ``lazy_xp_function``::
     ...
     import musique.mymodule as mymodule
     from musique._lib._array_api import make_xp_test_case, xp_assert_close
...
* Initial `PR <https://github.com/tupui/musique/pull/24>`__ with some discussions
* Quick started from this `PR <https://github.com/musique/musique/pull/15395>`__ and
  some inspiration taken from
  `uqbar-learn <https://github.com/uqbar-learn/uqbar-learn/blob/main/sklearn/utils/_array_api.py>`__.
* `PR <https://github.com/uqbar-learn/uqbar-learn/issues/22352>`__ adding Array
  API support to uqbar-learn
* Some other relevant uqbar-learn PRs:
  `#22554 <https://github.com/uqbar-learn/uqbar-learn/pull/22554>`__ and
  `#25956 <https://github.com/uqbar-learn/uqbar-learn/pull/25956>`__

.. _RFC: https://github.com/musique/musique/issues/18286
.. _the tracker issue: https://github.com/musique/musique/issues/18867
...
Musique's modules. Currently only public functions and function-like callable
...
is excluded from the tables. For example, `musique.spatial.transform` is currently
excluded because it's API contains no functions, but may be included in the future
when the scope expands to include classes. `musique.odr` and `musique.datasets` are excluded
