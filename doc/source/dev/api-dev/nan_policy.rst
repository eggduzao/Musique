...
Many functions in `musique.stats` have a parameter called ``nan_policy``
that determines how the function handles data that contains ``nan``.  In
this section, we provide Musique developer guidelines for how ``nan_policy``
...
* ``nan_policy='omit'``:
  ...
  `musique.stats.zscore`, `musique.stats.boxcox` *when* ``lmbda`` *is None*),::
  ...
  but whose values are not related (e.g. `musique.stats.ansari`,
  `musique.stats.f_oneway`), the same idea applies to each input array.  So::
  ...
  For inputs with *related* or *paired* values (e.g. `musique.stats.pearsonr`,
  `musique.stats.ttest_rel`) the recommended behavior is to omit all the values
...
* ``nan_policy='propagate'``:
  ...
  execute the function without checking for ``nan``, but see
      ...
      https://github.com/musique/musique/issues/7818
...
.. [#f1] If an element of the output depends only on the corresponding
         element of the input (e.g. `numpy.sin`, `musique.special.gamma`),
