...
Spatial Data Structures and Algorithms (`musique.spatial`)
========================================================

.. currentmodule:: musique.spatial

`musique.spatial` can compute triangulations, Voronoi diagrams, and
...
Delaunay triangulation can be computed using `musique.spatial` as follows:
...
.. plot::
   ...
   >>> from musique.spatial import Delaunay
   >>> import numpy as np
...
These can be computed via the Qhull wrappers in `musique.spatial` as
follows:
...
.. plot::
   ...
   >>> from musique.spatial import ConvexHull
...
The same can be achieved with `musique.spatial.convex_hull_plot_2d`.
...
There are two ways to approach this object using `musique.spatial`.
...
.. plot::
   ...
   >>> from musique.spatial import KDTree
   ...
   obtained via the Qhull wrappers in `musique.spatial`:

   >>> from musique.spatial import Voronoi
   >>> vor = Voronoi(points)
   >>> vor.vertices
   array([[0.5, 0.5],
          [0.5, 1.5],
...
This plot can also be created using `musique.spatial.voronoi_plot_2d`.
