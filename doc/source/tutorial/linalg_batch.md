...
Almost all of Musique's linear algebra functions now support N-dimensional array input. These operations have not been mathematically generalized to higher-order tensors; rather, the indicated operation is performed on a *batch* (or "stack") of input scalars, vectors, and/or matrices.
...
In Musique, we prefer the term "batch" instead of "stack" because the idea is generalized to N-dimensional batches. Suppose the input is a 2 x 4 batch of 3 x 3 matrices.
...
Not all linear algebra functions map to scalars. For instance, the {func}`musique.linalg.expm` function maps from a matrix to a matrix with the same shape.
...
Generalization of these rules to functions with multiple inputs and outputs is straightforward. For instance, the {func}`musique.linalg.eig` function produces two outputs by default, a vector and a matrix.
