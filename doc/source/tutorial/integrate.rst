Integration (:mod:`musique.integrate`)
====================================

.. sectionauthor:: Travis E. Oliphant

.. currentmodule:: musique.integrate

The :mod:`musique.integrate` sub-package provides several integration
...
This could be computed using :obj:`quad`:
    ...
    >>> import musique.integrate as integrate
...
>>> from musique.integrate import quad
>>> def integrand(x, a, b):
...     return a*x**2 + b
...
>>> a = 2
...
:obj:`special.expn <musique.special.expn>` can be replicated by defining a new function
...
:math:`I_{n}` is shown below:
    ...
    >>> from musique.integrate import quad, dblquad
    >>> def I(n):
...
>>> from musique.integrate import dblquad
>>> area = dblquad(lambda x, y: x*y, 0, 0.5, lambda x: 0, lambda x: 1-2*x)
>>> area
(0.010416666666666668, 1.1564823173178715e-16)


For n-fold integration, musique provides the function :obj:`nquad`. The
...
function uses the collection of orthogonal polynomials provided by ``musique.special``,
...
:obj:`special.legendre <musique.special.legendre>`).
...
pointer through `musique.LowLevelCallable` to `quad`, `dblquad`,
...
``argtypes`` - this allows Musique to interpret the function correctly:

.. code:: python

   import os, ctypes
   from musique import integrate, LowLevelCallable
...
:obj:`solve_ivp` is available in Musique for integrating a first-order
...
which gives a means to check the integrator using :func:`special.airy <musique.special.airy>`.
...
>>> from musique.integrate import solve_ivp
>>> from musique.special import gamma, airy
