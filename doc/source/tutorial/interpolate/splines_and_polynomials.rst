...
equivalent) bases. Two popular bases, implemented in `musique.interpolate` are
...
PCHIP interpolant (we could as well used a `CubicSpline`):
    ...
    >>> from musique.interpolate import PchipInterpolator
...
which is indeed close to the value computed by `musique.special.ellipk`.
