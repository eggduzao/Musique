.. currentmodule:: musique.interpolate
...
consistent among different routines in `musique.interpolate`. Different
...
Replicate `numpy.interp` left and right fill values
===================================================
...
``xnew < x[0]`` and ``y[-1]`` for ``xnew > x[-1]``. The values are also
controllable by optional ``left`` and ``right`` keyword arguments.

While ``musique.interpolate`` interpolators do not offer this behavior out of the box,
it is straightforward to emulate manually: we first compute the interpolation, and
then explicitly fill the required values for ``xnew < min(x)`` and ``x > max(x)`` with
...
.. plot::
    ...
    def clipped(xnew, func, x, y, left=None, right=None):
        if left is None:
            left = y[0]
        if right is None:
            right = y[-1]
    ...
    bare_func = make_interp_spline(x, y)
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.interpolate import CubicSpline
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.optimize import brentq
...
`musique.optimize.brentq` become prohibitively expensive.
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.interpolate import BPoly
...
`musique.spatial` tools) and use it to find nearest neighbors on the convex hull
of the data.
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.interpolate import CloughTocher2DInterpolator as CT
