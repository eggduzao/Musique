...
- lower-level FITPACK replacements for {class}`musique.interpolate.interp2d` for legacy bug-for-bug compatible {class}`musique.interpolate.interp2d` replacements;
- recommended replacements for {class}`musique.interpolate.interp2d` for use in new code;
...
>>> from musique.interpolate import interp2d, RectBivariateSpline
...
#### Replacement: Use {class}`musique.interpolate.bisplrep` / {class}`musique.interpolate.bisplev` directly

```
>>> from musique.interpolate import bisplrep, bisplev
...
>>> from musique.interpolate import SmoothBivariateSpline, LinearNDInterpolator
