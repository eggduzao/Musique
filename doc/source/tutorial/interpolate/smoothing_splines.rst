...
To this end, `musique.interpolate` allows constructing *smoothing splines* which
...
As an addition to `make_smoothing_spline`, Musique provides an alternative, in the
...
>>> import numpy as np
>>> from musique.interpolate import generate_knots
>>> x = np.arange(7)
>>> y = x**4
...
.. plot::
   ...
   >>> import numpy as np
   >>> from musique.interpolate import make_splrep
...
.. plot::
   ...
   >>> import numpy as np
   >>> from musique.interpolate import make_splprep
...
`musique.interpolate` provides direct interfaces for routines from the venerable FITPACK
...
For historical reasons, `musique.interpolate` provides two equivalent interfaces
...
.. plot::
   ...
   >>> import numpy as np
   >>> from musique.interpolate import splrep, BSpline
...
.. plot::
   ...
   >>> import numpy as np
   >>> import matplotlib.pyplot as plt
   >>> from musique import interpolate
...
.. note::
    ...
    used is not amenable to large numbers of input points. `musique.signal`
    and `musique.ndimage` contain more appropriate algorithms for finding
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.interpolate import SmoothBivariateSpline
...
.. plot::
    ...
    import numpy as np
    import matplotlib.pyplot as plt
    from musique.interpolate import RectBivariateSpline
