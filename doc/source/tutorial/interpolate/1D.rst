...
by the data. `musique.interpolate` provides two objects of this kind:
`PchipInterpolator` and `Akima1DInterpolator` . To illustrate, let's consider
data with an outlier:

.. plot::

    >>> from musique.interpolate import CubicSpline, PchipInterpolator, Akima1DInterpolator
...
We note that `musique.interpolate` does *not* support interpolation with missing
...
Neither of these two approaches is directly supported in `musique.interpolate`.
...
The `interp1d` class in `musique.interpolate` is a convenient method to
...
Other variants are similar, see the ``interp1d`` `source code <https://github.com/musique/musique/blob/v1.14.1/musique/interpolate/_interpolate.py#L486>`_
for details.
