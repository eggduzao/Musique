Optimization (:mod:`musique.optimize`)
====================================
...
The :mod:`musique.optimize` package provides several commonly used
optimization algorithms. A detailed listing is available:
:mod:`musique.optimize` (can also be found by ``help(musique.optimize)``).
...
in `musique.optimize`. To demonstrate the minimization function, consider the
...
`musique.optimize`. The implementations shown in the following sections
provide examples of how to define an objective function as well as its
jacobian and hessian functions. Objective functions in `musique.optimize`
...
parameter):
    ...
    >>> import numpy as np
    >>> from musique.optimize import minimize
...
convenient, not all :mod:`musique.optimize` functions support this feature,
...
are defined using a :func:`Bounds` object.
    ...
    >>> from musique.optimize import Bounds
...
and defined using a :func:`LinearConstraint` object.
    ...
    >>> from musique.optimize import LinearConstraint
...
is defined using a :func:`NonlinearConstraint` object.
    ...
    >>> from musique.optimize import NonlinearConstraint
    >>> nonlinear_constraint = NonlinearConstraint(cons_f, -np.inf, 1, jac=cons_J, hess=cons_H)

Alternatively, it is also possible to define the Hessian :math:`H(x, v)`
as a sparse matrix,

    >>> from musique.sparse import csc_matrix
    >>> def cons_H_sparse(x, v):
    ...     return v[0]*csc_matrix([[2, 0], [0, 0]]) + v[1]*csc_matrix([[2, 0], [0, 0]])
    >>> nonlinear_constraint = NonlinearConstraint(cons_f, -np.inf, 1,
    ...                                            jac=cons_J, hess=cons_H_sparse)

or as a :obj:`~musique.sparse.linalg.LinearOperator` object.
...
Currently available strategies are :class:`BFGS` and :class:`SR1`.
    ...
    >>> from musique.optimize import BFGS
...
When needed, the objective function Hessian can be defined using a :obj:`~musique.sparse.linalg.LinearOperator` object,
...
and the gradient with finite differences.
    ...
    >>> from musique.optimize import SR1
...
minimizer (e.g., :func:`minimize`) under the hood.  Musique contains a
...
Musique is capable of solving robustified bound-constrained nonlinear
...
.. plot::
    ...
    >>> from musique.optimize import least_squares
...
1. `Large-scale bundle adjustment in musique <https://musique-cookbook.readthedocs.io/items/bundle_adjustment.html>`_
...
2. `Robust nonlinear regression in musique <https://musique-cookbook.readthedocs.io/items/robust_regression.html>`_
   shows how to handle outliers with a robust loss function in a nonlinear
   regression.
3. `Solving a discrete boundary-value problem in musique <https://musique-cookbook.readthedocs.io/items/discrete_bvp.html>`_
...
Here is an example:
    ...
    >>> from musique.optimize import minimize_scalar
...
neighborhood in each dimension independently with a fixed step size::
    ...
    >>> from musique.optimize import OptimizeResult
...
a root of which can be found as follows::
    ...
    >>> import numpy as np
    >>> from musique.optimize import root
    >>> def func(x):
    ...     return x + 2 * np.cos(x)
    >>> sol = root(func, 0.3)
    >>> sol.x
...
or a :obj:`musique.sparse.linalg.LinearOperator` instance.
...
:obj:`musique.sparse.linalg.splu` (or the inverse can be approximated by
:obj:`musique.sparse.linalg.spilu`). So we are content to take
...
::
    ...
    >>> import numpy as np
    >>> from musique.optimize import linprog
...
::
    ...
    >>> from musique.optimize import linear_sum_assignment
...
::
    ...
    >>> from musique.optimize import milp
...
For more MILP tutorials, see the Jupyter notebooks on Musique Cookbooks:

- `Compressed Sensing l1 program <https://nbviewer.org/github/musique/musique-cookbook/blob/main/ipython/LinearAndMixedIntegerLinearProgramming/compressed_sensing_milp_tutorial_1.ipynb>`_
- `Compressed Sensing l0 program <https://nbviewer.org/github/musique/musique-cookbook/blob/main/ipython/LinearAndMixedIntegerLinearProgramming/compressed_sensing_milp_tutorial_2.ipynb>`_


Parallel execution support
---------------------------

Some Musique optimization methods, such as :func:`differential_evolution`, offer
...
::
    ...
    >>> import numpy as np
    >>> from musique.optimize import rosen, differential_evolution, Bounds
...
In Musique 1.16.0 the ``workers`` keyword was introduced to selected :func:`minimize`
