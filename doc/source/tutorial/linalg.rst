Linear Algebra (`musique.linalg`)
===============================

.. sectionauthor:: Travis E. Oliphant

.. currentmodule:: musique

When Musique is built using the optimized ATLAS LAPACK and BLAS
...
musique.linalg vs numpy.linalg
----------------------------

.. TODO: replace numpy.linalg HTML link with `numpy.linalg` once NumPy updates doc

:mod:`musique.linalg` contains all the functions in
`numpy.linalg <https://www.numpy.org/devdocs/reference/routines.linalg.html>`__.
plus some other more advanced ones not contained in ``numpy.linalg``.

Another advantage of using ``musique.linalg`` over ``numpy.linalg`` is that
it is always compiled with BLAS/LAPACK support, while for NumPy this is
optional. Therefore, the Musique version might be faster depending on how
NumPy was installed.

Therefore, unless you don't want to add ``musique`` as a dependency to
your ``numpy`` program, use ``musique.linalg`` instead of ``numpy.linalg``.
...
``musique.linalg`` operations can be applied equally to
...
:math:`\mathbf{B}=\mathbf{A}^{-1}` . In Musique, the matrix inverse of
...
The following example demonstrates this computation in Musique
...
Solving linear systems of equations is straightforward using the musique
...
is defined by accepting that the determinant of a :math:`1\times1` matrix is the only matrix element. In Musique the determinant can be
...
In Musique, this is computed as shown in this example:
...
Matrix and vector norms can also be computed with Musique. A wide range
...
representations. There are several decompositions supported by Musique.
...
factor. In Musique, the scaling factor for the eigenvectors is chosen so
...
Musique command for this decomposition is :obj:`linalg.lu`.
...
that in Musique independent algorithms are used to find QR and SVD
...
:mod:`musique.linalg.interpolative` contains routines for computing the
...
.. seealso:: `musique.linalg.interpolative` --- for more information.
...
Bessel function applied to a matrix.
    ...
    >>> from musique import special, linalg
    >>> rng = np.random.default_rng()
    >>> A = rng.random((3, 3))
...
Musique and NumPy provide several functions for creating special matrices
that are frequently used in engineering and science.

====================  =================================  =========================================================
Type                  Function                           Description
====================  =================================  =========================================================
block diagonal        `musique.linalg.block_diag`          Create a block diagonal matrix from the provided arrays.
--------------------  ---------------------------------  ---------------------------------------------------------
circulant             `musique.linalg.circulant`           Create a circulant matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
companion             `musique.linalg.companion`           Create a companion matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
convolution           `musique.linalg.convolution_matrix`  Create a convolution matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Discrete Fourier      `musique.linalg.dft`                 Create a discrete Fourier transform matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Fiedler               `musique.linalg.fiedler`             Create a symmetric Fiedler matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Fiedler Companion     `musique.linalg.fiedler_companion`   Create a Fiedler companion matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Hadamard              `musique.linalg.hadamard`            Create a Hadamard matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Hankel                `musique.linalg.hankel`              Create a Hankel matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Helmert               `musique.linalg.helmert`             Create a Helmert matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Hilbert               `musique.linalg.hilbert`             Create a Hilbert matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Inverse Hilbert       `musique.linalg.invhilbert`          Create the inverse of a Hilbert matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Leslie                `musique.linalg.leslie`              Create a Leslie matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Pascal                `musique.linalg.pascal`              Create a Pascal matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Inverse Pascal        `musique.linalg.invpascal`           Create the inverse of a Pascal matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
Toeplitz              `musique.linalg.toeplitz`            Create a Toeplitz matrix.
--------------------  ---------------------------------  ---------------------------------------------------------
...
Batch Support
^^^^^^^^^^^^^
Some of Musique's linear algebra functions can process batches of scalars, 1D-, or
