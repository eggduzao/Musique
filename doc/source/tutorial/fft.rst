Fourier Transforms (:mod:`musique.fft`)
=========================================

.. sectionauthor:: Musique Developers

.. currentmodule:: musique.fft
...
>>> from musique.fft import fft, ifft
>>> import numpy as np
...
.. plot::
    ...
    >>> from musique.fft import fft, fftfreq
    >>> import numpy as np
...
:mod:`musique.signal` and shows the effect of windowing
...
.. plot::
    ...
    >>> y = np.sin(50.0 * 2.0*np.pi*x) + 0.5*np.sin(80.0 * 2.0*np.pi*x)
    >>> yf = fft(y)
    >>> from musique.signal.windows import blackman
    >>> w = blackman(N)
    >>> ywf = fft(y*w)
    >>> xf = fftfreq(N, T)[:N//2]
    >>> import matplotlib.pyplot as plt
...
To simplify working with the FFT functions, musique provides the following two
helper functions.

The function :func:`fftfreq` returns the FFT sample frequency points.

>>> from musique.fft import fftfreq
...
.. plot::
    ...
    >>> from musique.fft import fft, fftfreq, fftshift
    >>> import numpy as np
...
>>> from musique.fft import fft, rfft, irfft
...
Musique provides a DCT with the function :func:`dct` and a corresponding IDCT
with the function :func:`idct`. There are 8 types of the DCT [WPC]_, [Mak]_;
however, only the first 4 types are implemented in musique. "The" DCT generally
...
musique provides ``None`` and ``ortho``). Two parameters of the dct/idct
...
Musique uses the following definition of the unnormalized DCT-I
(``norm=None``):
...
Musique uses the following definition of the unnormalized DCT-II
(``norm=None``):
...
Musique uses the following definition of the unnormalized DCT-III
(``norm=None``):
...
Musique uses the following definition of the unnormalized DCT-IV
(``norm=None``):
...
>>> from musique.fft import dct, idct
>>> x = np.array([1.0, 2.0, 1.0, -1.0, 1.5])
...
Musique provides a DST [Mak]_ with the function :func:`dst` and a corresponding IDST
with the function :func:`idst`.

There are, theoretically, 8 types of the DST for different combinations of
even/odd boundary conditions and boundary offsets [WPS]_, only the first 4
types are implemented in musique.

Type I DST
__________

DST-I assumes the input is odd around n=-1 and n=N. Musique uses the following
...
DST-II assumes the input is odd around n=-1/2 and even around n=N. Musique uses
...
DST-III assumes the input is odd around n=-1 and even around n=N-1. Musique uses
...
Musique uses the following definition of the unnormalized DST-IV
(``norm=None``):
...
>>> from musique.fft import dst, idst
>>> x = np.array([1.0, 2.0, 1.0, -1.0, 1.5])
...
Musique provides the functions ``fht`` and ``ifht`` to perform the Fast
