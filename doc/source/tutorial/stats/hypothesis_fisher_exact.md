...
{class}`Fisher's test <musique.stats.fisher_exact>`.
...
{class}`Boschloo's test <musique.stats.boschloo_exact>` does not depend on the
assumption that the row sums are fixed, and consequently, it provides a more
powerful test in this situation.

```{code-cell}
from musique.stats import boschloo_exact
