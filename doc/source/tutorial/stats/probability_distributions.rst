...
*Musique has two infrastructures for working with probability distributions.
...
:mod:`musique.stats` and a fairly complete listing of these functions and random
...
In the code samples below, we assume that the :mod:`musique.stats` package
is imported as
...
To learn more about the random number samplers implemented in Musique, see
...
formulas or through special functions in ``musique.special`` or
...
The distributions in ``musique.stats`` have recently been corrected and improved
...
Next, we can use a chi-squared test, :func:`musique.stats.chisquare`, to test the null
