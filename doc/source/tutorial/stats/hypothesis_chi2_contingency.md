...
{class}`chi-square test <musique.stats.contingency.chi2_contingency>` with the
...
effect of placebo". Because {class}`musique.stats.contingency.chi2_contingency`
performs a two-sided test, the alternative hypothesis does not indicate the
direction of the effect. We can use {class}`musique.stats.contingency.odds_ratio`
