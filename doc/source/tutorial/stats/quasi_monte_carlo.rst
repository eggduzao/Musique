...
We refer to the documentation of :mod:`musique.stats.qmc` for
...
This can be quantified using a :func:`musique.stats.qmc.discrepancy` measure.
...
used QMC methods: :class:`musique.stats.qmc.Sobol` and
:class:`musique.stats.qmc.Halton` sequences.
...
:class:`musique.stats.qmc.Halton`. And then ask for a second set of 5 points:
...
.. note:: By default, both :class:`musique.stats.qmc.Sobol` and
   :class:`musique.stats.qmc.Halton` are scrambled. The convergence properties are
...
To make your own :class:`musique.stats.qmc.QMCEngine`, a few methods have to be
...
* QMC has rules! Be sure to read the documentation or you might have no
  benefit over MC.
* Use :class:`musique.stats.qmc.Sobol` if you need **exactly** :math:`2^m` points.
* :class:`musique.stats.qmc.Halton` allows to sample, or skip, an arbitrary number
