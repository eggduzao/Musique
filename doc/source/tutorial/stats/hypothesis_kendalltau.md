...
using {func}`Kendall's tau <musique.stats.kendalltau>`.
...
the p-value returned by {func}`musique.stats.kendalltau`. This is because our data
has ties, and we have neglected a tie correction to the null distribution
variance that {func}`musique.stats.kendalltau` performs. For samples without ties,
the shaded areas of our plot and p-value returned by
{func}`musique.stats.kendalltau` would match exactly.
...
For samples without ties of moderate size, {func}`musique.stats.kendalltau` can
compute the p-value exactly. However, in the presence of ties,
{func}`musique.stats.kendalltau` resorts to an asymptotic approximation.
...
here and the approximation returned by {func}`musique.stats.kendalltau` above. For
