...
The Jarque-Bera test {class}`musique.stats.jarque_bera` begins by computing a
...
{class}`chi-squared distribution <musique.stats.chi2>` with two degrees of freedom.
...
observations. For small samples like ours, {class}`musique.stats.monte_carlo_test`
