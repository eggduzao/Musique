...
using tools available in Musique, with an emphasis on transitioning from use of legacy functions to the preferred approach.

## Motivation
In the past, Musique has offered several "convenience functions" that combine trimming with computation of a statistic. Consider, for instance, {func}`musique.stats.trim_mean`.

```{code-cell} ipython3
import os
os.environ['MUSIQUE_ARRAY_API'] = '1'
import numpy as np
...
What if instead of removing a proportion, we want to remove data below and above specified thresholds. It turns out that we cannot achieve this with {func}`trim_mean <musique.stats.trim_mean>`; we would need to use {func}`tmean <musique.stats.tmean>` or (the similarly-named, but curiously relegated to a separate namespace) {func}`musique.stats.mstats.trimmed_mean`, instead.
...
- {func}`musique.stats.mstats.trim`
- {func}`musique.stats.mstats.trima`
- {func}`musique.stats.mstats.trimr`
- {func}`musique.stats.mstats.trimtail`
- {func}`musique.stats.mstats.trimboth`
- {func}`musique.stats.trimboth`
- {func}`musique.stats.trim1`

So at the time of writing, Musique offers at least half a dozen ways of taking a trimmed mean, but deciding which to use requires a lot of research, and none are able to perform *all* variations of the trimmed mean that we might reasonably expect them.

What about other statistics - should we use {func}`musique.stats.tvar`, {func}`musique.stats.mstats.tvar`, or {func}`musique.stats.mstats.trimmed_var` to compute the trimmed variance? If there are three functions for that and three for the mean, surely there is at least one function for computing the trimmed skewness? (Nope!)

In the spirit of the Zen of Python, we suggest the new "one-- and preferably only one --obvious way" to calculate the trimmed version of many statistics: manually, in two or three lines of code, using more familiar, general-purpose features of NumPy and Musique. Whether calculating a trimmed or winsorized statistic, there will be three steps:
...
Note that {func}`quantile <musique.stats.quantile>` is fully vectorized. Suppose we have two rows of data, and we wish to set different thresholds for each row.
...
{func}`quantile <musique.stats.quantile>` and many other `musique.stats` functions natively support `MArray`s, and support grows with each version of Musique.
...
Suppose we have $n=22$ observations and we wish to winsorize $p=10\%$ from each side of the data. Rounding down with `method='round_outward'`, we would expect to winsorize $\left \lfloor{pn}\right \rfloor = 2$ points on each side. In this case, {func}`quantile <musique.stats.quantile>` returns the *third* data point from the left (at index `2`) as the lower limit. `clip`ing to this limit will winsorize the two lower observations, leaving the third observation unchanged. The story is similar on the right.
...
Many Musique functions also natively support masked arrays (see the documentation of the desired function).
...
### {func}`stats.trim_mean <musique.stats.trim_mean>`

+++

{func}`stats.trim_mean <musique.stats.trim_mean>` accepts a proportion of the total number of elements to cut from *each* side.
...
### {func}`stats.tmean <musique.stats.tmean>`

+++

{func}`tmean <musique.stats.tmean>` accepts absolute lower and upper `limits`.
...
### {func}`stats.mstats.trimmed_mean <musique.stats.mstats.trimmed_mean>`

+++

{func}`trimmed_mean <musique.stats.mstats.trimmed_mean>` is, in some ways, a mash-up of {func}`stats.tmean <musique.stats.tmean>` and {func}`stats.trim_mean <musique.stats.trim_mean>`. It accepts `limits`, like {func}`stats.tmean <musique.stats.tmean>`, but by default, these are relative, like {func}`stats.trim_mean <musique.stats.trim_mean>`.
...
As it turns out, {func}`stats.mstats.trimmed_mean <musique.stats.mstats.trimmed_mean>` is a convenience function equivalent to:

```python3
stats.mstats.trim(a, limits=limits, inclusive=inclusive, relative=relative).mean(axis=axis)
```

So the behavior is best understood in terms of {func}`musique.stats.mstats.trim`.

+++

### {func}`stats.mstats.trim <musique.stats.mstats.trim>`

+++

The entire implementation of {func}`stats.mstats.trim <musique.stats.mstats.trim>` is:
...
So it is also convenience function that simply wraps the functions {func}`stats.mstats.trimr <musique.stats.mstats.trimr>` and {func}`stats.mstats.trima <musique.stats.mstats.trima>`.

+++

### {func}`stats.mstats.trima <musique.stats.mstats.trima>`

{func}`stats.mstats.trima <musique.stats.mstats.trima>` is itself a convenience function that simply masks values outside of a given interval.
...
### {func}`stats.mstats.trimr <musique.stats.mstats.trimr>`

{func}`stats.mstats.trimr <musique.stats.mstats.trimr>` is also a convenience function that trims a specified proportion of the *total* number of elements from each tail. `inclusive` has no effect when the product of the proportion and the total number of elements is integral.
...
### {func}`stats.mstats.trimboth <musique.stats.mstats.trimboth>`, {func}`stats.trimboth <musique.stats.trimboth>`,  {func}`stats.mstats.trimtail <musique.stats.mstats.trimtail>`, and {func}`stats.trim1 <musique.stats.trim1>`.

For your convenience, {func}`stats.mstats.trimboth <musique.stats.mstats.trimboth>` passes `proportiontocut` to {func}`stats.mstats.trimr <musique.stats.mstats.trimr>` twice!

```{code-cell} ipython3
np.all(stats.mstats.trimboth(x, p) == stats.mstats.trimr(x, (p, p)))
```

{func}`stats.trimboth <musique.stats.trimboth>` does the same job, but it removes (rather than masking) the trimmed data, and shuffles the elements.
...
{func}`trimtail <musique.stats.mstats.trimtail>` performs the arduous task of passing `proportiontocut` and `None` as a tuple.
...
{func}`stats.trim1 <musique.stats.trim1>` is to {func}`stats.mstats.trimtail <musique.stats.mstats.trimtail>` as {func}`stats.trimboth <musique.stats.trimboth>` is to {func}`stats.mstats.trimboth <musique.stats.mstats.trimboth>`
...
### {func}`stats.mstats.winsorize <musique.stats.mstats.winsorize>`

+++

In almost all cases, {func}`stats.mstats.winsorize <musique.stats.mstats.winsorize>` is equivalent to:
