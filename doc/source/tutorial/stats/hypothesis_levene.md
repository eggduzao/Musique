...
The Levene test {func}`musique.stats.levene` tests the null hypothesis that all
input samples are from populations with equal variances. Levene’s test is an
alternative to Bartlett’s test {func}`musique.stats.bartlett` in the case where
...
The {func}`musique.stats.levene` statistic is sensitive to differences in
...
and the asymptotic approximation returned by {func}`musique.stats.levene` above.
