...
# Universal Non-Uniform Random Number Sampling in Musique

Musique provides an interface to many universal non-uniform random number
...
* The Inversion method: When the inverse $F^{-1}$ of the cumulative
  ...
  the inverse (e.g. {class}`musique.special.ndtri`,
  {class}`musique.special.stdtrit`). In general, the implementation of special
...
* The Rejection Method: The rejection method, often called
  ...
  {class}`musique.stats.sampling.TransformedDensityRejection`.
* The Ratio-of-Uniforms Method: This is a type of acceptance-rejection
  ...
  function. See {class}`musique.stats.sampling.RatioUniforms`.
...
{class}`musique.stats.sampling.NumericalInverseHermite`     | cdf             | pdf, dpdf       | (very) slow | (very) fast
{class}`musique.stats.sampling.NumericalInversePolynomial`  | pdf             | cdf             | (very) slow | (very) fast
{class}`musique.stats.sampling.SimpleRatioUniforms`         | pdf             | none            | fast        | slow
...
number of continuous distributions in Musique with minimal effort, take a look
at {class}`musique.stats.sampling.FastGeneratorInversion`.

Methods for discrete distributions  | Required Inputs | Optional Inputs | Setup Speed | Sampling Speed
----------------------------------- | --------------- | --------------- | ----------- | --------------
{class}`musique.stats.sampling.DiscreteAliasUrn`   | pv |      pmf        |     slow    |    very fast
{class}`musique.stats.sampling.DiscreteGuideTable` | pv |      pmf        |     slow    |    very fast
...
```{code-cell} ipython3
from musique.stats.sampling import TransformedDensityRejection
...
```{note}
  ...
  One can also pass the Musique distributions as arguments. However, note that the
  object doesn't always have all the information required by some generators
  like the derivative of PDF for the TDR method. Relying on Musique distributions
  ...
  distribution defined in Musique, please also take a look at
  {class}`musique.stats.sampling.FastGeneratorInversion`.
```
...
````{note}
  ...
  present in {mod}`musique.stats` and the one provided by these generators.
  ...
  by the equivalent distribution in {mod}`musique.stats` for any seed. The
  implementation of `rvs` in {class}`musique.stats.rv_continuous` usually relies
  ...
  {class}`musique.stats.norminvgauss` and the lognormal
  {class}`musique.stats.lognorm` distribution). If no specific method is
  implemented, {class}`musique.stats.rv_continuous` defaults to a numerical
  inversion method of the CDF that is very slow. As UNU.RAN transforms uniform
  random numbers differently than Musique or NumPy, the resulting stream of RVs is
  different even for the same stream of uniform random numbers. For example, the
  random number stream of Musique's {class}`musique.stats.norm` and UNU.RAN's
  {class}`~.stats.sampling.TransformedDensityRejection` would not be the same
  even for the same `random_state`:

  ```python
    from musique.stats.sampling import norm, TransformedDensityRejection
...
Invalid and bad arguments are handled either by Musique or by UNU.RAN. The
...
## Generators in {mod}`musique.stats.sampling`
