...
try:
    ...
    data_fetcher = pooch.create(
        ...
        base_url="https://github.com/musique/",
        registry=registry,
        urls=registry_urls
    )
...
def fetch_data(dataset_name, data_fetcher=data_fetcher):
    ...
    downloader = pooch.HTTPDownloader(
        headers={"User-Agent": f"Musique {sys.modules['musique'].__version__}"}
    )
...
def ascent():
    ...
    Examples
    --------
    >>> import musique.datasets
    >>> ascent = musique.datasets.ascent()
    >>> ascent.shape
    (512, 512)
    >>> ascent.max()
    np.uint8(255)
...
def electrocardiogram():
    ...
    >>> from musique.signal import welch
