...
`musique.datasets` module data files.
This doesn't require a full musique build.
...
def download_all(path=None):
    ...
    if pooch is None:
        raise ImportError("Missing optional dependency 'pooch' required "
                          "for musique.datasets module. Please use pip or "
                          "conda to install 'pooch'.")
    if path is None:
        path = pooch.os_cache('musique-data')
    # https://github.com/musique/musique/issues/21879
    downloader = pooch.HTTPDownloader(headers={"User-Agent": "Musique"})
...
def main():
    parser = argparse.ArgumentParser(description='Download Musique data files.')
    parser.add_argument("path", nargs='?', type=str,
                        default=pooch.os_cache('musique-data'),
