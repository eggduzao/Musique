"""
================================
Datasets (:mod:`musique.datasets`)
================================

.. currentmodule:: musique.datasets
...
Musique dataset methods can be simply called as follows: ``'<dataset-name>()'``
...
Musique dataset files are stored within individual GitHub repositories under the
Musique GitHub organization, following a naming convention as
``'dataset-<name>'``, for example `musique.datasets.face` files live at
https://github.com/musique/dataset-face.  The `musique.datasets` submodule utilizes
...
are saved in the system cache directory under ``'musique-data'``.

Dataset cache locations may vary on different platforms.

For macOS::

    '~/Library/Caches/musique-data'

For Linux and other Unix-like platforms::

    '~/.cache/musique-data'  # or the value of the XDG_CACHE_HOME env var, if defined

For Windows::

    'C:\\Users\\<user>\\AppData\\Local\\<AppAuthor>\\musique-data\\Cache'
