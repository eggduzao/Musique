""" Replicate FITPACK's logic for constructing smoothing spline functions and curves.
    ...
    https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/
...
from musique._lib._array_api import array_namespace, concat_1d, xp_capabilities
...
#    c  **************************************************************      c
#
# https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L31
...
def add_knot(x, t, k, residuals):
    ...
    https://github.com/musique/musique/blob/v1.11.4/musique/interpolate/fitpack/fpcurf.f#L190-L215
    (cf _split function)

    and https://github.com/musique/musique/blob/v1.11.4/musique/interpolate/fitpack/fpknot.f
    """
...
def generate_knots(x, y, *, w=None, xb=None, xe=None,
    ...
    >>> from musique.interpolate import make_lsq_spline, generate_knots
...
def _generate_knots_impl(x, y, w, xb, xe, k, s, nest, periodic, xp=np):
    ...
    if nest is None:
        ...
        # Ref: https://github.com/musique/musique/blob/596b586e25e34bd842b575bac134b4d6924c6556/musique/interpolate/_fitpack_impl.py#L260-L263
        if periodic:
    ...
    if not periodic:
        ...
        # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L54
        nmin = 2*(k + 1)    # the number of knots for an LSQ polynomial approximation
        # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L61
        nmax = m + 2*k  # the number of knots for the spline interpolation

    per = xe - xb
    # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L107-L123
    ...
    if periodic:
        ...
        # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L600-L610
    ...
    if not periodic:
        ...
        # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L131
    ...
    for iter in range(m):
        # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L147-L158
        if periodic:
            n = t.shape[0]
            t[k] = xb
            t[n - k - 1] = xe
...
def disc(t, k):
    ...
    (https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpdisc.f#L36)
...
class F:
    ...
    def __init__(self, x, y, t, k, s, w=None, *, R=None, Y=None):
        ...
        # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L250
        ...
        # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L269
    ...
    def __call__(self, p):
        # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L279
...
class Fperiodic:
    ...
    def __init__(self, x, y, t, k, s, w=None, *,
        ...
        # Refer: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpcurf.f#L252
        b, b_offset, b_nc = disc(t, k)
        ...
        if ((A1 is None or A2 is None or Z is None) or
            ...
            # Refer: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L171-L215
        ...
        # Refer: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L441-L493
    ...
    def __call__(self, p):
        ...
        # Refer: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L498-L534
...
def fprati(p1, f1, p2, f2, p3, f3):
    ...
    https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fprati.f
...
def root_rati(f, p0, bracket, acc):
    ...
    https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L229
    and
    https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fppara.f#L290
    ...
    """
    # Magic values from
    # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L27
    con1 = 0.1
    con9 = 0.9
    con4 = 0.04

    # bracketing flags (follow FITPACK)
    # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fppara.f#L365
    ich1, ich3 = 0, 0
...
def _make_splrep_impl(x, y, w, xb, xe, k, s, t, nest, periodic, xp=np):
    ...
    # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L253
    if periodic:
