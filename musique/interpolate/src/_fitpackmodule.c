...
/**
  * There are three wrappers for surfit below, for different callsites in Musique as legacy code.
  ...
  * The remaining is a a C wrapper for use in the legacy function "musique.interpolate.bisplrep".
...
fitpack_sphere(PyObject* Py_UNUSED(dummy), PyObject *args)
{
    ...
    // Currently Musique FITPACK invocations are inconsistent due to legacy reasons
...
static int fitpack_module_exec(PyObject *module) {
    ...
    fitpack_error = PyErr_NewException("musique.interpolate._fitpack.error", NULL, NULL);
