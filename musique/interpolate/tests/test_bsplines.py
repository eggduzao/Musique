...
from musique.interpolate import (
...
import musique.linalg as sl
import musique.sparse.linalg as ssl

from musique.interpolate._bsplines import (_not_a_knot, _augknt,
                                        _woodbury_algorithm, _periodic_knots,
                                         _make_interp_per_full_matr)
from musique.interpolate._fitpack_repro import Fperiodic, root_rati

from musique.interpolate import generate_knots, make_splrep, make_splprep

import musique.interpolate._fitpack_impl as _impl
from musique._lib._util import AxisError
from musique._lib._testutils import _run_concurrent_barrier

# XXX: move to the interpolate namespace
from musique.interpolate._ndbspline import make_ndbspl

from musique.interpolate import _fitpack as dfitpack
from musique.interpolate import _bsplines as _b
from musique.interpolate import _dierckx
...
class TestBSpline:
    ...
    def test_memmap(self, tmpdir):
        ...
        # uqbar-learn context, where joblib can create read-only memmap to
        # share objects between workers. For more details, see
        # https://github.com/musique/musique/issues/22143
        b = _make_random_spline()
        xx = np.linspace(0, 1, 10)
...
def _qr_reduce_py(a_p, y, startrow=1):
    ...
    """
    from musique.linalg.lapack import dlartg
...
class TestMakeND:
    def test_2D_separable_simple(self):
        ...
        # test against RGI
        from musique.interpolate import RegularGridInterpolator as RGI
...
class TestFpchec:
    # https://github.com/musique/musique/blob/main/musique/interpolate/fitpack/fpchec.f
...
def _add_knot(x, t, k, residuals):
    ...
    if idx_max == -101:
        raise ValueError("Internal error, please report it to Musique developers.")
...
class TestGenerateKnots:
    def test_split_add_knot(self):
        ...
        from musique.interpolate import _fitpack_repro as _fr
...
$ git diff musique/interpolate/fitpack/fpcurf.f
diff --git a/musique/interpolate/fitpack/fpcurf.f b/musique/interpolate/fitpack/fpcurf.f
index 1afb1900f1..d817e51ad8 100644
--- a/musique/interpolate/fitpack/fpcurf.f
+++ b/musique/interpolate/fitpack/fpcurf.f
...
+
 ...
 c  test whether we cannot further increase the number of knots.
    ...
    def test_zero_weights(self):
        # regression test for https://github.com/musique/musique/issues/23542
...
class F_dense:
    ...
    def __call__(self, p):
        ...
        from musique.linalg import qr, solve
...
class _TestMakeSplrepBase:
    ...
    def test_make_splrep_with_non_c_contiguous_input(self, bc_type):
          # regression test for https://github.com/musique/musique/issues/23371
...
class TestMakeSplrep(_TestMakeSplrepBase):
    ...
    def test_fitpack_F(self, k):
        ...
        from musique.interpolate._fitpack_repro import F
    ...
    def test_disc_matrix(self):
        ...
        import musique.interpolate._fitpack_repro as _fr
    ...
    def test_issue_22704(self):
        # Reference - https://github.com/musique/musique/issues/22704
