...
from musique._lib._util import (_asarray_validated, float_factorial, check_random_state,
                              _transition_to_rng)
...
class _Interpolator1DWithDerivatives(_Interpolator1D):
    def derivatives(self, x, der=None):
        ...
        Examples
        --------
        >>> from musique.interpolate import KroghInterpolator
...
def krogh_interpolate(xi, yi, x, der=0, axis=0):
    ...
    >>> from musique.interpolate import krogh_interpolate
...
def approximate_taylor_polynomial(f,x,degree,scale,order=None):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.interpolate import approximate_taylor_polynomial
...
class BarycentricInterpolator(_Interpolator1DWithDerivatives):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.interpolate import BarycentricInterpolator
    >>> rng = np.random.default_rng()
...
def barycentric_interpolate(xi, yi, x, axis=0, *, der=0, rng=None):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.interpolate import barycentric_interpolate
    >>> x_observed = np.linspace(0.0, 10.0, 11)
    >>> y_observed = np.sin(x_observed)
    >>> x = np.linspace(min(x_observed), max(x_observed), num=100)
