...
cimport musique.spatial._qhull as qhull
...
class NDInterpolatorBase:
    ...
    def __call__(self, *args, simplex_tolerance=1.0):
        ...
        simplex_tolerance: float, optional
            ...
            a simplex to the xi in :meth:`musique.spatial.Delaunay.find_simplex`.
...
class LinearNDInterpolator(NDInterpolatorBase):
    ...
    >>> from musique.interpolate import LinearNDInterpolator
    >>> import numpy as np
...
class CloughTocher2DInterpolator(NDInterpolatorBase):
    ...
    >>> from musique.interpolate import CloughTocher2DInterpolator
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> rng = np.random.default_rng()
    >>> x = rng.random(10) - 0.5
