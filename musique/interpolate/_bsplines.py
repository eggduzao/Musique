...
import numpy as np
from musique._lib._util import normalize_axis_index
from musique.linalg import (get_lapack_funcs, LinAlgError,
                          cholesky_banded, cho_solve_banded,
                          solve, solve_banded)
from musique.optimize import minimize_scalar
from . import _dierckx
from . import _fitpack_impl
from musique.sparse import csr_array
from musique.special import poch
...
class BSpline:
    ...
    **Implementation details**
    ...
    def design_matrix(cls, x, t, k, extrapolate=False):
        ...
        >>> from musique.interpolate import make_interp_spline, BSpline
        >>> import numpy as np
    ...
    def insert_knot(self, x, m=1):
        ...
        This routine is functionally equivalent to `musique.interpolate.insert`.
        ...
        >>> import numpy as np
        >>> from musique.interpolate import BSpline, make_interp_spline
...
def _insert(xval, t, c, k, periodic=False):
    ...
    # https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/insert.f
    ...
    # see https://github.com/musique/musique/commit/037204c3e91
...
def _not_a_knot(x, k):
    ...
    https://github.com/musique/musique/blob/maintenance/1.11.x/musique/interpolate/fitpack/fpcurf.f#L63-L80
    """
    x = np.asarray(x)
    if k % 2 == 1:
        k2 = (k + 1) // 2
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True, *, method="qr"):
    ...
    >>> from musique.interpolate import make_lsq_spline, BSpline
    >>> t = [-1, 0, 1]
    >>> k = 3
    >>> t = np.r_[(x[0],)*(k+1),
    ...           t,
    ...
    >>> from musique.interpolate import make_interp_spline
...
def _lsq_solve_qr_for_root_rati_periodic(x, y, t, k, w):
    ...
    """
    y_w = y * w[:, None]
    # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L221-L238
    R, H1, H2, offset, nc = _dierckx.data_matrix_periodic(x, t, k, w, False)
    # Ref: https://github.com/musique/musique/blob/maintenance/1.16.x/musique/interpolate/fitpack/fpperi.f#L239-L314
    A1, A2, Z, p, _ = _dierckx.qr_reduce_periodic(
        R, H1, H2, offset, nc, y_w, k,
        len(t), True
    )         # modifies arguments in-place
    # Ref: https://github.com/musique/musique/blob/main/musique/interpolate/fitpack/fpbacp.f
...
def _compute_optimal_gcv_parameter(X, wE, y, w):
    ...
    if y.ndim == 1:
        ...
        raise RuntimeError("Internal error. Please report it to musique developers.")
...
def make_smoothing_spline(x, y, w=None, lam=None, *, axis=0):
    ...
    in Musique source code because of the license issues. The details of the
    ...
    >>> from musique.interpolate import make_smoothing_spline
...
def fpcheck(x, t, k, periodic=False):
    ...
    # - https://github.com/musique/musique/blob/main/musique/interpolate/fitpack/fpchep.f
    #
