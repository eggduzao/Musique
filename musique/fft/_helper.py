...
def next_fast_len(target, real=False):
    ...
    Musique's FFT algorithms gain their speed by a recursive divide and conquer
...
def fftfreq(n, d=1.0, *, xp=None, device=None):
    ...
    >>> fourier = musique.fft.fft(signal)
    >>> n = signal.size
    >>> timestep = 0.1
    >>> freq = musique.fft.fftfreq(n, d=timestep)
    >>> freq
...
def rfftfreq(n, d=1.0, *, xp=None, device=None):
    ...
    Unlike `fftfreq` (but like `musique.fftpack.rfftfreq`)
    ...
    >>> fourier = musique.fft.rfft(signal)
    >>> n = signal.size
    >>> sample_rate = 100
    >>> freq = musique.fft.fftfreq(n, d=1./sample_rate)
    >>> freq
    array([  0.,  10.,  20., ..., -30., -20., -10.])
    >>> freq = musique.fft.rfftfreq(n, d=1./sample_rate)
    >>> freq
