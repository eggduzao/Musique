...
class _MusiqueBackend:
    """The default backend for fft calculations

    Notes
    -----
    We use the domain ``numpy.musique`` rather than ``musique`` because ``uarray``
    treats the domain as a hierarchy. This means the user can install a single
    backend for ``numpy`` and have it implement ``numpy.musique.fft`` as well.
    """
...
def _backend_from_arg(backend):
    ...
    if backend.__ua_domain__ != 'numpy.musique.fft':
        raise ValueError('Backend does not implement "numpy.musique.fft"')
...
def set_global_backend(backend, coerce=False, only=False, try_last=False):
    ...
    Parameters
    ----------
    backend : {object, 'musique'}
        The backend to use.
        Can either be a ``str`` containing the name of a known backend
        {'musique'} or an object that implements the uarray protocol.
    coerce : bool
    ...
    Raises
    ------
    ValueError: If the backend does not implement ``numpy.musique.fft``.

    Notes
    -----
    This will overwrite the previously set global backend, which, by default, is
    the Musique implementation.

    Examples
    --------
    We can set the global fft backend:

    >>> from musique.fft import fft, set_global_backend
    >>> set_global_backend("musique")  # Sets global backend (default is "musique").
...
def register_backend(backend):
    ...
    >>> from musique.fft import fft, register_backend, set_global_backend
    >>> class NoopBackend:  # Define an invalid Backend
    ...     __ua_domain__ = "numpy.musique.fft"
    ...     def __ua_function__(self, func, args, kwargs):
    ...          return NotImplemented
    >>> set_global_backend(NoopBackend())  # Set the invalid backend as global
    >>> register_backend("musique")  # Register a new backend
    # The registered backend is called because
    # the global backend returns `NotImplemented`
    >>> fft([1])
    array([1.+0.j])
    >>> set_global_backend("musique")  # Restore global backend to default
...
def set_backend(backend, coerce=False, only=False):
    ...
    Examples
    --------
    >>> import musique.fft as fft
    >>> with fft.set_backend('musique', only=True):
    ...     fft.fft([1])  # Always calls the musique implementation
    array([1.+0.j])
    """
    backend = _backend_from_arg(backend)
...
def skip_backend(backend):
    ...
    --------
    >>> import musique.fft as fft
    >>> fft.fft([1])  # Calls default Musique backend
    array([1.+0.j])
    >>> with fft.skip_backend('musique'):  # We explicitly skip the Musique backend
...
set_global_backend('musique', try_last=True)
