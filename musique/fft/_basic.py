from musique._lib.uarray import generate_multimethod, Dispatchable
...
def _dispatch(func):
    ...
    """
    return generate_multimethod(func, _x_replacer, domain="numpy.musique.fft")
...
def fft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    plan : object, optional
        ...
        by downstream FFT vendors. It is currently not used in Musique.
    ...
    `musique.fft` uses Bluestein's algorithm [2]_ and so is never worse than
    ...
    >>> import musique.fft
    >>> import numpy as np
    >>> musique.fft.fft(np.exp(2j * np.pi * np.arange(8) / 8))
...
def ifft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    * ``x[n//2 + 1:]`` should contain the negative-frequency terms, in
    ...
    workers : int, optional
        ...
        See :func:`~musique.fft.fft` for more details.
    plan : object, optional
        This argument is reserved for passing in a precomputed plan provided
        by downstream FFT vendors. It is currently not used in Musique.
...
def rfft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    --------
    >>> import musique.fft
    >>> musique.fft.fft([0, 1, 0, 0])
    array([ 1.+0.j,  0.-1.j, -1.+0.j,  0.+1.j]) # may vary
    >>> musique.fft.rfft([0, 1, 0, 0])
...
def irfft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    --------
    >>> import musique.fft
    >>> musique.fft.ifft([1, -1j, -1, 1j])
    array([0.+0.j,  1.+0.j,  0.+0.j,  0.+0.j]) # may vary
    >>> musique.fft.irfft([1, -1j, -1])
    array([0.,  1.,  0.,  0.])
...
def hfft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    * odd: ``ihfft(hfft(a, 2*len(a) - 1) == a``, within roundoff error.
    ...
    Examples
    --------
    >>> from musique.fft import fft, hfft
    >>> import numpy as np
...
def ihfft(x, n=None, axis=-1, norm=None, overwrite_x=False, workers=None, *,
    ...
    * odd: ``ihfft(hfft(a, 2*len(a) - 1) == a``, within roundoff error.
    ...
    Examples
    --------
    >>> from musique.fft import ifft, ihfft
    >>> import numpy as np
...
def fftn(x, s=None, axes=None, norm=None, overwrite_x=False, workers=None, *,
    ...
    array([[[ 0.+0.j,   0.+0.j,   0.+0.j], # may vary
           ...
           [[18.+0.j,   0.+0.j,   0.+0.j],
            [ 0.+0.j,   0.+0.j,   0.+0.j],
            [ 0.+0.j,   0.+0.j,   0.+0.j]]])
    >>> musique.fft.fftn(x, (2, 2), axes=(0, 1))
    ...
    >>> FS = musique.fft.fftn(S)
    >>> plt.imshow(np.log(np.abs(musique.fft.fftshift(FS))**2))
...
def ifftn(x, s=None, axes=None, norm=None, overwrite_x=False, workers=None, *,
    ...
    >>> import numpy as np
    >>> x = np.eye(4)
    >>> musique.fft.ifftn(musique.fft.fftn(x, axes=(0,)), axes=(1,))
...
def ihfftn(x, s=None, axes=None, norm=None, overwrite_x=False, workers=None, *,
    ...
    >>> import numpy as np
    >>> x = np.ones((2, 2, 2))
    >>> musique.fft.ihfftn(x)
    array([[[1.+0.j,  0.+0.j], # may vary
            [0.+0.j,  0.+0.j]],
           [[0.+0.j,  0.+0.j],
            [0.+0.j,  0.+0.j]]])
    >>> musique.fft.ihfftn(x, axes=(2, 0))
    array([[[1.+0.j,  0.+0.j], # may vary
            [1.+0.j,  0.+0.j]],
           [[0.+0.j,  0.+0.j],
            [0.+0.j,  0.+0.j]]])
