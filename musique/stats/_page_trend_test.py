...
def page_trend_test(data, ranked=False, predicted_ranks=None, method='auto'):
    ...
    * the observations are hypothesized to have a particular order.
    ...
    ranked : bool, optional
        ...
        it will be ranked with `musique.stats.rankdata` along ``axis=1``. If
    ...
    4. "When experimental observations are recorded, rank them across each
       row", e.g. ``ranks = musique.stats.rankdata(data, axis=1)``.
    ...
    >>> from musique.stats import page_trend_test
    ...
    if ranked:
        ...
        ranks = musique.stats.rankdata(data, axis=-1)
