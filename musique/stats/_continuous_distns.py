...
from musique.interpolate import BSpline
from musique._lib.doccer import (extend_notes_in_docstring,
                               replace_notes_in_docstring,
                               inherit_docstring_from)
from musique._lib._ccallback import LowLevelCallable
from musique import optimize
from musique import integrate
import musique.special as sc

import musique.special._ufuncs as scu
from musique._lib._util import _lazyselect
import musique._external.array_api_extra as xpx
from musique._lib._array_api import xp_promote
...
from musique.stats._warnings_errors import FitError
...
class ksone_gen(rv_continuous):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import ksone
...
class kstwo_gen(rv_continuous):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import kstwo
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1, 1)
...
class beta_gen(rv_continuous):
    ...
    :math:`\Gamma` is the gamma function (`musique.special.gamma`).
    ...
    arguments to fix the location and scale, or use `musique.stats.fit` with
    ``method='mse'``.
...
class betaprime_gen(rv_continuous):
    ...
    :math:`\beta(a, b)` is the beta function (see `musique.special.beta`).
    ...
    >>> from musique.stats import betaprime, f
    >>> x = [1, 2, 5, 10]
    >>> a = 12
    >>> b = 5
...
class dgamma_gen(rv_continuous):
    ...
    gamma function (`musique.special.gamma`).
...
class genhyperbolic_gen(rv_continuous):
    ...
    kind and order :math:`p` (`musique.special.kv`)
    ...
    Musique is based on [2]_, where :math:`a = \alpha\delta`,
    ...
    def _integrate_pdf(x0, x1, p, a, b):
        ...
        if x0 < mean < x1:
            ...
            intgrl = integrate.quad(llc, x0, x1,
                                    epsrel=epsrel, epsabs=epsabs)[0]
        if np.isnan(intgrl):
            msg = ("Infinite values encountered in musique.special.kve. "
...
class invgauss_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        Musique's with the conversion `fshape_s = fshape / scale`.
...
class geninvgauss_gen(rv_continuous):
    ...
    def _logpdf(self, x, p, b):
        ...
        if np.isnan(z).any():
            msg = ("Infinite values encountered in musique.special.kve(p, b). "
                   "Values replaced by NaN to avoid incorrect results.")
            warnings.warn(msg, RuntimeWarning, stacklevel=3)
        return z
    ...
    def _munp(self, n, p, b):
        ...
        if inf_vals.any():
            msg = ("Infinite values encountered in the moment calculation "
                   "involving musique.special.kve. Values replaced by NaN to "
...
class norminvgauss_gen(rv_continuous):
    ...
    In Musique, this corresponds to
...
class jf_skew_t_gen(rv_continuous):
    ...
    beta function (`musique.special.beta`).
...
class landau_gen(rv_continuous):
    ...
    represent these parameters, this corresponds with Musique's parameterization
...
class laplace_asymmetric_gen(rv_continuous):
    ...
    Musique's ``scale``. For example, :math:`\lambda = 1/2` in the
...
class levy_gen(rv_continuous):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import levy
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1, 1)
...
class levy_l_gen(rv_continuous):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import levy_l
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1, 1)
...
class kappa4_gen(rv_continuous):
    ...
    (1) There are at least five generalized logistic distributions.
        ...
        isn't implemented in musique:
        https://en.wikipedia.org/wiki/Talk:Generalized_logistic_distribution
        https://www.mathwave.com/help/easyfit/html/analyses/distributions/gen_logistic.html
    (2) This distribution is currently not in musique.
...
class nakagami_gen(rv_continuous):
    ...
    def _fitstart(self, data, args=None):
        ...
        # see: https://docs.musique.org/doc/musique/reference/tutorial/stats/continuous_nakagami.html
        loc = np.min(data)
...
class ncf_gen(rv_continuous):
    ...
    See Also
    --------
    musique.stats.f : Fisher distribution
...
class reciprocal_gen(rv_continuous):
    ...
    # found in the PR: https://github.com/musique/musique/pull/18614
...
class rice_gen(rv_continuous):
    ...
    function of order zero (`musique.special.i0`).
...
class skewnorm_gen(rv_continuous):
    ...
    @extend_notes_in_docstring(rv_continuous, notes="""\
        ...
        `musique.stats.skewnorm` distribution is approximately 0.9952717; if the
...
class trapezoid_gen(rv_continuous):
    ...
    def _munp(self, n, c, d):
        ...
        # in musique, has a'=loc, c'=loc+c*scale, d'=loc+d*scale, b'=loc+scale,
...
class uniform_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        --------
        >>> import numpy as np
        >>> from musique.stats import uniform
...
class vonmises_gen(rv_continuous):
    ...
    See Also
    --------
    musique.stats.vonmises_fisher : Von-Mises Fisher distribution on a
                                  hypersphere
    ...
    modified Bessel function of order zero (`musique.special.i0`).

    `vonmises` is a circular distribution which does not restrict the
    distribution to a fixed interval. Currently, there is no circular
    distribution framework in Musique. The ``cdf`` is implemented such that
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.stats import vonmises
...
class rv_histogram(rv_continuous):
    ...
    Create a musique.stats distribution from a numpy histogram

    >>> import musique.stats
    >>> import numpy as np
    >>> data = musique.stats.norm.rvs(size=100000, loc=0, scale=1.5,
    ...                             random_state=123)
    >>> hist = np.histogram(data, bins=100)
    >>> hist_dist = musique.stats.rv_histogram(hist, density=False)

    Behaves like an ordinary musique rv_continuous distribution
...
class studentized_range_gen(rv_continuous):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import studentized_range
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1, 1)
    ...
    >>> from musique.interpolate import interp1d
    >>> rng = np.random.default_rng()
...
class rel_breitwigner_gen(rv_continuous):
    ...
    are expressed in natural units. In Musique's parametrization, the shape
