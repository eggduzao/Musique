...
import numpy as np
from musique import special, interpolate, stats
from musique._lib._array_api import xp_capabilities
from musique.stats._censored_data import CensoredData
from musique.stats._common import ConfidenceInterval
...
class EmpiricalDistributionFunction:
    """An empirical distribution function produced by `musique.stats.ecdf`
    ...
    def confidence_interval(self, confidence_level=0.95, *, method='linear'):
        ...
        ci : ``ConfidenceInterval``
            ...
            `~musique.stats._result_classes.EmpiricalDistributionFunction` that
...
@dataclass
class ECDFResult:
    """ Result object returned by `musique.stats.ecdf`

    Attributes
    ----------
    cdf : `~musique.stats._result_classes.EmpiricalDistributionFunction`
        An object representing the empirical cumulative distribution function.
    sf : `~musique.stats._result_classes.EmpiricalDistributionFunction`
...
def ecdf(sample: "npt.ArrayLike | CensoredData") -> ECDFResult:
    ...
    Parameters
    ----------
    sample : 1D array_like or `musique.stats.CensoredData`
        Besides array_like, instances of `musique.stats.CensoredData` containing
        uncensored and right-censored observations are supported. Currently,
        other instances of `musique.stats.CensoredData` will result in a
        ``NotImplementedError``.

    Returns
    -------
    res : `~musique.stats._result_classes.ECDFResult`
    ...
    an instance of `musique.stats.CensoredData`.
...
@dataclass
class LogRankResult:
    """Result object returned by `musique.stats.logrank`.
...
def logrank(
    ...
    Returns
    -------
    res : `~musique.stats._result_classes.LogRankResult`
    ...
    preserved, allowing `musique.stats.logrank` to offer one-sided alternative
    hypotheses.
    ...
    `musique.stats.CensoredData` class is used because the data is
