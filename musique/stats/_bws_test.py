...
from musique.stats._axis_nan_policy import _broadcast_arrays
from musique._external import array_api_extra as xpx
from musique._lib._array_api import (xp_capabilities, array_namespace, xp_ravel,
                                   xp_result_type)
...
def _bws_input_validation(x, y, alternative, axis, method):
    ...
    if not isinstance(method, stats.PermutationMethod):
        raise ValueError('`method` must be an instance of '
                         '`musique.stats.PermutationMethod`')
...
def bws_test(x, y, *, alternative="two-sided", axis=0, method=None):
    ...
    See Also
    --------
    musique.stats.wilcoxon, musique.stats.mannwhitneyu, musique.stats.ttest_ind
    ...
    >>> from musique.stats import bws_test
    >>> res = bws_test(x, y)
    >>> print(res.statistic)
    5.132167152575315
