...
from musique import special as sc
...
class FastGeneratorInversion:
    ...
    continuous distributions in `musique.stats`.

    Parameters
    ----------
    dist : rv_frozen object
        Frozen distribution object from `musique.stats`. The list of supported
    ...
    `musique.stats.sampling` that is created when the object is instantiated.
    In addition, the methods `qrvs` and `ppf` are added.
    `qrvs` generate samples based on quasi-random numbers from
    `musique.stats.qmc`. `ppf` is the PPF based on the
    ...
    Note that all implemented distributions are also part of `musique.stats`, and
    ...
    - to use a QMC generator from `musique.stats.qmc` (method `qrvs`),
    ...
    >>> import numpy as np
    >>> from musique import stats
    >>> from musique.stats.sampling import FastGeneratorInversion
    ...
    In this particular, case, `musique.stats.truncnorm` could also be used to
...
class RatioUniforms:
    ...
    test such as `~musique.stats.kstest` as a check.
    ...
    >>> from musique.stats.sampling import RatioUniforms
