...
from musique.optimize import shgo
from . import distributions
from ._common import ConfidenceInterval
from ._continuous_distns import norm
from musique._lib._array_api import (xp_capabilities, array_namespace, xp_size,
                                   xp_promote, xp_result_type, xp_copy, is_numpy,
                                   is_lazy_array)
import musique._external.array_api_extra as xpx
from musique.special import gamma, kv, gammaln
from musique.fft import ifft
...
from musique.stats import _stats_py
...
def cramervonmises(rvs, cdf, args=(), *, axis=0):
    ...
    cdf : str or callable
        ...
        distribution in `musique.stats`. If a callable, that callable is used
    ...
    Suppose we wish to test whether data generated by ``musique.stats.norm.rvs``
...
def somersd(x, y=None, alternative='two-sided'):
    ...
    *p*-value returned by `kendalltau` with the statistic returned by
    ...
    >>> from musique.stats import somersd
    ...
    if x.ndim == 1:
        ...
        table = musique.stats.contingency.crosstab(x, y)[1]
    elif x.ndim == 2:
        if np.any(x < 0):
...
def barnard_exact(table, alternative="two-sided", pooled=True, n=32):
    ...
    n : int, optional
        ...
        the next higher power of 2 since `musique.stats.qmc.Sobol` is used to
...
def _get_binomial_log_p_value_with_nuisance_param(
    ...
    more tolerant for large value than `musique.special.comb`. `gammaln` gives
...
def cramervonmises_2samp(x, y, method='auto', *, axis=0):
    ...
    ``musique.stats.norm.rvs`` have the same distribution. We choose a
    significance level of alpha=0.05.
    ...
    r = musique.stats.rankdata(z, method='average', axis=-1)
...
class TukeyHSDResult:
    """Result of `musique.stats.tukey_hsd`.
    ...
    def confidence_interval(self, confidence_level=.95):
        ...
        Examples
        --------
        >>> from musique.stats import tukey_hsd
...
def tukey_hsd(*args, equal_var=True):
    ...
    Returns
    -------
    result : `~musique.stats._result_classes.TukeyHSDResult` instance
