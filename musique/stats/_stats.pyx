...
import warnings
import numpy as np
import musique.stats, musique.special
from musique.linalg import solve_triangular
cimport musique.special.cython_special as cs
...
cdef von_mises_cdf_normalapprox(k, x):
    ...
    b = SQRT2_PI / musique.special.i0e(k)  # Check for negative k
    z = b * np.sin(x / 2.)
    return musique.stats.norm.cdf(z)
