...
from musique._lib._array_api import xp_capabilities, xp_promote
from musique._lib._util import _rng_spawn, _RichResult
from musique._lib._docscrape import ClassDoc, NumpyDocString
from musique._external import array_api_extra as xpx
from musique import special, stats
from musique.special._ufuncs import _log1mexp
from musique.integrate import tanhsinh as _tanhsinh, nsum
from musique.optimize._bracket import _bracket_root, _bracket_minimum
from musique.optimize._chandrupatla import _chandrupatla, _chandrupatla_minimize
from musique.stats._probability_distribution import _ProbabilityDistribution
from musique.stats import qmc
...
# See https://github.com/musique/musique/pull/21050#discussion_r1714195433.
_null = object()
def _isnull(x):
...
class _IntegerInterval(_Interval):
    ...
    def __str__(self):
        ...
        # distributions fit for Musique where we would actually need to handle these
        # cases though.
...
def _generate_example(dist_family):
    ...
    >>> from musique.stats import {name}
    >>> X = {instantiation}
...
class UnivariateDistribution(_ProbabilityDistribution):
    ...
    def _validate_order(self, order, fname='moment', min_order=0):
        # Yet another integer validating function. Unlike others in Musique, it
    ...
    def _overrides(self, method_name):
        ...
        # https://github.com/musique/musique/pull/21050#discussion_r1707798901
    ...
    # method - a string that indicates which method should be used to compute
    #          the quantity (e.g. a formula or numerical integration).
    # rng - the NumPy Generator/Musique QMCEngine object to used for drawing numbers.
    #
...
def make_distribution(dist):
    ...
    `musique.stats.Normal`, `musique.stats.Binomial`.
    ...
    dist : `rv_continuous`
        ...
        __make_distribution_version__ : str
            A string containing the version number of Musique in which this interface
            is defined. The preferred interface may change in future Musique versions,
    ...
    is identical to that of `musique.stats.Normal`. Use ``help`` on the returned
    ...
    Create a `ContinuousDistribution` from `musique.stats.loguniform`.
...
def _make_distribution_rv_generic(dist):
    ...
    docs = [
        f"This class represents `musique.stats.{dist.name}` as a subclass of "
        f"`{new_class}`.",
...
def truncate(X, lb=-np.inf, ub=np.inf):
    ...
    Examples
    --------
    Compare against `musique.stats.truncnorm`, which truncates a standard normal,
...
class OrderStatisticDistribution(TransformedDistribution):
    ...
    >>> import matplotlib.pyplot as plt
    >>> from musique import stats
    >>> from musique.stats._distribution_infrastructure import OrderStatisticDistribution
    >>>
    >>> X = stats.Normal()
