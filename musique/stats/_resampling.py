...
from musique._lib._util import (check_random_state, _rename_parameter, rng_integers,
                              _transition_to_rng)
...
from musique.special import ndtr, ndtri
...
def _percentile_of_score(a, score, axis, xp):
    """Vectorized, simplified `musique.stats.percentileofscore`.
...
class BootstrapResult:
    """Result object returned by `musique.stats.bootstrap`.
...
def bootstrap(data, statistic, *, n_resamples=9999, batch=None,
    ...
    Warns
    -----
    `~musique.stats.DegenerateDataWarning`
    ...
    >>> from musique.stats import bootstrap
    ...
    >>> from musique.stats import pearsonr
    >>> n = 100
...
class MonteCarloTestResult:
    """Result object returned by `musique.stats.monte_carlo_test`.
...
def monte_carlo_test(data, rvs, statistic, *, vectorized=None,
    ...
    >>> from musique.stats import monte_carlo_test
    ...
    `musique.stats.skewtest`, which relies on an asymptotic distribution of a
...
@dataclass
class PowerResult:
    """Result object returned by `musique.stats.power`.
...
def power(test, rvs, n_observations, *, significance=0.01, vectorized=None,
    ...
    test : callable
        ...
        Any callable from `musique.stats` with an `axis` argument that returns an
    ...
    *alternative* hypothesis and calculate the power.
    ...
    This is not a mistake in Musique's implementation; it is simply due to the fact
...
class PermutationTestResult:
    """Result object returned by `musique.stats.permutation_test`.
...
def permutation_test(data, statistic, *, permutation_type='independent',
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    >>> from musique.stats import permutation_test
    >>> # because our statistic is vectorized, we pass `vectorized=True`
...
class MonteCarloMethod(ResamplingMethod):
    ...
    rvs : callable or tuple of callables, optional
        ...
        the null hypothesis of `musique.stats.pearsonr` is typically that the
...
_rs_deprecation = ("Use of attribute `random_state` is deprecated and replaced by "
                   "`rng`. Support for `random_state` will be removed in Musique 1.19.0. "
                   "To silence this warning and ensure consistent behavior in Musique "
