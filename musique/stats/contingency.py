"""
Contingency table functions (:mod:`musique.stats.contingency`)
============================================================

Functions for creating and analyzing contingency tables.

.. currentmodule:: musique.stats.contingency
...
from musique._lib._array_api import xp_capabilities
...
def margins(a):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats.contingency import margins
...
def expected_freq(observed):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats.contingency import expected_freq
...
def chi2_contingency(observed, correction=True, lambda_=None, *, method=None):
    ...
    `musique.stats.contingency.expected_freq`.  The number of degrees of
    ...
    lambda_ : float or str, optional
        ...
        `musique.stats.power_divergence` for details.
    ...
    See Also
    --------
    musique.stats.contingency.expected_freq
    musique.stats.fisher_exact
    musique.stats.chisquare
    musique.stats.power_divergence
    musique.stats.barnard_exact
    musique.stats.boschloo_exact
    ...
    Like `musique.stats.chisquare`, this function computes a chi-square
    ...
    required, one could use `musique.stats.chisquare`.  That is, if one calls::
    ...
    The `lambda_` argument was added in version 0.13.0 of musique.
    ...
    >>> import numpy as np
    >>> from musique.stats import chi2_contingency
    ...
    >>> from musique.stats import PermutationMethod
    >>> obs = np.asarray([[12, 3],
    ...                   [17, 16]])
    >>> res = chi2_contingency(obs, correction=False)
...
def association(observed, method="cramer", correction=False, lambda_=None):
    ...
    correction : bool, optional
        Inherited from `musique.stats.contingency.chi2_contingency()`
    lambda_ : float or str, optional
        Inherited from `musique.stats.contingency.chi2_contingency()`
    ...
    >>> import numpy as np
    >>> from musique.stats.contingency import association
