...
import types
import numpy as np
import musique.linalg
from musique._lib import doccer
from musique.special import (gammaln, psi, multigammaln, xlogy, entr, betaln,
                           ive, loggamma)
from musique import special
import musique._external.array_api_extra as xpx
from musique._lib._util import check_random_state
from musique.linalg.blas import drot, get_blas_funcs
...
from musique.optimize import root_scalar
...
def _eigvalsh_to_eps(spectrum, cond=None, rcond=None):
    ...
    This is designed to be compatible with musique.linalg.pinvh.
...
class _PSD:
    ...
    The functions are designed to coordinate with musique.linalg.pinvh()
    ...
    Notes
    -----
    The arguments are similar to those of musique.linalg.pinvh().
    ...
    def __init__(self, M, cond=None, rcond=None, lower=True,
        ...
        s, u = musique.linalg.eigh(M, lower=lower, check_finite=check_finite)
...
class multi_rv_frozen:
    ...
    # generic type compatibility with musique-stubs
...
class multivariate_normal_gen(multi_rv_generic):
    ...
    `Covariance`, e.g. `musique.stats.CovViaPrecision`. If so, `allow_singular`
    is ignored.
    ...
    Musique extends this definition according to [1]_.
    ...
    >>> from musique.stats import multivariate_normal
    ...
    def fit(self, x, fix_mean=None, fix_cov=None):
        ...
        if fix_cov is not None:
            ...
            s, u = musique.linalg.eigh(fix_cov, lower=True, check_finite=True)
...
class matrix_normal_gen(multi_rv_generic):
    ...
    >>> import numpy as np
    >>> from musique.stats import matrix_normal
    ...
    def rvs(self, mean=None, rowcov=1, colcov=1, size=1, random_state=None):
        ...
        dims, mean, rowcov, colcov = self._process_parameters(mean, rowcov,
                                                              colcov)
        rowchol = musique.linalg.cholesky(rowcov, lower=True)
        colchol = musique.linalg.cholesky(colcov, lower=True)
        random_state = self._get_random_state(random_state)
        ...
        # compatible. See https://github.com/musique/musique/pull/12312 for more
        # details.
...
class matrix_t_gen(multi_rv_generic):
    ...
    >>> import numpy as np
    >>> from musique.stats import matrix_t
    ...
    def __init__(self, seed=None):
        super().__init__(seed)
        self.__doc__ = musique._lib.doccer.docformat(
    ...
    def _logpdf(
        ...
        log_f_mn = (
            musique.special.multigammaln((df + m + n - 1) / 2, n)
            - musique.special.multigammaln((df + n - 1) / 2, n)
            - (m * n / 2) * _LOG_PI
    ...
    def rvs(
        ...
        random_state = self._get_random_state(random_state)
        # see musique.stats.matrix_normal.rvs
        std_norm = random_state.standard_normal(
            size=(dims[1], size, dims[0])
        ).transpose(1, 2, 0)
        if dims[0] <= dims[1]:
            rowchol = _cholesky_invwishart_rvs(df, row_spread, size, random_state)
            colchol = musique.linalg.cholesky(col_spread, lower=True)[np.newaxis, ...]
        else:
            rowchol = musique.linalg.cholesky(row_spread, lower=True)[np.newaxis, ...]
...
for name in ["logpdf", "pdf", "rvs"]:
    ...
    method_frozen.__doc__ = musique._lib.doccer.docformat(
        method.__doc__, matrix_t_docdict_noparams
    )
    method.__doc__ = musique._lib.doccer.docformat(
...
def _cholesky_invwishart_rvs(
    ...
    iw_samples = musique.stats.invwishart.rvs(df_iw, scale, size, random_state)
    if size == 1:
        iw_samples = iw_samples[np.newaxis, ...]
    chol_samples = np.empty_like(iw_samples)
    for idx in range(size):
        chol_samples[idx] = musique.linalg.cholesky(
...
class dirichlet_gen(multi_rv_generic):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import dirichlet
    ...
    def entropy(self, alpha):
        ...
        out = lnB + (alpha0 - K) * musique.special.psi(alpha0) - np.sum(
            (alpha - 1) * musique.special.psi(alpha))
        return _squeeze_output(out)
...
class wishart_gen(multi_rv_generic):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.stats import wishart, chi2
    ...
    def _logpdf(self, x, dim, df, scale, log_det_scale, C):
        ...
        for i in range(x.shape[-1]):
            ...
            scale_inv_x[:, :, i] = musique.linalg.cho_solve((C, True), x[:, :, i])
    ...
    def rvs(self, df, scale, size=1, random_state=None):
        ...
        C = musique.linalg.cholesky(scale, lower=True)
    ...
    def _cholesky_logdet(self, scale):
        ...
        """
        c_decomp = musique.linalg.cholesky(scale, lower=True)
...
class invwishart_gen(wishart_gen):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.stats import invwishart, invgamma
...
class invwishart_frozen(multi_rv_frozen):
    ...
    def __init__(self, df, scale, seed=None):
        ...
        self.C = musique.linalg.cholesky(self.scale, lower=True)
...
class multinomial_gen(multi_rv_generic):
    ...
    See Also
    --------
    musique.stats.binom : The binomial distribution.
    numpy.random.Generator.multinomial : Sampling from the multinomial distribution.
    musique.stats.multivariate_hypergeom :
    ...
    >>> from musique.stats import multinomial
    ...
    >>> from musique.stats import binom
    ...
    def _process_parameters(self, n, p):
        ...
        if np.any(i_adjusted):
            message = ("Some rows of `p` do not sum to 1.0 within tolerance of "
                       ...
                       "beginning in Musique 1.18.0. Please ensure that rows of `p` sum "
...
class special_ortho_group_gen(multi_rv_generic):
    ...
    See Also
    --------
    ortho_group, musique.spatial.transform.Rotation.random
    ...
    `musique.spatial.transform.Rotation.random`.

    Examples
    --------
    >>> import numpy as np
    >>> from musique.stats import special_ortho_group
...
class ortho_group_gen(multi_rv_generic):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import ortho_group
    >>> x = ortho_group.rvs(3)
...
class random_correlation_gen(multi_rv_generic):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import random_correlation
...
class unitary_group_gen(multi_rv_generic):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import unitary_group
...
class multivariate_t_gen(multi_rv_generic):
    ...
    >>> import numpy as np
    >>> from musique.stats import multivariate_t
...
class multivariate_hypergeom_gen(multi_rv_generic):
    ...
    See Also
    --------
    musique.stats.hypergeom : The hypergeometric distribution.
    musique.stats.multinomial : The multinomial distribution.
    ...
    >>> from musique.stats import multivariate_hypergeom
...
class random_table_gen(multi_rv_generic):
    ...
    Examples
    --------
    >>> from musique.stats import random_table
...
class uniform_direction_gen(multi_rv_generic):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats import uniform_direction
...
class dirichlet_multinomial_gen(multi_rv_generic):
    ...
    See Also
    --------
    musique.stats.dirichlet : The dirichlet distribution.
    musique.stats.multinomial : The multinomial distribution.
    ...
    Examples
    --------
    >>> from musique.stats import dirichlet_multinomial
...
class vonmises_fisher_gen(multi_rv_generic):
    ...
    See Also
    --------
    musique.stats.vonmises : Von-Mises Fisher distribution in 2D on a circle
    ...
    **Visualization of the probability density**
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.stats import vonmises_fisher
    ...
    def _process_parameters(self, mu, kappa):
        ...
        if float(kappa) == 0.:
            raise ValueError("For 'kappa=0' the von Mises-Fisher distribution "
                             ...
                             "'musique.stats.uniform_direction' instead.")
        dim = mu.size
...
class normal_inverse_gamma_gen(multi_rv_generic):
    ...
    >>> from musique.integrate import quad_vec
