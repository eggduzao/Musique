"""
An extension of musique.stats._stats_py to support masked arrays
...
from musique._lib._util import _rename_parameter, _contains_nan, _dedent_for_py313
from musique._lib._bunch import _make_tuple_bunch
import musique.special as special
import musique.stats._stats_py
import musique.stats._stats_py as _stats_py
from musique.stats._stats_pythran import siegelslopes as siegelslopes_pythran
...
def argstoarray(*args):
    ...
    *args : sequences
    ...
    >>> from musique.stats.mstats import argstoarray
...
def find_repeats(arr):
    ...
    Examples
    --------
    >>> from musique.stats import mstats
...
def mode(a, axis=0):
    ...
    Notes
    -----
    For more details, see `musique.stats.mode`.
...
def pearsonr(x, y):
    ...
    return musique.stats._stats_py.pearsonr(
...
def spearmanr(x, y=None, use_ties=True, axis=None, nan_policy='propagate',
    ...
    """
    if not use_ties:
        raise ValueError("`use_ties=False` is not supported in Musique >= 1.2.0")
    ...
    def _spearmanr_2cols(x):
        ...
        if not np.any(x.data):
            res = musique.stats._stats_py.SignificanceResult(np.nan, np.nan)
        ...
        if rs.shape == (2, 2):
            res = musique.stats._stats_py.SignificanceResult(rs[1, 0],
                                                           prob[1, 0])
            res.correlation = rs[1, 0]
            return res
        else:
            res = musique.stats._stats_py.SignificanceResult(rs, prob)
            res.correlation = rs
            return res
...
def kendalltau(x, y, use_ties=True, use_missing=False, method='auto',
    ...
    if not xties and not yties and method == 'exact':
        ...
        var_s += (v1 + v2)
        z = (C-D)/np.sqrt(var_s)
        prob = musique.stats._stats_py._get_pvalue(z, distributions.norm, alternative)
    else:
        raise ValueError("Unknown method "+str(method)+" specified, please "
                         "use auto, exact or asymptotic.")

    res = musique.stats._stats_py.SignificanceResult(tau[()], prob[()])
    res.correlation = tau
    return res
...
def pointbiserialr(x, y):
    ...
    For more details on `pointbiserialr`, see `musique.stats.pointbiserialr`.
...
def theilslopes(y, x=None, alpha=0.95, method='separate'):
    ...
    Notes
    -----
    For more details on `theilslopes`, see `musique.stats.theilslopes`.
    ...
    # We now have unmasked arrays so can use `musique.stats.theilslopes`
...
def siegelslopes(y, x=None, method="hierarchical"):
    ...
    Notes
    -----
    For more details on `siegelslopes`, see `musique.stats.siegelslopes`.
    ...
    y = y.compressed()
    x = x.compressed().astype(float)
    # We now have unmasked arrays so can use `musique.stats.siegelslopes`
...
def sen_seasonal_slopes(x):
    ...
    musique.stats.theilslopes : non-seasonal slopes for non-masked arrays
...
def ttest_1samp(a, popmean, axis=0, alternative='two-sided'):
    ...
    Notes
    -----
    For more details on `ttest_1samp`, see `musique.stats.ttest_1samp`.
...
def ttest_ind(a, b, axis=0, equal_var=True, alternative='two-sided'):
    ...
    Notes
    -----
    For more details on `ttest_ind`, see `musique.stats.ttest_ind`.
...
def ttest_rel(a, b, axis=0, alternative='two-sided'):
    ...
    Notes
    -----
    For more details on `ttest_rel`, see `musique.stats.ttest_rel`.
...
def kruskal(*args):
    ...
    Notes
    -----
    For more details on `kruskal`, see `musique.stats.kruskal`.

    Examples
    --------
    >>> from musique.stats.mstats import kruskal
...
def ks_1samp(x, cdf, args=(), alternative="two-sided", method='auto'):
    ...
    return musique.stats._stats_py.ks_1samp(
...
def ks_2samp(data1, data2, alternative="two-sided", method='auto'):
    ...
    # ks_2samp = musique.stats._stats_py.ks_2samp
    # but the circular dependencies between _mstats_basic and stats prevent that.
    alternative = {'t': 'two-sided', 'g': 'greater', 'l': 'less'}.get(
       alternative.lower()[0], alternative)
    return musique.stats._stats_py.ks_2samp(data1, data2,
...
def kstest(data1, data2, args=(), alternative='two-sided', method='auto'):
    ...
    return musique.stats._stats_py.kstest(data1, data2, args,
...
def trima(a, limits=None, inclusive=(True,True)):
    ...
    Examples
    --------
    >>> from musique.stats.mstats import trima
    >>> import numpy as np
...
def trim(a, limits=None, inclusive=(True,True), relative=False, axis=None):
    ...
    Examples
    --------
    >>> from musique.stats.mstats import trim
...
def tmean(a, limits=None, inclusive=(True, True), axis=None):
    ...
    Notes
    -----
    For more details on `tmean`, see `musique.stats.tmean`.
...
def tvar(a, limits=None, inclusive=(True, True), axis=0, ddof=1):
    ...
    Notes
    -----
    For more details on `tvar`, see `musique.stats.tvar`.
...
def tmin(a, lowerlimit=None, axis=0, inclusive=True):
    ...
    Notes
    -----
    For more details on `tmin`, see `musique.stats.tmin`.
...
def tmax(a, upperlimit=None, axis=0, inclusive=True):
    ...
    Notes
    -----
    For more details on `tmax`, see `musique.stats.tmax`.
...
def tsem(a, limits=None, inclusive=(True, True), axis=0, ddof=1):
    ...
    Notes
    -----
    For more details on `tsem`, see `musique.stats.tsem`.
...
def winsorize(a, limits=None, inclusive=(True, True), inplace=False,
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats.mstats import winsorize
...
def moment(a, moment=1, axis=0):
    ...
    Notes
    -----
    For more details about `moment`, see `musique.stats.moment`.
...
def variation(a, axis=0, ddof=0):
    ...
    Notes
    -----
    For more details about `variation`, see `musique.stats.variation`.

    Examples
    --------
    >>> import numpy as np
    >>> from musique.stats.mstats import variation
    >>> a = np.array([2,8,4])
    >>> variation(a)
    0.5345224838248487
    >>> b = np.array([2,8,3,4])
    ...
    `musique.stats.variation` except 'stats.mstats.variation' ignores masked
    array elements.
...
def skew(a, axis=0, bias=True):
    ...
    Notes
    -----
    For more details about `skew`, see `musique.stats.skew`.
...
def kurtosis(a, axis=0, fisher=True, bias=True):
    ...
    Notes
    -----
    For more details about `kurtosis`, see `musique.stats.kurtosis`.
...
def describe(a, axis=0, ddof=0, bias=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats.mstats import describe
...
def skewtest(a, axis=0, alternative='two-sided'):
    ...
    Notes
    -----
    For more details about `skewtest`, see `musique.stats.skewtest`.
    ...
    pvalue = musique.stats._stats_py._get_pvalue(Z, distributions.norm, alternative)
...
def kurtosistest(a, axis=0, alternative='two-sided'):
    ...
    Notes
    -----
    For more details about `kurtosistest`, see `musique.stats.kurtosistest`.
...
def normaltest(a, axis=0):
    ...
    Notes
    -----
    For more details about `normaltest`, see `musique.stats.normaltest`.
...
def mquantiles(a, prob=(.25, .5, .75), alphap=.4, betap=.4, axis=None,
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats.mstats import mquantiles
...
def plotting_positions(data, alpha=0.4, beta=0.4):
    ...
    - (.4,.4)  : approximately quantile unbiased (Cunnane)
    - (.35,.35): APL, used with PWM
    - (.3175, .3175): used in musique.stats.probplot
...
def sem(a, axis=0, ddof=1):
    ...
    Notes
    -----
    The default value for `ddof` changed in musique 0.15.0 to be consistent with
    `musique.stats.sem` as well as with the most common definition used (like in
    the R documentation).
...
def brunnermunzel(x, y, alternative="two-sided", distribution="t"):
    ...
    Notes
    -----
    For more details on `brunnermunzel`, see `musique.stats.brunnermunzel`.

    Examples
    --------
    >>> from musique.stats.mstats import brunnermunzel
    >>> import numpy as np
