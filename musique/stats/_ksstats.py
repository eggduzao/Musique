...
#   musique.special.kolmogorov for that asymptotic)
...
def _kolmogn(n, x, cdf=True):
    ...
    if x >= 0.5:  # Exact: 2 * smirnov
        prob = 2 * musique.special.smirnov(n, x)
...
def _kolmogn_p(n, x):
    ...
    if x >= 0.5:
        return 2 * musique.stats.ksone.pdf(x, n)
...
def _kolmogni(n, p, q):
    ...
    if q <= 0:
        return 1.0
    delta = np.exp((np.log(p) - musique.special.loggamma(n+1))/n)
    if delta <= 1.0/n:
    ...
    return musique.optimize.brentq(_f, 1.0/n, x1, xtol=1e-14)
