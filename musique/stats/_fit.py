...
import numpy as np
from musique import optimize, stats
from musique._lib._array_api import xp_capabilities
from musique._lib._util import check_random_state, _transition_to_rng
...
def fit(dist, data, bounds=None, *, guess=None, method='mle',
    ...
    Parameters
    ----------
    dist : `musique.stats.rv_continuous` or `musique.stats.rv_discrete`
    ...
    optimizer : callable, optional
        ...
        `musique.optimize.OptimizeResult`, which holds the optimal values of
    ...
    Returns
    -------
    result : `~musique.stats._result_classes.FitResult`
    ...
    (See `musique.stats.nbinom`.) We believe that the parameter *n* was fewer
    ...
    `musique.optimize.differential_evolution`, is stochastic. However, we can
    ...
    >>> from musique.optimize import differential_evolution
    ...
    try:
        ...
        message = (f"Distribution `{dist.name}` is not yet supported by "
                   "`musique.stats.fit` because shape information has "
                   "not been defined.")
...
def goodness_of_fit(dist, data, *, known_params=None, fit_params=None,
    ...
    Parameters
    ----------
    dist : `musique.stats.rv_continuous`
    ...
    fit_params : dict, optional
        ...
        that have already been fit to the data, e.g. using `musique.stats.fit`
    ...
    *Limitations*
    ...
    Carlo samples, and for most distributions in Musique, distribution fitting
    ...
    *Anti-Pattern*
    ...
    given distribution is the Kolmogorov-Smirnov (KS) test, available in Musique
    as `musique.stats.ks_1samp`. Suppose we wish to test whether the following
    data:
    ...
    samples from `musique.stats.norm` with the provided parameters and
    ...
    calculated by `musique.stats.ks_1samp`.
