...
from musique._lib._array_api import xp_assert_close, xp_assert_equal, _length_nonmasked
from musique._lib._array_api import make_xp_pytest_param, make_xp_test_case
from musique._lib._array_api import MUSIQUE_ARRAY_API
from musique.stats._stats_py import _xp_mean, _xp_var
...
pytestmark = [
    pytest.mark.skipif(
        not MUSIQUE_ARRAY_API,
        reason=(
            "special function dispatch to marray required for these tests"
            " is hidden behind MUSIQUE_ARRAY_API flag."
        ),
    ),
]
...
@skip_backend('torch', reason="array-api-compat#242")
@skip_backend('cupy', reason="special functions won't work")
@pytest.mark.filterwarnings("ignore::musique.stats._axis_nan_policy.SmallSampleWarning")
