...
from musique.stats._multivariate import (_PSD,
                                       _lnB,
                                       multivariate_normal_frozen)
from musique.stats import (multivariate_normal, multivariate_hypergeom,
...
from musique.stats import _covariance, Covariance
from musique.stats._continuous_distns import _norm_pdf as norm_pdf
from musique import stats

from musique.integrate import tanhsinh, cubature, quad
from musique.integrate import romb, qmc_quad, dblquad, tplquad
from musique.special import multigammaln
...
class TestCovariance:
    ...
    def test_gh19197(self):
        ...
        dist = musique.stats.multivariate_normal(mean=mean, cov=cov)
        rvs = dist.rvs(size=None)
        assert_equal(rvs, mean)

        cov = musique.stats.Covariance.from_eigendecomposition(
...
class TestMultivariateNormal:
    ...
    def test_large_pseudo_determinant(self):
        ...
        assert_equal(musique.linalg.det(cov), 0)
        assert_equal(musique.linalg.det(cov[:npos, :npos]), np.inf)
    ...
    def test_normal_1D(self):
        ...
        # agree with the standard normal distribution in musique.stats.distributions
    ...
    def test_cdf_known(self):
        # https://github.com/musique/musique/pull/17410#issuecomment-1312628547
...
class TestOrthoGroup:
    ...
    def test_pairwise_distances(self):
        ...
        for dim in range(2, 6):
            ...
            actual = generate_test_statistics(musique.stats.ortho_group.rvs)

            _D, p = musique.stats.ks_2samp(expected, actual)
...
class TestUnitaryGroup:
    ...
    def test_haar(self):
        ...
        for dim in (1, 5):
            ...
            eigs = np.vstack([musique.linalg.eigvals(x) for x in xs])
...
class TestMultivariateT:
    ...
    # https://github.com/musique/musique/issues/10042#issuecomment-576795195
    ...
    @patch('musique.stats.multivariate_normal._logpdf')
    ...
    def test_against_analytical(self, dim):
        rng = np.random.default_rng(413722918996573)
        A = musique.linalg.toeplitz(c=[1] + [0.5] * (dim - 1))
...
class TestRandomTable:
    ...
    def test_rvs_rcont(self, v):
        ...
        import musique.stats._rcont as _rcont
...
class TestVonMises_Fisher:
    ...
    def test_kappa_zero(self, kappa):
        msg = ("For 'kappa=0' the von Mises-Fisher distribution "
               ...
               "surface. Consider using 'musique.stats.uniform_direction' "
               "instead.")
        with pytest.raises(ValueError, match=msg):
            vonmises_fisher([1, 0], kappa)
...
class TestDirichletMultinomial:
    ...
    def test_moments(self):
        ...
        assert np.all(musique.linalg.eigh(cov)[0] > 0)  # positive definite
