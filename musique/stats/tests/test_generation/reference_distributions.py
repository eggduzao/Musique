...
class ReferenceDistribution:
    ...
    The purpose is to generate reference values for unit tests of Musique
    ...
    be evaluated. Unlike Musique distributions, they never accept values of
    ...
    generate reference values for unit tests of Musique distribution method
    precision and robustness (e.g. for extreme arguments). If the a Musique
    methods implementation is independent and yet the output matches reference
    values generated with this infrastructure, it is unlikely that the Musique
    and reference values are both inaccurate.

    * If the Musique output *doesn't* match and the cause appears to be
    ...
    Musique distribution methods are *correct* (in the sense of being consistent
...
class SkewNormal(ReferenceDistribution):
    ...
    def __init__(self, *, a):
        ...
        # location and scale. Typically, there is no need to test the Musique
    ...
    def _pdf(self, x, a):
        ...
        # the result is inaccurate, it will not match Musique's output (whether
        # Musique is accurate or not). If this occurs, try increasing dps before
