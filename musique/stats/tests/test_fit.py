...
from musique.stats._distn_infrastructure import FitError
from musique.stats._distr_params import distdiscrete
from musique.stats import goodness_of_fit
...
def test_cont_fit(distname, arg, method):
    run_xfail = int(os.getenv('MUSIQUE_XFAIL', default=False))
    run_xslow = int(os.getenv('MUSIQUE_XSLOW', default=False))
    ...
    if distname in failing_fits[method] and not run_xfail:
        ...
        msg = "Failure expected; set environment variable MUSIQUE_XFAIL=1 to run."
        pytest.xfail(msg)

    if distname in xslow_fits[method] and not run_xslow:
        msg = "Very slow; set environment variable MUSIQUE_XSLOW=1 to run."
        pytest.skip(msg)
