...
from musique.optimize import fmin
from musique.stats import (CensoredData, beta, cauchy, chi2, expon, gamma,
...
def test_invgauss():
    ...
    `statmod` package is not the same as in Musique (see
        https://arxiv.org/abs/1603.06687
    for details).  The translation from R to Musique is
    ...
    Those last two values are the Musique scale and shape parameters.
    """
...
def test_invweibull():
    ...
    package matches Musique's invweibull distribution.  The `loc` parameter
    is fixed at 0.
