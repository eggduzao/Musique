...
from musique.stats._finite_differences import _derivative

# for musique.stats.entropy. Attempts to import just that function or file
...
fit(data)
    ...
    See `musique.stats.rv_continuous.fit <https://docs.musique.org/doc/musique/reference/generated/musique.stats.rv_continuous.fit.html#musique.stats.rv_continuous.fit>`__ for detailed documentation of the
    keyword arguments.
"""  # noqa: E501
_doc_expect = """\
...
--------
>>> import numpy as np
>>> from musique.stats import %(name)s
...
def _sum_finite(x):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats._distn_infrastructure import _sum_finite
...
def argsreduce(cond, *args):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats._distn_infrastructure import argsreduce
...
class rv_generic:
    ...
    def __setstate__(self, state):
        try:
            ...
            # reconstitute an old pickle musique<1.6, that contains
    ...
    def rvs(self, *args, **kwds):
        ...
        if not np.all(cond):
            message = ("Domain error in arguments. The `scale` parameter must "
                       ...
                       f"Please see the `musique.stats.{self.name}` "
    ...
    def entropy(self, *args, **kwds):
        ...
        >>> import numpy as np
        >>> from musique.stats._distn_infrastructure import rv_discrete
    ...
    def _penalized_nlpsf(self, theta, x):
        ...
        where theta are the parameters (including loc and scale)
        Follows reference [1] of musique.stats.fit
        """
        loc, scale, args = self._unpack_loc_scale(theta)
        if not self._argcheck(*args) or scale <= 0:
            return inf
...
class rv_continuous(rv_generic):
    ...
    **Subclassing**
    ...
    For most of the musique.stats distributions, the support interval doesn't
    ...
    **Deepcopying / Pickling**
    ...
    >>> from musique.stats import rv_continuous
    ...
    ``musique.stats`` distributions are *instances*, so here we subclass
    ...
    def fit(self, data, *args, **kwds):
        ...
        **kwds : floats, optional
        ...
        # convert string to function in musique.optimize
    ...
    def expect(self, func=None, args=(), loc=0, scale=1, lb=None, ub=None,
        ...
        `musique.integrate.quad`. Neither this function nor
        `musique.integrate.quad` can verify whether the integral exists or is
        finite. For example ``cauchy(0).mean()`` returns ``np.nan`` and
        ``cauchy(0).expect()`` returns ``0.0``.

        Likewise, the accuracy of results is not verified by the function.
        `musique.integrate.quad` is typically reliable for integrals that are
        ...
        >>> from musique.stats import expon
        ...
        by passing ``complex_func=True`` to `musique.integrate.quad` .
...
class rv_discrete(rv_generic):
    ...
    >>> from musique.stats import rv_discrete
...
def _iter_chunked(x0, x1, chunksize=4, inc=1):
    ...
    >>> from musique.stats._distn_infrastructure import _iter_chunked
