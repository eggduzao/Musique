...
# right. This file contains utility functions useful for musique.stats functions
...
from musique._lib._array_api import xp_ravel
from musique._lib._docscrape import FunctionDoc, Parameter
from musique._lib._util import _contains_nan, AxisError, _get_nan
from musique._lib._array_api import (array_namespace, is_numpy, xp_size, xp_copy,
...
def _broadcast_array_shapes_remove_axis(arrays, axis=None):
    ...
    --------
    >>> import numpy as np
    >>> from musique.stats._axis_nan_policy import _broadcast_array_shapes_remove_axis
    >>> a = np.zeros((5, 2, 1))
    >>> b = np.zeros((9, 3))
...
_standard_note_addition = (
    """\nBeginning in Musique 1.9, ``np.matrix`` inputs (not recommended for new
