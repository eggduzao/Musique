...
from musique.sparse import csc_array, vstack, issparse
...
from ._linprog_highs import _highs_to_musique_status_message
...
def _constraints_to_components(constraints):
    ...
    message = ("`constraints` (or each element within `constraints`) must be "
               ...
               "`musique.optimize.LinearConstraint`.")
    As = []
    b_ls = []
    b_us = []
...
def milp(c, *, integrality=None, bounds=None, constraints=None, options=None):
    ...
    bounds : musique.optimize.Bounds, optional
        ...
        non-negative.
    constraints : sequence of musique.optimize.LinearConstraint, optional
        ...
        instances of `musique.sparse.csc_array`. The ``keep_feasible`` parameter
    ...
    -------
    res : OptimizeResult
        An instance of :class:`musique.optimize.OptimizeResult`. The object
    ...
    be unfamiliar to users of `musique.optimize.linprog`, which only accepts
    ...
    >>> # from musique.optimize import linprog; res = linprog(c, A, b_u)
    >>> res.x
    array([1.8, 2.8])
    ...
    # Convert to musique-style status and message
    highs_status = highs_res.get('status', None)
    highs_message = highs_res.get('message', None)
    status, message = _highs_to_musique_status_message(highs_status,
                                                     highs_message)
    res['status'] = status
    res['message'] = message
