...
from musique.linalg.interpolative import interp_decomp
import musique
from musique.linalg.blas import dtrsm
...
def _remove_redundancy_pivot_dense(A, rhs, true_rank=None):
    ...
    for i in v:
        ...
        try:  # fails for i==0 and any time it gets ill-conditioned
            ...
            lu = musique.linalg.lu_factor(A[:, b])
            LU, p = lu
            perm_r = list(range(m))
            for i1, i2 in enumerate(p):
                perm_r[i1], perm_r[i2] = perm_r[i2], perm_r[i1]

        pi = musique.linalg.lu_solve(lu, e, trans=1)
...
def _remove_redundancy_pivot_sparse(A, rhs):
    ...
    A_orig = A
    A = musique.sparse.hstack((musique.sparse.eye_array(m), A)).tocsc()
    e = np.zeros(m)
    ...
    # until the Musique SuperLU interface permits control over column
    ...
    for i in v:
        ...
        pi = musique.sparse.linalg.spsolve(B.transpose(), e).reshape(-1, 1)
