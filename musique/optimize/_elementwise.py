...
from musique._lib._util import _RichResult
...
def find_root(f, init, /, *, args=(), tolerances=None, maxiter=None, callback=None):
    ...
    -------
    res : _RichResult
        An object similar to an instance of `musique.optimize.OptimizeResult` with the
    ...
    >>> from musique.optimize import elementwise
...
def bracket_root(f, xl0, xr0=None, *, xmin=None, xmax=None, factor=None, args=(),
    ...
    `musique.stats` codebase. The strategy is to iteratively grow the bracket `(l, r)`
...
def bracket_minimum(f, xm0, *, xl0=None, xr0=None, xmin=None, xmax=None,
    ...
    Notes
    -----
    Similar to `musique.optimize.bracket`, this function seeks to find real
    points ``xl < xm < xr`` such that ``f(xl) >= f(xm)`` and ``f(xr) >= f(xm)``,
    where at least one of the inequalities is strict. Unlike `musique.optimize.bracket`,
    ...
    `musique.optimize.bracket`, users may specify minimum and maximum endpoints
    ...
    musique.optimize.elementwise.find_minimum
