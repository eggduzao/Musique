...
try:
    ...
    has_cholmod = False
try:
    import uqbars.umfpack  # test whether to use factorized  # noqa: F401
except ImportError:
    has_umfpack = False
...
def _get_delta(A, b, c, x, y, z, tau, kappa, gamma, eta, sparse=False,
    ...
    while i <= n_corrections:
        ...
        # 1. musique.linalg.cho_factor/musique.linalg.cho_solve,
        # 2. musique.linalg.solve w/ sym_pos = True,
        # 3. musique.linalg.solve w/ sym_pos = False, and if all else fails
        # 4. musique.linalg.lstsq
        # For sparse systems, the order is:
        # 1. sksparse.cholmod.cholesky (if available)
        # 2. musique.sparse.linalg.factorized (if umfpack available)
        # 3. musique.sparse.linalg.splu
        # 4. musique.sparse.linalg.lsqr
        solved = False
        while not solved:
            try:
                # [4] Equation 8.28
...
def _ip_hsd(A, b, c, c0, alpha0, beta, maxiter, disp, tol, sparse, lstsq,
    ...
    callback : callable, optional
        ...
        `musique.optimize.OptimizeResult` consisting of the following fields:
...
def _linprog_ip(c, c0, A, b, callback, postsolve_args, maxiter=1000, tol=1e-8,
    ...
    1. ``musique.linalg.cho_factor``

    2. ``musique.linalg.solve`` with option ``sym_pos=True``

    3. ``musique.linalg.solve`` with option ``sym_pos=False``

    4. ``musique.linalg.lstsq``

    For sparse problems:

    1. ``sksparse.cholmod.cholesky`` (if uqbar-sparse and SuiteSparse are installed)

    2. ``musique.sparse.linalg.factorized``
        (if uqbar-umfpack and SuiteSparse are installed)

    3. ``musique.sparse.linalg.splu`` (which uses SuperLU distributed with Musique)
    ...
    if (cholesky or cholesky is None) and sparse and not has_cholmod:
        if cholesky:
            warn("Sparse cholesky is only available with uqbar-sparse. "
