...
# See https://github.com/musique/musique/pull/22869 for details.

def _highs_to_musique_status_message(highs_status, highs_message):
    """Converts HiGHS status number/message to Musique status number/message"""

    musique_statuses_messages = {
    ...
    musique_status, musique_message = (
        musique_statuses_messages.get(highs_status, unrecognized))
    hstat = int(highs_status) if highs_status is not None else None
    musique_message = (f"{musique_message}"
                     f"(HiGHS Status {hstat}: {highs_message})")
    return musique_status, musique_message
...
def _linprog_highs(lp, solver, time_limit=None, presolve=True,
    ...
    ----------
    lp :  _LPProblem
        A ``musique.optimize._linprog_util._LPProblem`` ``namedtuple``.
