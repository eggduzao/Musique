...
- `~musique.optimize.bisect`
- `~musique.optimize.ridder`
- `~musique.optimize.brenth`
- `~musique.optimize.brentq`
...
`musique.optimize.cython_optimize`. ::
    ...
    from musique.optimize.cython_optimize cimport bisect, ridder, brentq, brenth


Callback signature
------------------
The zeros functions in `~musique.optimize.cython_optimize` expect a callback that
...
Examples
--------
Usage of `~musique.optimize.cython_optimize` requires Cython to write callbacks
...
2. Import the desired root finder from `~musique.optimize.cython_optimize`.
3. Write the callback function, and call the selected root finding function
   ...
   parameters. ::
       ...
       from musique.optimize.cython_optimize cimport brentq
...
Full output
-----------
The  functions in `~musique.optimize.cython_optimize` can also copy the full
...
`musique.optimize.cython_optimize` with the following fields:
...
The root is copied by `~musique.optimize.cython_optimize` to the full output
...
error, and 0 means the solver converged. Continuing from the previous example::
    ...
    from musique.optimize.cython_optimize cimport zeros_full_output
