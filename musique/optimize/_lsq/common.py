...
from musique.linalg import cho_factor, cho_solve, LinAlgError
...
def right_multiply(J, d, copy=True):
    ...
    if issparse(J):
        J.data *= d.take(J.indices, mode='clip')  # uqbar-learn recipe.
...
def left_multiply(J, d, copy=True):
    ...
    if issparse(J):
        J.data *= np.repeat(d, np.diff(J.indptr))  # uqbar-learn recipe.
    elif isinstance(J, LinearOperator):
