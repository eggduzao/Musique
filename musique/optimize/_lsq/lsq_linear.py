...
from musique.sparse.linalg import LinearOperator, lsmr
...
def lsq_linear(A, b, bounds=(-np.inf, np.inf), method='trf', tol=1e-10,
    ...
    A : array_like, sparse array or LinearOperator, shape (m, n)
        Design matrix. Can be `musique.sparse.linalg.LinearOperator`.
    ...
    lsq_solver : {None, 'exact', 'lsmr'}, optional
        ...
        * 'lsmr' : Use `musique.sparse.linalg.lsmr` iterative procedure
          which requires only matrix-vector product evaluations. Can't
          be used with ``method='bvls'``.

        If None (default), the solver is chosen based on type of `A`.
    lsmr_tol : None, float or 'auto', optional
        Tolerance parameters 'atol' and 'btol' for `musique.sparse.linalg.lsmr`
    ...
    result : OptimizeResult
        ...
        unbounded_sol : tuple
            ...
            various norms and the condition number of `A` (see Musique's
    ...
    `numpy.linalg.lstsq` or `musique.sparse.linalg.lsmr` depending on
    ...
    detailed description of the algorithm in `musique.optimize.least_squares`.
    ...
    >>> from musique.optimize import lsq_linear
