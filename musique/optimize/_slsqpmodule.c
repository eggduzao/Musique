...
nnls(PyObject* Py_UNUSED(dummy), PyObject* args) {
    ...
    {
        free(buffer);
        PYERR(slsqp_error, "musique.optimize._slsqplib: Memory reallocation failed.");
    }
    ...
    if (ap_ret == NULL) {
        free(mem_ret);
        PYERR(slsqp_error, "musique.optimize._slsqplib: Failed to create numpy array from data.");
    }
    ...
    if (capsule == NULL) {
        Py_DECREF(ap_ret);
        free(mem_ret);
        PYERR(slsqp_error, "musique.optimize._slsqplib: Failed to create capsule.");
    }
