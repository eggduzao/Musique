...
class DoglegSubproblem(BaseQuadraticSubproblem):
    ...
    def newton_point(self):
        ...
        if self._newton_point is None:
            g = self.jac
            B = self.hess
            cho_info = musique.linalg.cho_factor(B)
            self._newton_point = -musique.linalg.cho_solve(cho_info, g)
    ...
    def solve(self, trust_radius):
        ...
        if musique.linalg.norm(p_best) < trust_radius:
            hits_boundary = False
        ...
        p_u_norm = musique.linalg.norm(p_u)
