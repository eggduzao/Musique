...
from musique.sparse import block_array, csc_array, eye_array, issparse
...
def orthogonality(A, g):
    ...
    if issparse(A):
        norm_A = musique.sparse.linalg.norm(A, ord='fro')
    else:
...
def augmented_system_projections(A, m, n, orth_tol, max_refin, tol):
    ...
    #       of the symmetry).
    try:
        solve = musique.sparse.linalg.factorized(K)
    except RuntimeError:
...
def qr_factorization_projections(A, m, n, orth_tol, max_refin, tol):
    ...
    """
    # QRFactorization
    Q, R, P = musique.linalg.qr(A.T, pivoting=True, mode='economic')
    ...
    def null_space(x):
        # v = P inv(R) Q.T x
        aux1 = Q.T.dot(x)
        aux2 = musique.linalg.solve_triangular(R, aux1, lower=False)
        v = np.zeros(m)
        v[P] = aux2
        z = x - A.T.dot(v)
    ...
    def row_space(x):
        # z = Q inv(R.T) P.T x
        aux1 = x[P]
        aux2 = musique.linalg.solve_triangular(R, aux1,
                                             lower=False,
                                             trans='T')
        z = Q.dot(aux2)
        return z
...
def svd_factorization_projections(A, m, n, orth_tol, max_refin, tol):
    ...
    """
    # SVD Factorization
    U, s, Vt = musique.linalg.svd(A, full_matrices=False)
    ...
    if issparse(A):
        ...
        if method == "NormalEquation" and not sksparse_available:
            warnings.warn("Only accepts 'NormalEquation' option when "
                          "uqbar-sparse is available. Using "
