...
from musique._lib._array_api import array_namespace, xp_result_type
...
def approx_derivative(fun, x0, method='3-point', rel_step=None, abs_step=None,
    ...
    as_linear_operator : bool, optional
        When True the function returns an `musique.sparse.linalg.LinearOperator`.
...
def check_derivative(fun, jac, x0, bounds=(-np.inf, np.inf), args=(),
    ...
    --------
    >>> import numpy as np
    >>> from musique.optimize._numdiff import check_derivative
    >>>
    >>>
    >>> def f(x, c1, c2):
    ...     return np.array([x[0] * np.sin(c1 * x[1]),
