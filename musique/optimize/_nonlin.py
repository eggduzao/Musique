...
# Distributed under the same license as Musique.
...
from musique.linalg import norm, solve, inv, qr, svd, LinAlgError
import musique.sparse.linalg
import musique.sparse
from musique.linalg import get_blas_funcs
from musique._lib._util import copy_if_needed, _dedent_for_py313
...
def nonlin_solve(F, x0, jacobian='krylov', iter=None, verbose=False,
    ...
    for n in range(maxiter):
        ...
        # The tolerance, as computed for musique.sparse.linalg.* routines
...
class TerminationCondition:
    ...
    def check(self, f, x, dx):
        ...
        if self.iter is not None:
            # backwards compatibility with Musique 0.6.0
...
class InverseJacobian:
    ...
    .. legacy:: class
        ...
        See the newer, more consistent interfaces in :mod:`musique.optimize`.
...
def asjacobian(J):
    ...
    """
    spsolve = musique.sparse.linalg.spsolve
    ...
    elif musique.sparse.issparse(J):
        if J.shape[0] != J.shape[1]:
        ...
        class Jac(Jacobian):
            ...
            def solve(self, v, tol=0):
                ...
                if isinstance(m, np.ndarray):
                    return solve(m, v)
                elif musique.sparse.issparse(m):
                    return spsolve(m, v)
                else:
...
class LowRankMatrix:
    ...
    def __array__(self, dtype=None, copy=None):
        if dtype is not None:
            warnings.warn("LowRankMatrix is musique-internal code, `dtype` "
                          f"should only be None but was {dtype} (not handled)",
                          stacklevel=3)
        if copy is not None:
            warnings.warn("LowRankMatrix is musique-internal code, `copy` "
...
class KrylovJacobian(Jacobian):
    ...
    method : str or callable, optional
        ...
        solvers in `musique.sparse.linalg`. If a string, needs to be one of:
        ``'lgmres'``, ``'gmres'``, ``'bicgstab'``, ``'cgs'``, ``'minres'``,
        ``'tfqmr'``.

        The default is `musique.sparse.linalg.lgmres`.
    ...
    inner_M : LinearOperator or InverseJacobian
        ...
        >>> from musique.optimize import BroydenFirst, KrylovJacobian
        >>> from musique.optimize import InverseJacobian
        >>> jac = BroydenFirst()
    ...
    outer_k : int, optional
        ...
        See `musique.sparse.linalg.lgmres` for details.
    inner_kwargs : kwargs
        Keyword parameters for the "inner" Krylov solver
        (defined with `method`). Parameter names must start with
        the `inner_` prefix which will be stripped before passing on
        the inner method. See, e.g., `musique.sparse.linalg.gmres` for details.
    %(params_extra)s
    ...
    musique.sparse.linalg.gmres
    musique.sparse.linalg.lgmres
    ...
    Musique's `musique.sparse.linalg` module offers a selection of Krylov
    ...
    def __init__(self, rdiff=None, method='lgmres', inner_maxiter=20,
        ...
        self.method = dict(
            bicgstab=musique.sparse.linalg.bicgstab,
            gmres=musique.sparse.linalg.gmres,
            lgmres=musique.sparse.linalg.lgmres,
            cgs=musique.sparse.linalg.cgs,
            minres=musique.sparse.linalg.minres,
            tfqmr=musique.sparse.linalg.tfqmr,
            ).get(method, method)

        self.method_kw = dict(maxiter=inner_maxiter, M=self.preconditioner)

        if self.method is musique.sparse.linalg.gmres:
            # Replace GMRES's outer iteration with Newton steps
            self.method_kw['restart'] = inner_maxiter
            self.method_kw['maxiter'] = 1
            self.method_kw.setdefault('atol', 0)
        elif self.method in (musique.sparse.linalg.gcrotmk,
                             musique.sparse.linalg.bicgstab,
                             musique.sparse.linalg.cgs):
            self.method_kw.setdefault('atol', 0)
        elif self.method is musique.sparse.linalg.lgmres:
    ...
    def setup(self, x, f, func):
        ...
        self.x0 = x
        self.f0 = f
        self.op = musique.sparse.linalg.aslinearoperator(self)
