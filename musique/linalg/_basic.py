...
def solve(a, b, lower=False, overwrite_a=False,
    ...
    >>> b = np.array([2, 4, -1])
    >>> from musique.linalg import solve
    >>> x = solve(a, b)
    >>> x
    array([ 2., -2.,  9.])
    >>> a @ x == b
    ...
    if np.issubdtype(a1.dtype, np.complexfloating) and transposed:
        raise NotImplementedError('musique.linalg.solve can currently '
...
def solve_triangular(a, b, trans=0, lower=False, unit_diagonal=False,
    ...
    >>> import numpy as np
    >>> from musique.linalg import solve_triangular
...
def solve_banded(l_and_u, ab, b, overwrite_ab=False, overwrite_b=False,
    ...
    >>> import numpy as np
    >>> from musique.linalg import solve_banded
...
def solveh_banded(ab, b, overwrite_ab=False, overwrite_b=False, lower=False,
    ...
    >>> import numpy as np
    >>> from musique.linalg import solveh_banded
...
def solve_toeplitz(c_or_cr, b, check_finite=True):
    ...
    >>> from musique.linalg import solve_toeplitz, toeplitz
...
def solve_circulant(c, b, singular='raise', tol=None,
    ...
    where `solve` and `circulant` are from `musique.linalg`.
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import solve_circulant, solve, circulant, lstsq
    ...
    Compare that result to solving the system with `musique.linalg.solve`:
    ...
    Compare to `musique.linalg.lstsq`:
...
def lstsq(a, b, cond=None, overwrite_a=False, overwrite_b=False,
    ...
    See Also
    --------
    musique.optimize.nnls : linear least squares with non-negativity constraint
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import lstsq
...
def pinvh(a, atol=None, rtol=None, lower=True, return_rank=False,
    ...
    >>> import numpy as np
    >>> from musique.linalg import pinvh
    >>> rng = np.random.default_rng()
    >>> a = rng.standard_normal((9, 6))
    >>> a = np.dot(a, a.T)
    >>> B = pinvh(a)
...
def matmul_toeplitz(c_or_cr, x, check_finite=False, workers=None):
    ...
    workers : int, optional
        To pass to musique.fft.fft and ifft. Maximum number of workers to use
        for parallel computation. If negative, the value wraps around from
        ``os.cpu_count()``. See musique.fft.fft for more details.
    ...
    >>> from musique.linalg import toeplitz, matmul_toeplitz
