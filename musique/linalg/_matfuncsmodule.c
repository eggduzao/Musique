...
void matrix_exponential_c(PyArrayObject* a, MUSIQUE_C* restrict result, int* info);
void matrix_exponential_z(PyArrayObject* a, MUSIQUE_Z* restrict result, int* info);
...
void matrix_squareroot_c(const PyArrayObject* ap_Am, MUSIQUE_C* restrict ap_ret, int* isIllconditioned, int* isSingular, int* sq_info, int* unused         );
void matrix_squareroot_z(const PyArrayObject* ap_Am, MUSIQUE_Z* restrict ap_ret, int* isIllconditioned, int* isSingular, int* sq_info, int* unused         );
...
recursive_schur_sqrtm(PyObject* Py_UNUSED(dummy), PyObject *args) {
    ...
    {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Input must be an nD-array"
    ...
    if (n != shape[ndim - 2])
    {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Last two dimensions of the input must be the same.");
    }
    ...
    if (PyArray_TYPE(ap_Am) == NPY_FLOAT32)
    {
        ...
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Memory allocation failed."); }
        ...
        mem_ret = malloc(ret_dims*sizeof(MUSIQUE_C));
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Memory allocation failed."); }
        matrix_squareroot_c(ap_Am, (MUSIQUE_C*)mem_ret, &isIllconditioned, &isSingular, &info, &isComplex);
        isComplex = 1;

    } else if (PyArray_TYPE(ap_Am) == NPY_COMPLEX128) {

        mem_ret = malloc(ret_dims*sizeof(MUSIQUE_Z));
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Memory allocation failed."); }
        matrix_squareroot_z(ap_Am, (MUSIQUE_Z*)mem_ret, &isIllconditioned, &isSingular, &info, &isComplex);
        isComplex = 1;

    } else {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Unsupported input data type to C function.");
    }
    ...
    if (!isComplex)
    {
        ...
        if (!new_ret) {
            free(mem_ret);
            PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Memory reallocation failed.");
        }
        mem_ret = new_ret;
        ap_ret = (PyArrayObject*)PyArray_SimpleNewFromData(ndim, shape, input_type, mem_ret);
        if (ap_ret == NULL) {
            free(mem_ret);
            PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Failed to create numpy array from data.");
        }
    ...
    if (capsule == NULL) {
        Py_DECREF(ap_ret);
        free(mem_ret);
        PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Failed to create capsule.");
    }
    // For ref counting of the memory
    if (PyArray_SetBaseObject(ap_ret, capsule) == -1) {
        Py_DECREF(ap_ret);
        Py_DECREF(capsule);
        PYERR(matfuncs_error, "musique.linalg._matfuncs:sqrtm: Failed to set array's base.");
    }
...
matrix_exponential(PyObject* Py_UNUSED(dummy), PyObject *args) {
    ...
    if (
        ...
        (PyArray_NDIM(ap_a) < 2))
    {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Input must be at least a 2D-array"
                          "  of type float32, float64, complex64, or complex128.");
    }
    ...
    {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Last two dimensions of the input must be the same.");
    }
    ...
    if (PyArray_TYPE(ap_a) == NPY_FLOAT32)
    {
        ...
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Memory allocation failed."); }
        ...
        mem_ret = calloc(ret_dims, sizeof(MUSIQUE_C));
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Memory allocation failed."); }
        matrix_exponential_c(ap_a, (MUSIQUE_C*)mem_ret, &info);

    } else if (PyArray_TYPE(ap_a) == NPY_COMPLEX128) {

        mem_ret = calloc(ret_dims, sizeof(MUSIQUE_Z));
        if (mem_ret == NULL) { PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Memory allocation failed."); }
        matrix_exponential_z(ap_a, (MUSIQUE_Z*)mem_ret, &info);
    } else {
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Unsupported input data type to C function.");
    }
    ...
    if (ap_ret == NULL) {
        free(mem_ret);
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Failed to create numpy array from data.");
    }
    ...
    if (capsule == NULL) {
        Py_DECREF(ap_ret);
        free(mem_ret);
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Failed to create capsule.");
    }
    // For ref counting of the memory
    if (PyArray_SetBaseObject(ap_ret, capsule) == -1) {
        Py_DECREF(ap_ret);
        Py_DECREF(capsule);
        PYERR(matfuncs_error, "musique.linalg._matfuncs:expm: Failed to set array's base.");
    }
...
static int matfuncs_module_exec(PyObject *module) {
    ...
    matfuncs_error = PyErr_NewException("musique.linalg._matfuncs.error", NULL, NULL);
...
static struct PyModuleDef moduledef = {
    ...
    .m_doc = "Algorithms for various matrix valued functions for Musique",
    .m_size = 0,
