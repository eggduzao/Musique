...
# Local imports
from musique._lib._util import _asarray_validated, _apply_over_batch
from ._misc import LinAlgError, _datacopied
from musique.linalg._misc import norm   # noqa: F401  (backwards compat)
from .lapack import (
...
def eig_banded(a_band, lower=False, eigvals_only=False, overwrite_a_band=False,
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import eig_banded
...
def eigvalsh(a, b=None, *, lower=True, overwrite_a=False,
    ...
    This function serves as a one-liner shorthand for `musique.linalg.eigh` with
    ...
    Examples
    --------
    For more examples see `musique.linalg.eigh`.

    >>> import numpy as np
    >>> from musique.linalg import eigvalsh
    >>> A = np.array([[6, 3, 1, 5], [3, 0, 5, 1], [1, 5, 6, 2], [5, 1, 2, 2]])
    >>> w = eigvalsh(A)
    >>> w
...
def eigvals_banded(a_band, lower=False, overwrite_a_band=False,
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import eigvals_banded
    >>> A = np.array([[1, 5, 2, 0], [5, 2, 5, 2], [2, 5, 3, 5], [0, 2, 5, 4]])
    >>> Ab = np.array([[1, 2, 3, 4], [5, 5, 5, 0], [2, 2, 0, 0]])
...
def eigvalsh_tridiagonal(d, e, select='a', select_range=None,
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import eigvalsh_tridiagonal, eigvalsh
    >>> d = 3*np.ones(4)
    >>> e = -1*np.ones(3)
...
def eigh_tridiagonal(d, e, eigvals_only=False, select='a', select_range=None,
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import eigh_tridiagonal
    >>> d = 3*np.ones(4)
    >>> e = -1*np.ones(3)
...
def hessenberg(a, calc_q=False, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import hessenberg
...
def cdf2rdf(w, v):
    ...
    For example, obtained by ``w, v = musique.linalg.eig(X)`` or
