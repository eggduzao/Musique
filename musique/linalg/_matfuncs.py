...
# Local imports
from musique.linalg import LinAlgError, LinAlgWarning
...
def fractional_matrix_power(A, t):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import fractional_matrix_power
    ...
    # this function calls onenormest which is in musique.sparse.
    A = _asarray_square(A)
    import musique.linalg._matfuncs_inv_ssq
    return musique.linalg._matfuncs_inv_ssq._fractional_matrix_power(A, t)
...
def logm(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import logm, expm
    >>> a = np.array([[1.0, 3.0], [1.0, 4.0]])
    >>> b = logm(a)
    >>> b
    ...
    import musique.linalg._matfuncs_inv_ssq
    F = musique.linalg._matfuncs_inv_ssq._logm(A)
    F = _maybe_real(A, F)
    errtol = 1000*eps
...
def expm(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import expm, sinm, cosm
    ...
    if info != 0:
        if info <= -11:
            ...
            raise MemoryError("musique.linalg.expm could not allocate "
            ...
            raise RuntimeError("musique.linalg.expm: Internal LAPACK "
...
def sqrtm(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import sqrtm
    >>> a = np.array([[1.0, 3.0], [1.0, 4.0]])
    >>> r = sqrtm(a)
    >>> r
    array([[ 0.75592895,  1.13389342],
    ...
    if a.dtype.char not in 'fdFD':
        raise TypeError("musique.linalg.sqrtm is not supported for the data type"
                        f" {a.dtype}")

    res, isIllconditioned, isSingular, info = recursive_schur_sqrtm(a)
    if info < 0:
        raise LinAlgError(f"Internal error in musique.linalg.sqrtm: {info}")
...
def tanm(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import tanm, sinm, cosm
    >>> a = np.array([[1.0, 3.0], [1.0, 4.0]])
    >>> t = tanm(a)
    >>> t
...
def coshm(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import tanhm, sinhm, coshm
    >>> a = np.array([[1.0, 3.0], [1.0, 4.0]])
    >>> c = coshm(a)
    >>> c
...
def funm(A, func, disp=True):
    ...
    >>> from musique.linalg import eigh
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import funm
    >>> a = np.array([[1.0, 3.0], [1.0, 4.0]])
    >>> funm(a, lambda x: x*x)
    array([[  4.,  15.],
           [  5.,  19.]])
...
def signm(A):
    ...
    Examples
    --------
    >>> from musique.linalg import signm, eigvals
    >>> a = [[1,2,3], [1,2,1], [1,1,1]]
    >>> eigvals(a)
