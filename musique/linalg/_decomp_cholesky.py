...
def cholesky(a, lower=False, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import cholesky
...
def cho_factor(a, lower=False, overwrite_a=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import cho_factor
...
def cho_solve(c_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import cho_factor, cho_solve
    >>> A = np.array([[9, 3, 1, 5], [3, 7, 5, 1], [1, 5, 9, 2], [5, 1, 2, 6]])
    >>> c, low = cho_factor(A)
...
def cholesky_banded(ab, overwrite_ab=False, lower=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import cholesky_banded
...
def cho_solve_banded(cb_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import cholesky_banded, cho_solve_banded
    >>> Ab = np.array([[0, 0, 1j, 2, 3j], [0, -1, -2, 3, 4], [9, 8, 7, 6, 9]])
    >>> A = np.diag(Ab[0,2:], k=2) + np.diag(Ab[1,1:], k=1)
    >>> A = A + A.conj().T + np.diag(Ab[2, :])
    >>> c = cholesky_banded(Ab)
