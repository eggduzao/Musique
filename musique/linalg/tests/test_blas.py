...
import musique
from musique.linalg import _fblas as fblas, get_blas_funcs, toeplitz, solve

try:
    from musique.linalg import _cblas as cblas
except ImportError:
    cblas = None
...
def test_gh_169309():
    x = np.repeat(10, 9)
    actual = musique.linalg.blas.dnrm2(x, 5, 3, -1)
...
def test_dnrm2_neg_incx():
    ...
    with assert_raises(fblas.__fblas_error):
        musique.linalg.blas.dnrm2(x, 5, 3, incx)
