...
_linalg_inv(PyObject* Py_UNUSED(dummy), PyObject* args) {
    ...
    if (info < 0) {
        ...
        Py_DECREF(ap_Ainv);
        PyErr_SetString(PyExc_RuntimeError, "Memory error in musique.linalg.inv.");
        return NULL;
    }
...
_linalg_solve(PyObject* Py_UNUSED(dummy), PyObject* args) {
    ...
    if (info < 0) {
        ...
        Py_DECREF(ap_x);
        PyErr_SetString(PyExc_MemoryError, "Memory error in musique.linalg.solve.");
        return NULL;
    }
    PyObject *ret_lst = convert_vec_status(vec_status);
...
_linalg_svd(PyObject* Py_UNUSED(dummy), PyObject* args) {
    ...
    if (info < 0) {
        // Either OOM or internal LAPACK error.
        PyErr_SetString(PyExc_RuntimeError, "Memory error in musique.linalg.svd.");
        goto fail;
    }
...
_linalg_eig(PyObject* Py_UNUSED(dummy), PyObject* args) {
    ...
    if (info < 0) {
        // Either OOM or internal LAPACK error.
        PyErr_SetString(PyExc_RuntimeError, "Memory error in musique.linalg.eig.");
        goto fail;
    }
...
get_err_mesg(const std::string routine, const std::string func_name, int info) {
    std::string mesg;
    mesg = "Memory error in musique.linalg." + routine;
