...
static float cnorm1(MUSIQUE_C*, float*, const Py_ssize_t);
static double znorm1(MUSIQUE_Z*, double*, const Py_ssize_t);
static float snorm1est(float*, int);
static double dnorm1est(double*, int);
static float cnorm1est(MUSIQUE_C*, int);
static double znorm1est(MUSIQUE_Z*, int);
...
static float
cnorm1(MUSIQUE_C* A, float* work, const Py_ssize_t n)
{
    Py_ssize_t i, j;
    float temp = 0.0;
    // Write absolute values of first row of A to work
...
static double
znorm1(MUSIQUE_Z* A, double* work, const Py_ssize_t n)
{
    Py_ssize_t i, j;
    double temp = 0.0;
    // Write absolute values of first row of A to work
...
static float
cnorm1est(MUSIQUE_C* A, int n)
{
    int from = 2*n, to = n, kase = 0, tempint, int1 = 1;
    int isave[3];
    float est;
    MUSIQUE_C dbl1 = CPLX_C(1.0, 0.0), dbl0 = CPLX_C(0.0, 0.0);
    char* opA;
    MUSIQUE_C* work_arr = PyMem_RawMalloc(3*n*sizeof(MUSIQUE_C));
    if (!work_arr) { return -100; }
...
static double
znorm1est(MUSIQUE_Z* A, int n)
{
    int from = 2*n, to = n, kase = 0, tempint, int1 = 1;
    int isave[3];
    double est;
    MUSIQUE_Z dbl1 = CPLX_Z(1.0, 0.0), dbl0 = CPLX_Z(0.0, 0.0);
    char* opA;
    MUSIQUE_Z* work_arr = PyMem_RawMalloc(3*n*sizeof(MUSIQUE_Z));
    if (!work_arr) { return -100; }
...
static void
pick_pade_structure_c(MUSIQUE_C* Am, const Py_ssize_t size_n, int* m, int* s)
{
    Py_ssize_t i, j;
    Py_ssize_t dims[2];
    int lm = 0, int1 = 1, n = (int)size_n;
    ...
    MUSIQUE_C cdbl1 = CPLX_C(1.0, 0.0), cdbl0 = CPLX_C(0.0, 0.0);
    float d4, d6, d8, d10, eta0, eta1, eta2, eta3, eta4, two_pow_s, temp, test;
    float theta[5];
    float coeff[5];
    // work_arr is two (n,) cols that will be used multiplying absA, alternating.
...
static void
pick_pade_structure_z(MUSIQUE_Z* Am, const Py_ssize_t size_n, int* m, int* s)
{
    Py_ssize_t i, j;
    Py_ssize_t dims[2];
    int lm = 0, int1 = 1, n = (int)size_n;
    ...
    MUSIQUE_Z cdbl1 = CPLX_Z(1.0, 0.0), cdbl0 = CPLX_Z(0.0, 0.0);
    double d4, d6, d8, d10, eta0, eta1, eta2, eta3, eta4, two_pow_s, temp, test;
    double theta[5];
    double coeff[5];
    // work_arr is two (n,) cols that will be used multiplying absA, alternating.
...
static void
pade_UV_calc_c(MUSIQUE_C* restrict Am, int* restrict ipiv, const Py_ssize_t size_n, const int m, int* info)
{
    // U, V computation.
    // We utilize the unused powers of Am as scratch memory depending on m value.
    // Final U is stored Am[3], final V is stored Am[1] for all m.
    ...
    float b[13];
    float two = 2.0;
    MUSIQUE_C temp1, temp2, temp3, cb1;
    MUSIQUE_C cdbl1 = CPLX_C(1.0, 0.0), cdbl0 = CPLX_C(0.0, 0.0), cdblm1 = CPLX_C(-1.0, 0.0);

#if defined(_MSC_VER)
    MUSIQUE_C inter1, inter2, inter3, inter4;
#endif
...
static void
pade_UV_calc_z(MUSIQUE_Z* restrict Am, int* restrict ipiv, const Py_ssize_t size_n, const int m, int* info)
{
    // U, V computation.
    // We utilize the unused powers of Am as scratch memory depending on m value.
    // Final U is stored Am[3], final V is stored Am[1] for all m.
    ...
    double b[13];
    double two = 2.0;
    MUSIQUE_Z temp1, temp2, temp3, cb1;
    MUSIQUE_Z cdbl1 = CPLX_Z(1.0, 0.0), cdbl0 = CPLX_Z(0.0, 0.0), cdblm1 = CPLX_Z(-1.0, 0.0);

#if defined(_MSC_VER)
    MUSIQUE_Z inter1, inter2, inter3, inter4;
#endif
...
void
matrix_exponential_c(PyArrayObject* a, MUSIQUE_C* restrict result, int* info)
{
    int m = 0, s = 0, is_lower = 0;
    npy_intp lband = 0, uband = 0;
    // --------------------------------------------------------------------
    // Input Array Attributes
    // --------------------------------------------------------------------
    MUSIQUE_C* restrict a_data = (MUSIQUE_C*)PyArray_DATA(a);
    int ndim = PyArray_NDIM(a);              // Number of dimensions
    npy_intp* shape = PyArray_SHAPE(a);      // Array shape
    int n = (int)shape[ndim - 1];                // Slice size
    npy_intp* restrict strides = PyArray_STRIDES(a);
    ...
    // 2*n is both for 1-norm calculations
    // 2*n for the scaling/squaring in the triangular case
    MUSIQUE_C* restrict Am = malloc(sizeof(MUSIQUE_C)*(6*n*n + 4*n));
    if (Am == NULL) { *info = -100; return; }
    int* ipiv = malloc(sizeof(int)*n);
    if (ipiv == NULL) { free(Am); *info = -101; return; }
    MUSIQUE_C* restrict Am1 = &Am[n*n];
    // These two arrays are only used for the triangular case for scaling/squaring
    MUSIQUE_C* restrict diag_aw = &Am[6*n*n + 2*n];
    MUSIQUE_C* restrict sd = &Am[6*n*n + 3*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        MUSIQUE_C* restrict slice_ptr = (MUSIQUE_C*)(a_data + (offset/sizeof(MUSIQUE_C)));

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                Am1[i * n + j] = *(slice_ptr + (i*strides[ndim - 2]/sizeof(MUSIQUE_C)) + (j*strides[ndim - 1]/sizeof(MUSIQUE_C)));
            }
        }
        ...
        if (s > 0) {
            ...
            if ((lband == 0) || (uband == 0)) {
                ...
                for (int iter = s - 1; iter >= 0; iter--) {
#if defined(_MSC_VER)
                    MUSIQUE_C c_one = CPLX_C(1.0f, 0.0f);
                    MUSIQUE_C c_zero = CPLX_C(0.0f, 0.0f);
                    cgemm_("N", "N", &n, &n, &n, &c_one, temp1, &n, temp1, &n, &c_zero, temp2, &n);
#else
                    cgemm_("N", "N", &n, &n, &n, &(MUSIQUE_C){CPLX_C(1.0f, 0.0f)}, temp1, &n, temp1, &n, &(MUSIQUE_C){CPLX_C(0.0f, 0.0f)}, temp2, &n);
#endif
                    MUSIQUE_C* swap = temp1;
                    temp1 = temp2;
                    temp2 = swap;
                    ...
                    if (is_lower) {
                        ...
                        for (npy_intp i = 0; i < n - 1; i++) {
#if defined(_MSC_VER)
                            MUSIQUE_C d_i = _FCmulcr(diag_aw[i], scale);
                            MUSIQUE_C d_ip1 = _FCmulcr(diag_aw[i + 1], scale);
                            MUSIQUE_C diff = CPLX_C(crealf(d_ip1) - crealf(d_i), cimagf(d_ip1) - cimagf(d_i));
                            MUSIQUE_C exp_d_i = cexpf(d_i);
                            MUSIQUE_C exp_d_ip1 = cexpf(d_ip1);
                            MUSIQUE_C exp_diff = CPLX_C(crealf(exp_d_ip1) - crealf(exp_d_i), cimagf(exp_d_ip1) - cimagf(exp_d_i));
                            float diff_abs2 = crealf(diff)*crealf(diff) + cimagf(diff)*cimagf(diff);
                            MUSIQUE_C diff_inv = CPLX_C(crealf(diff)/diff_abs2, -cimagf(diff)/diff_abs2);
                            MUSIQUE_C exp_sinch = (cabsf(diff) == 0.0f) ? exp_d_i : _FCmulcc(exp_diff, diff_inv);
                            temp1[i * n + i] = exp_d_i;                                            // diagonal
                            temp1[i * n + (i + 1)] = _FCmulcr(_FCmulcc(exp_sinch, sd[i]), scale);  // subdiagonal
#else
                            MUSIQUE_C d_i = diag_aw[i] * scale;
                            MUSIQUE_C d_ip1 = diag_aw[i + 1] * scale;
                            MUSIQUE_C diff = d_ip1 - d_i;
                            MUSIQUE_C exp_sinch = (cabsf(diff) == 0.0f) ? cexpf(d_i) : (cexpf(d_ip1) - cexpf(d_i)) / diff;
...
void
matrix_exponential_z(PyArrayObject* a, MUSIQUE_Z* restrict result, int* info)
{
    int m = 0, s = 0, is_lower = 0;
    npy_intp lband = 0, uband = 0;
    // --------------------------------------------------------------------
    // Input Array Attributes
    // --------------------------------------------------------------------
    MUSIQUE_Z* restrict a_data = (MUSIQUE_Z*)PyArray_DATA(a);
    int ndim = PyArray_NDIM(a);              // Number of dimensions
    npy_intp* shape = PyArray_SHAPE(a);      // Array shape
    int n = (int)shape[ndim - 1];                // Slice size
    npy_intp* restrict strides = PyArray_STRIDES(a);
    ...
    // 2*n is both for 1-norm calculations
    // 2*n for the scaling/squaring in the triangular case
    MUSIQUE_Z* restrict Am = malloc(sizeof(MUSIQUE_Z)*(6*n*n + 4*n));
    if (Am == NULL) { *info = -100; return; }
    int* ipiv = malloc(sizeof(int)*n);
    if (ipiv == NULL) { free(Am); *info = -101; return; }
    MUSIQUE_Z* restrict Am1 = &Am[n*n];
    // These two arrays are only used for the triangular case for scaling/squaring
    MUSIQUE_Z* restrict diag_aw = &Am[6*n*n + 2*n];
    MUSIQUE_Z* restrict sd = &Am[6*n*n + 3*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        MUSIQUE_Z* restrict slice_ptr = (MUSIQUE_Z*)(a_data + (offset/sizeof(MUSIQUE_Z)));

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                Am1[i * n + j] = *(slice_ptr + (i*strides[ndim - 2]/sizeof(MUSIQUE_Z)) + (j*strides[ndim - 1]/sizeof(MUSIQUE_Z)));
            }
        }
        ...
        if (s > 0) {
            ...
            if ((lband == 0) || (uband == 0)) {
                ...
                for (int iter = s - 1; iter >= 0; iter--) {
#if defined(_MSC_VER)
                    MUSIQUE_Z z_one = CPLX_Z(1.0, 0.0);
                    MUSIQUE_Z z_zero = CPLX_Z(0.0, 0.0);
                    zgemm_("N", "N", &n, &n, &n, &z_one, temp1, &n, temp1, &n, &z_zero, temp2, &n);
#else
                    zgemm_("N", "N", &n, &n, &n, &(MUSIQUE_Z){CPLX_Z(1.0, 0.0)}, temp1, &n, temp1, &n, &(MUSIQUE_Z){CPLX_Z(0.0, 0.0)}, temp2, &n);
#endif
                    MUSIQUE_Z* swap = temp1;
                    temp1 = temp2;
                    temp2 = swap;
                    ...
                    if (is_lower) {
                        ...
                        for (npy_intp i = 0; i < n - 1; i++) {
#if defined(_MSC_VER)
                            MUSIQUE_Z d_i = _Cmulcr(diag_aw[i], scale);
                            MUSIQUE_Z d_ip1 = _Cmulcr(diag_aw[i + 1], scale);
                            MUSIQUE_Z diff = CPLX_Z(creal(d_ip1) - creal(d_i), cimag(d_ip1) - cimag(d_i));
                            MUSIQUE_Z exp_d_i = cexp(d_i);
                            MUSIQUE_Z exp_d_ip1 = cexp(d_ip1);
                            MUSIQUE_Z exp_diff = CPLX_Z(creal(exp_d_ip1) - creal(exp_d_i), cimag(exp_d_ip1) - cimag(exp_d_i));
                            double diff_abs2 = creal(diff)*creal(diff) + cimag(diff)*cimag(diff);
                            MUSIQUE_Z diff_inv = CPLX_Z(creal(diff)/diff_abs2, -cimag(diff)/diff_abs2);
                            MUSIQUE_Z exp_sinch = (cabs(diff) == 0.0) ? exp_d_i : _Cmulcc(exp_diff, diff_inv);
                            temp1[i * n + i] = exp_d_i;                                          // diagonal
                            temp1[i * n + (i + 1)] = _Cmulcr(_Cmulcc(exp_sinch, sd[i]), scale); // subdiagonal
#else
                            MUSIQUE_Z d_i = diag_aw[i] * scale;
                            MUSIQUE_Z d_ip1 = diag_aw[i + 1] * scale;
                            MUSIQUE_Z diff = d_ip1 - d_i;
                            MUSIQUE_Z exp_sinch = (cabs(diff) == 0.0) ? cexp(d_i) : (cexp(d_ip1) - cexp(d_i)) / diff;
