...
static int sqrtm_recursion_c(MUSIQUE_C* T, npy_intp bign, npy_intp n);
static int sqrtm_recursion_z(MUSIQUE_Z* T, npy_intp bign, npy_intp n);
...
matrix_squareroot_s(const PyArrayObject* ap_Am, float* restrict ret_data, int* isIllconditioned, int* isSingular, int* sq_info, int* view_as_complex)
{
    ...
    *isSingular = 0;
    ...
    MUSIQUE_C* complex_data = &((MUSIQUE_C*)buffer)[0];
    MUSIQUE_C* complex_vs = &((MUSIQUE_C*)buffer)[n*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        if (isComplex)
        {
            ...
            if (!isSchur)
            {
                ...
                cgemm_("N", "N", &intn, &intn, &intn, &c_one, complex_vs, &intn, complex_data, &intn, &c_zero, &((MUSIQUE_C*)ret_data)[idx*n*n], &intn);
                cgemm_("N", "C", &intn, &intn, &intn, &c_one, &((MUSIQUE_C*)ret_data)[idx*n*n], &intn, complex_vs, &intn, &c_zero, complex_data, &intn);
            }
        ...
        if (isComplex)
        {
            ...
            swap_cf_c(complex_data, &((MUSIQUE_C*)ret_data)[idx*n*n], n, n, n);
        } else {
...
matrix_squareroot_d(const PyArrayObject* ap_Am, double* restrict ret_data, int* isIllconditioned, int* isSingular, int* sq_info, int* view_as_complex)
{
    ...
    *isSingular = 0;
    ...
    MUSIQUE_Z* complex_data = &((MUSIQUE_Z*)buffer)[0];
    MUSIQUE_Z* complex_vs = &((MUSIQUE_Z*)buffer)[n*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        if (isComplex)
        {
            ...
            if (!isSchur)
            {
                MUSIQUE_Z c_one = CPLX_Z(1.0, 0.0);
                MUSIQUE_Z c_zero = CPLX_Z(0.0, 0.0);
                zgemm_("N", "N", &intn, &intn, &intn, &c_one, complex_vs, &intn, complex_data, &intn, &c_zero, &((MUSIQUE_Z*)ret_data)[idx*n*n], &intn);
                zgemm_("N", "C", &intn, &intn, &intn, &c_one, &((MUSIQUE_Z*)ret_data)[idx*n*n], &intn, complex_vs, &intn, &c_zero, complex_data, &intn);
            }
        } else {
        ...
        if (isComplex)
        {
            ...
            swap_cf_z(complex_data, &((MUSIQUE_Z*)ret_data)[idx*n*n], n, n, n);
        } else {
...
void
matrix_squareroot_c(const PyArrayObject* ap_Am, MUSIQUE_C* restrict ret_data, int* isIllconditioned, int* isSingular, int* sq_info, int* unused)
{

    *isIllconditioned = 0;
    *isSingular = 0;

    MUSIQUE_C* restrict Am_data = (MUSIQUE_C*)PyArray_DATA(ap_Am);
    int ndim = PyArray_NDIM(ap_Am);
    npy_intp* shape = PyArray_SHAPE(ap_Am);
    npy_intp n = shape[ndim - 1];
    npy_intp* restrict strides = PyArray_STRIDES(ap_Am);
    ...
    int info = 0, sdim = 0, lwork = -1, intn = (int)n;
    MUSIQUE_C tmp_float = CPLX_C(0.0f, 0.0f), cone = CPLX_C(1.0f, 0.0f), czero = CPLX_C(0.0f, 0.0f);
    ...
    MUSIQUE_C* buffer = malloc(buffer_size*sizeof(MUSIQUE_C));
    if (buffer == NULL) { *sq_info = -101; return; }


    MUSIQUE_C* restrict data = &buffer[0];
    MUSIQUE_C* restrict vs = &buffer[n*n];
    MUSIQUE_C* restrict w = &buffer[2*n*n];
    float* restrict rwork = (float*)(&buffer[2*n*n + n]);
    MUSIQUE_C* restrict work = &buffer[2*n*n + 2*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        MUSIQUE_C* restrict slice_ptr = (MUSIQUE_C*)(Am_data + (offset/sizeof(MUSIQUE_C)));
        for (Py_ssize_t i = 0; i < n; i++) {
            for (Py_ssize_t j = 0; j < n; j++) {
                vs[i * n + j] = *(slice_ptr + (i*strides[ndim - 2]/sizeof(MUSIQUE_C)) + (j*strides[ndim - 1]/sizeof(MUSIQUE_C)));
            }
        }
...
void
matrix_squareroot_z(const PyArrayObject* ap_Am, MUSIQUE_Z* restrict ret_data, int* isIllconditioned, int* isSingular, int* sq_info, int* unused)
{

    *isIllconditioned = 0;
    *isSingular = 0;

    MUSIQUE_Z* restrict Am_data = (MUSIQUE_Z*)PyArray_DATA(ap_Am);
    int ndim = PyArray_NDIM(ap_Am);
    npy_intp* shape = PyArray_SHAPE(ap_Am);
    npy_intp n = shape[ndim - 1];
    npy_intp* restrict strides = PyArray_STRIDES(ap_Am);
    ...
    int info = 0, sdim = 0, lwork = -1, intn = (int)n;
    MUSIQUE_Z tmp_float = CPLX_Z(0.0f, 0.0f), cone = CPLX_Z(1.0f, 0.0f), czero = CPLX_Z(0.0f, 0.0f);
    zgees_("V", "N", NULL, &intn, NULL, &intn, &sdim, NULL, NULL, &intn, &tmp_float, &lwork, NULL, NULL, &info);
    if (info != 0) { *sq_info = -100; return; }

    lwork = (int)creal(tmp_float);
    size_t buffer_size = 2*n*n + 2*n + lwork;
    MUSIQUE_Z* buffer = malloc(buffer_size*sizeof(MUSIQUE_Z));
    if (buffer == NULL) { *sq_info = -101; return; }


    MUSIQUE_Z* restrict data = &buffer[0];
    MUSIQUE_Z* restrict vs = &buffer[n*n];
    MUSIQUE_Z* restrict w = &buffer[2*n*n];
    double* restrict rwork = (double*)(&buffer[2*n*n + n]);
    MUSIQUE_Z* restrict work = &buffer[2*n*n + 2*n];
    ...
    for (npy_intp idx = 0; idx < outer_size; idx++) {
        ...
        MUSIQUE_Z* restrict slice_ptr = (MUSIQUE_Z*)(Am_data + (offset/sizeof(MUSIQUE_Z)));
        for (Py_ssize_t i = 0; i < n; i++) {
            for (Py_ssize_t j = 0; j < n; j++) {
                vs[i * n + j] = *(slice_ptr + (i*strides[ndim - 2]/sizeof(MUSIQUE_Z)) + (j*strides[ndim - 1]/sizeof(MUSIQUE_Z)));
            }
        }
...
int
sqrtm_recursion_c(MUSIQUE_C* T, npy_intp bign, npy_intp n)
{
    int i, i1 = 0, i2 = 0, info = 0, j, halfn, otherhalfn, int1 = 1, intbign = (int)bign;
    float scale = 0.0;
    if (n == 1)
    {
        ...
        MUSIQUE_C tt = CPLX_C(crealf(T[0]) + crealf(T[bign + 1]), cimagf(T[0]) + cimagf(T[bign + 1]));
        float cc = cabsf(tt);
        ...
        otherhalfn = (int)(n - halfn);
        MUSIQUE_C* T11 = T;
        MUSIQUE_C* T22 = &T[halfn*(bign + 1)];
        MUSIQUE_C* T12 = &T[halfn*bign];
...
int
sqrtm_recursion_z(MUSIQUE_Z* T, npy_intp bign, npy_intp n)
{
    int i, i1 = 0, i2 = 0, info = 0, j, halfn, otherhalfn, int1 = 1, intbign = (int)bign;
    double scale = 0.0;
    if (n == 1)
    {
...
#if defined(_MSC_VER)
        // MSVC does not support complex division
        MUSIQUE_Z tt = CPLX_Z(creal(T[0]) + creal(T[bign + 1]), cimag(T[0]) + cimag(T[bign + 1]));
        double cc = cabs(tt);
        ...
        otherhalfn = (int)(n - halfn);
        MUSIQUE_Z* T11 = T;
        MUSIQUE_Z* T22 = &T[halfn*(bign + 1)];
        MUSIQUE_Z* T12 = &T[halfn*bign];
