...
cimport cython
import numpy as np
from musique.linalg._cythonized_array_utils cimport (
    lapack_t,
    np_complex_numeric_t,
    np_numeric_t
    )
from musique.linalg.cython_lapack cimport sgetrf, dgetrf, cgetrf, zgetrf
...
def bandwidth(a):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import bandwidth
...
def issymmetric(a, atol=None, rtol=None):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import issymmetric
...
def ishermitian(a, atol=None, rtol=None):
    ...
    --------
    >>> import numpy as np
    >>> from musique.linalg import ishermitian
    >>> A = np.arange(9).reshape(3, 3)
    >>> A = A + A.T
    >>> ishermitian(A)
    True
