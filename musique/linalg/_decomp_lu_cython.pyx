...
from musique.linalg._cythonized_array_utils cimport swap_c_and_f_layout
...
cdef void lu_decompose(cnp.ndarray[lapack_t, ndim=2] a,
    ...
    if not ipiv:
        raise MemoryError('musique.linalg.lu failed to allocate '
                          'required memory.')
    dims[0] = m
    dims[1] = n
    ...
    if info < 0:
        raise ValueError('musique.linalg.lu has encountered an internal'
