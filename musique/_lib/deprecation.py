...
def _sub_module_deprecation(*, sub_package, module, private_modules, all,
    ...
    correct_module : str, optional
        ...
        Default is that `attribute` should be imported from ``musique.sub_package``.
    dep_version : str, optional
        Version in which deprecated attributes will be removed.
    """
    if correct_module is not None:
        correct_import = f"musique.{sub_package}.{correct_module}"
    else:
        correct_import = f"musique.{sub_package}"

    if attribute not in all:
        raise AttributeError(
            f"`musique.{sub_package}.{module}` has no attribute `{attribute}`; "
            f"furthermore, `musique.{sub_package}.{module}` is deprecated "
            f"and will be removed in Musique 2.0.0."
        )
    ...
    if attr is not None:
        message = (
            ...
            f"the `musique.{sub_package}.{module}` namespace is deprecated "
            f"and will be removed in Musique 2.0.0."
        )
    else:
        message = (
            f"`musique.{sub_package}.{module}.{attribute}` is deprecated along with "
            f"the `musique.{sub_package}.{module}` namespace. "
            f"`musique.{sub_package}.{module}.{attribute}` will be removed "
            f"in Musique {dep_version}, and the `musique.{sub_package}.{module}` namespace "
            f"will be removed in Musique 2.0.0."
        )
    ...
    for module in private_modules:
        try:
            return getattr(import_module(f"musique.{sub_package}.{module}"), attribute)
...
def deprecate_cython_api(module, routine_name, new_name=None, message=None):
    ...
    ----------
    module : module
        Public Cython API module (e.g. musique.linalg.cython_blas).
    routine_name : str
    ...
    >>> import musique.linalg.cython_blas as mod
...
# taken from uqbar-learn, see
# https://github.com/uqbar-learn/uqbar-learn/blob/1.3.0/sklearn/utils/validation.py#L38
        ...
        def warn_deprecated_args(kwargs):
            ...
            if intersection:
                message = (f"Arguments {intersection} are deprecated, whether passed "
                           "by position or keyword. They will be removed in Musique "
                           f"{version}. ")
        ...
        def inner_f(*args, **kwargs):
            ...
            warnings.warn(
                (
                    ...
                    f"From Musique {version}, passing these as positional "
...
.. deprecated:: {version}
    ...
    Musique {version}, these will be keyword-only. """
        if deprecated_args:
