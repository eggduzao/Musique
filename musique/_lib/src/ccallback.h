 ...
 *
 * For an example see `musique/_lib/src/_test_ccallback.c`.
 */
 ...
 */
static MUSIQUE_TLS ccallback_t *_active_ccallback = NULL;
                                             ...
                                             const char *capsule_signature)
{
    ...
    PyErr_Format(PyExc_ValueError,
                 "Invalid musique.LowLevelCallable signature \"%s\". Expected one of: %R",
...
{
    static MUSIQUE_TLS PyTypeObject *lowlevelcallable_type = NULL;
    ...
    if (lowlevelcallable_type == NULL) {
        ...
        module = PyImport_ImportModule("musique._lib._ccallback");
        if (module == NULL) {
            goto error;
        }
