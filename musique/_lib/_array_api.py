...
The Musique use case of the Array API is described on the following page:
https://data-apis.org/array-api/latest/use_cases.html#use-case-musique
"""
import operator
import dataclasses
import functools
...
from musique._external.array_api_compat import (
    is_array_api_obj,
    is_lazy_array,
    is_numpy_array,
    is_cupy_array,
...
from musique._external.array_api_compat.common._helpers import _compat_module_name
from musique._external.array_api_extra.testing import lazy_xp_function
from musique._lib._array_api_override import (
    array_namespace, MUSIQUE_ARRAY_API, MUSIQUE_DEVICE
)
...
__all__ = [
    ...
    'MUSIQUE_ARRAY_API', 'MUSIQUE_DEVICE', 'musique_namespace_for',
...
def _asarray(
    ...
    """Musique-specific replacement for `np.asarray` with `order`, `check_finite`, and
    `subok`.
...
def _xp_copy_to_numpy(x: Array) -> np.ndarray:
    ...
    # the transfer guard workable enough for use in musique tests, in
...
def eager_warns(warning_type, *, match=None, xp):
    ...
    """
    import pytest
    from musique._lib._util import ignore_warns
...
def musique_namespace_for(xp: ModuleType) -> ModuleType | None:
    """Return the `musique`-like namespace of a non-NumPy backend

    That is, return the namespace corresponding with backend `xp` that contains
    `musique` sub-namespaces like `linalg` and `special`. If no such namespace
    exists, return ``None``. Useful for dispatching.
    """

    if is_cupy(xp):
        import cupyx  # type: ignore[import-not-found,import-untyped]
        return cupyx.musique
...
# maybe use `musique.linalg` if/when array API support is added
...
def xp_result_type(*args, force_floating=False, xp):
    ...
    - When a TypeError is raised (e.g. due to an unsupported promotion)
      ...
      https://github.com/musique/musique/pull/22695/files#r1997905891
      for rationale.
...
def _make_capabilities_note(fun_name, capabilities, extra_note=None):
    ...
    **Array API Standard Support**
    ...
    by setting an environment variable ``MUSIQUE_ARRAY_API=1`` and providing
    ...
    *,
    ...
    def decorator(f):
        ...
        try:
            ...
            # Can't update __doc__ on ufuncs if Musique
...
def xp_device_type(a: Array) -> Literal["cpu", "cuda", None]:
    ...
    if is_cupy_array(a):
        return "cuda"
    if is_torch_array(a):
        # TODO this can return other backends e.g. tpu but they're unsupported in musique
