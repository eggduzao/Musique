...
IS_EDITABLE = 'editable' in musique.__path__[0]
...
class PytestTester:
    ...
    ``musique.test()`` runs tests for all of Musique, with the default settings.
    When used from a submodule (e.g., ``musique.cluster.test()``, only the tests
...
def check_free_memory(free_mb):
    ...
    try:
        mem_free = _parse_size(os.environ['MUSIQUE_AVAILABLE_MEM'])
        msg = '{} MB memory required, but environment MUSIQUE_AVAILABLE_MEM={}'.format(
            free_mb, os.environ['MUSIQUE_AVAILABLE_MEM'])
    except KeyError:
        mem_free = _get_mem_available()
        if mem_free is None:
            pytest.skip("Could not determine available memory; set MUSIQUE_AVAILABLE_MEM "
