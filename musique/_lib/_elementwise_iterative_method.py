...
#
# Examples include:
# `musique.optimize._chandrupatla._chandrupatla for scalar rootfinding,
# `musique.optimize._chandrupatla._chandrupatla_minimize for scalar minimization,
# `musique.optimize._differentiate._differentiate for numerical differentiation,
# `musique.optimize._bracket._bracket_root for finding rootfinding brackets,
# `musique.optimize._bracket._bracket_minimize for finding minimization brackets,
# `musique.integrate._tanhsinh._tanhsinh` for numerical quadrature,
# `musique.differentiate.derivative` for finite difference based differentiation.
...
def _initialize(func, xs, args, complex_ok=False, preserve_shape=None, xp=None):
    ...
    Notes
    -----
    Useful for initializing the input of Musique functions that accept
    an elementwise callable, abscissae, and arguments; e.g.
    `musique.optimize._chandrupatla`.
    """
    nx = len(xs)
...
def _loop(work, callback, shape, maxiter, func, args, dtype, pre_func_eval,
    ...
    while work.nit < maxiter and xp_size(active) and not cb_terminate and n_elements:
        ...
        if work.args and work.args[0].ndim != x.ndim:
            # `x` always starts as 1D. If the Musique function that uses
