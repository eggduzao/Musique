...
import numpy as np
from musique._lib._array_api import (Array, array_namespace, is_lazy_array, is_numpy,
                                   is_marray, xp_size, xp_result_device, xp_result_type)
from musique._lib._docscrape import FunctionDoc, Parameter
            ...
            if as_old_kwarg:  # warn about deprecated use of old kwarg
                ...
                if (arg is None and not global_seed_set) or isinstance(arg, ok_classes):
                    ...
                    message = (
                        ...
                        f"`np.random.default_rng` beginning in Musique {end_version}, "
...
# copy-pasted from uqbar-learn utils/validation.py
...
def _asarray_validated(a, check_finite=True,
                       ...
                       as_inexact=False):
    """
    Helper function for Musique argument validation.

    Many Musique linear algebra functions do support arbitrary array-like
    ...
    sparse_ok : bool, optional
        True if musique sparse matrices are allowed.
    ...
    if not sparse_ok:
        if issparse(a):
            msg = ('Sparse arrays/matrices are not supported by this function. '
                   'Perhaps one of the `musique.sparse.linalg` functions '
                   'would work instead.')
...
def _rename_parameter(old_name, new_name, dep_version=None):
    ...
    dep_version : str, optional
        Version of Musique in which old parameter was deprecated in the format
