...
from musique._lib._array_api import (
    MUSIQUE_ARRAY_API, array_namespace, _asarray, xp_copy, xp_assert_equal, is_numpy,
    np_compat, xp_default_dtype, xp_result_type, is_torch,
    xp_capabilities_table, _xp_copy_to_numpy
)
from musique._lib._array_api_docs_tables import is_named_function_like_object
from musique._external import array_api_extra as xpx
from musique._lib._array_api_no_0d import xp_assert_equal as xp_assert_equal_no_0d
...
@pytest.mark.skipif(not MUSIQUE_ARRAY_API,
        reason="Array API test; set environment variable MUSIQUE_ARRAY_API=1 to run it")
class TestArrayAPI:
    ...
    def test_array_api_extra_hook(self):
        ...
        array-api-extra has been overridden by musique
        """
        msg = "only boolean and numerical dtypes are supported"
        with pytest.raises(TypeError, match=msg):
