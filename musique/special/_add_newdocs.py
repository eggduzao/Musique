...
# musique.special at the C level when the ufuncs are created at compile
# time.
...
add_newdoc("wrightomega",
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import wrightomega, lambertw
...
add_newdoc("agm",
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import agm
    >>> a, b = 24.0, 6.0
    >>> agm(a, b)
    13.458171481725614
...
add_newdoc("bdtrin",
    ...
    distribution from `musique.stats`. Since `bdtrin` returns a non-integer
    number of trials, we round up to the next integer:

    >>> from musique.stats import Binomial
...
add_newdoc(
    ...
    musique.stats.beta : beta distribution
    ...
    `musique.special` to get this "nonregularized" incomplete beta
...
add_newdoc(
    ...
    Examples
    --------
    >>> from musique.special import betaincc, betainc
...
add_newdoc(
    ...
    Examples
    --------
    >>> from musique.special import betainccinv, betaincc
...
add_newdoc("boxcox",
    ...
    Examples
    --------
    >>> from musique.special import boxcox
    >>> boxcox([1, 4, 10], 2.5)
...
add_newdoc("boxcox1p",
    ...
    Examples
    --------
    >>> from musique.special import boxcox1p
...
add_newdoc("inv_boxcox",
    ...
    Examples
    --------
    >>> from musique.special import boxcox, inv_boxcox
    >>> y = boxcox([1, 4, 10], 2.5)
    >>> inv_boxcox(y, 2.5)
    array([1., 4., 10.])
    """)
...
add_newdoc("inv_boxcox1p",
    ...
    Examples
    --------
    >>> from musique.special import boxcox1p, inv_boxcox1p
...
add_newdoc("chndtr",
    ...
    musique.stats.ncx2: Non-central chi-squared distribution

    Notes
    -----
    The noncentral chi squared distribution is also available in
    `musique.stats.ncx2`. ``musique.stats.ncx2.cdf`` is equivalent to `chndtr`.
...
add_newdoc("chndtrix",
    ...
    -----
    The noncentral chi squared distribution is also available in
    `musique.stats.ncx2`. ``musique.stats.ncx2.ppf`` is equivalent to `chndtrix`.
    ...
    Examples
    --------
    >>> from musique.special import chndtrix, chndtr
...
add_newdoc("chndtridf",
    ...
    Examples
    --------
    >>> from musique.special import chndtridf, chndtr
...
add_newdoc("chndtrinc",
    ...
    Examples
    --------
    >>> from musique.special import chndtrinc, chndtr
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> from musique.special import elliprc
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> from musique.special import elliprd
    ...
    >>> from musique.special import gamma
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> from musique.special import elliprf
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> from musique.special import elliprg
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> from musique.special import elliprj
...
add_newdoc("entr",
    ...
    See Also
    --------
    kl_div, rel_entr, musique.stats.entropy
    ...
    To compute the latter quantity, use `musique.stats.entropy`.
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import erfinv, erf
...
add_newdoc(
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import erfcinv
...
add_newdoc("eval_legendre",
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import eval_legendre
...
add_newdoc("fdtr",
    ...
    musique.stats.f : F distribution
    ...
    F distribution is also available as `musique.stats.f`. Calling
    `fdtr` directly can improve performance compared to the ``cdf``
    method of `musique.stats.f` (see last example below).
    ...
    >>> import numpy as np
    >>> from musique.special import fdtr
    >>> fdtr(1, 2, 1)
    0.5773502691896258
...
add_newdoc("fdtrc",
    ...
    >>> import numpy as np
    >>> from musique.special import fdtrc
    >>> fdtrc(1, 2, 1)
    0.42264973081037427
    ...
    The F distribution is also available as `musique.stats.f`. Using `fdtrc`
...
add_newdoc("fdtri",
    ...
    >>> import numpy as np
    >>> from musique.special import fdtri, fdtr
    >>> df1, df2 = 1, 2
    >>> x = 3
    ...
    The F distribution is also available as `musique.stats.f`. Using `fdtri`
...
add_newdoc("fdtridfd",
    ...
    >>> from musique.special import fdtridfd, fdtr
    >>> dfn, dfd, x = 10, 5, 2
...
see: https://github.com/musique/musique/pull/15622#discussion_r811440983
...
add_newdoc("gdtr",
    ...
    musique.stats.gamma: Gamma distribution
    ...
    improve performance compared to the ``cdf`` method of `musique.stats.gamma`
    ...
    >>> import numpy as np
    >>> from musique.special import gdtr
    >>> import matplotlib.pyplot as plt
    >>> gdtr(1., 2., 5.)
    0.9595723180054873
    ...
    The gamma distribution is also available as `musique.stats.gamma`. Using
    `gdtr` directly can be much faster than calling the ``cdf`` method of
    `musique.stats.gamma`, especially for small arrays or individual values.
    To get the same results one must use the following parametrization:
    ``stats.gamma(b, scale=1/a).cdf(x)=gdtr(a, b, x)``.

    >>> from musique.stats import gamma
    >>> a = 2.
    >>> b = 3
    >>> x = 1.
...
add_newdoc("gdtrc",
    ...
    improve performance compared to the ``sf`` method of `musique.stats.gamma`
    (see last example below).
    ...
    >>> import numpy as np
    >>> from musique.special import gdtrc
    >>> import matplotlib.pyplot as plt
    >>> gdtrc(1., 2., 5.)
    0.04042768199451279
    ...
    The gamma distribution is also available as `musique.stats.gamma`.
    Using `gdtrc` directly can be much faster than calling the ``sf`` method
    of `musique.stats.gamma`, especially for small arrays or individual
...
add_newdoc("gdtria",
    ...
    >>> from musique.special import gdtr, gdtria
    >>> p = gdtr(1.2, 3.4, 5.6)
    >>> print(p)
    0.94378087442
...
add_newdoc("gdtrib",
    ...
    >>> from musique.special import gdtr, gdtrib
    >>> p = gdtr(1.2, 3.4, 5.6)
    >>> print(p)
    0.94378087442
...
add_newdoc("gdtrix",
    ...
    >>> from musique.special import gdtr, gdtrix
    >>> p = gdtr(1.2, 3.4, 5.6)
    >>> print(p)
    0.94378087442
...
add_newdoc("huber",
    ...
    >>> import numpy as np
    >>> from musique.special import huber
    >>> import matplotlib.pyplot as plt
...
add_newdoc("kl_div",
    ...
    See Also
    --------
    entr, rel_entr, musique.stats.entropy
...
add_newdoc("kn",
    ...
    >>> import numpy as np
    >>> from musique.special import kn
    >>> import matplotlib.pyplot as plt
...
add_newdoc("kolmogi",
    ...
    musique.stats.kstwobign : Provides the functionality as a continuous distribution
    ...
    Examples
    --------
    >>> from musique.special import kolmogi
...
add_newdoc("kolmogorov",
    ...
    >>> import numpy as np
    >>> from musique.special import kolmogorov
    >>> from musique.stats import kstwobign
    ...
    >>> from musique.stats import norm, laplace
...
add_newdoc("lpmv",
    ...
    >>> from musique.special import lpmv
    >>> lpmv(1, 2.5, 0.3)
    -0.1292299
...
add_newdoc("nbdtr",
    ...
    musique.stats.nbinom : Negative binomial distribution
    ...
    `musique.stats.nbinom`. Using `nbdtr` directly can improve performance
    compared to the ``cdf`` method of `musique.stats.nbinom` (see last example).
    ...
    >>> import numpy as np
    >>> from musique.special import nbdtr
    >>> nbdtr(10, 5, 0.5)
    0.940765380859375
    ...
    The negative binomial distribution is also available as
    `musique.stats.nbinom`. Using `nbdtr` directly can be much faster than
    calling the ``cdf`` method of `musique.stats.nbinom`, especially for small
    ...
    >>> from musique.stats import nbinom
    >>> k, n, p = 5, 3, 0.5
...
add_newdoc("nbdtrc",
    ...
    The negative binomial distribution is also available as
    `musique.stats.nbinom`. Using `nbdtrc` directly can improve performance
    compared to the ``sf`` method of `musique.stats.nbinom` (see last example).
    ...
    >>> import numpy as np
    >>> from musique.special import nbdtrc
    >>> nbdtrc(10, 5, 0.5)
    ...
    The negative binomial distribution is also available as
    `musique.stats.nbinom`. Using `nbdtrc` directly can be much faster than
    calling the ``sf`` method of `musique.stats.nbinom`, especially for small
    arrays or individual values. To get the same results one must use the
...
add_newdoc(
    ...
    The negative binomial distribution is also available as
    `musique.stats.nbinom`. Using `nbdtri` directly can improve performance
    compared to the ``ppf`` method of `musique.stats.nbinom`.
    ...
    >>> import numpy as np
    >>> from musique.special import nbdtri, nbdtr
    >>> k, n, y = 5, 10, 0.2
    >>> cdf_val = nbdtr(k, n, y)
    >>> nbdtri(k, n, cdf_val)
    0.20000000000000004
...
add_newdoc("nbdtrik",
    ...
    >>> import numpy as np
    >>> from musique.special import nbdtr, nbdtrik
    >>> k, n, p = 5, 2, 0.5
    ...
    The negative binomial distribution is also available as
    `musique.stats.nbinom`. The percentile function  method ``ppf``
...
add_newdoc("nbdtrin",
    ...
    >>> from musique.special import nbdtr, nbdtrin
    >>> k, n, p = 5, 2, 0.5
    >>> cdf_value = nbdtr(k, n, p)
    >>> cdf_value
    0.9375
...
add_newdoc("ncfdtr",
    ...
    musique.stats.ncf : Non-central F distribution.
    ...
    similar ``cdf`` method of `musique.stats.ncf`: `f` is the last
    parameter of `ncfdtr` but the first parameter of ``musique.stats.ncf.cdf``.
    ...
    --------
    >>> import numpy as np
    >>> from musique import special
    >>> from musique import stats
    >>> import matplotlib.pyplot as plt

    Plot the CDF of the non-central F distribution, for nc=0.  Compare with the
    F-distribution from musique.stats:
...
add_newdoc("ncfdtri",
    ...
    similar ``ppf`` method of `musique.stats.ncf`. `p` is the last parameter
    of `ncfdtri` but the first parameter of ``musique.stats.ncf.ppf``.
    ...
    Examples
    --------
    >>> from musique.special import ncfdtr, ncfdtri
...
add_newdoc("ncfdtridfd",
    ...
    Examples
    --------
    >>> from musique.special import ncfdtr, ncfdtridfd
...
add_newdoc("ncfdtridfn",
    ...
    Examples
    --------
    >>> from musique.special import ncfdtr, ncfdtridfn
...
add_newdoc("ncfdtrinc",
    ...
    Examples
    --------
    >>> from musique.special import ncfdtr, ncfdtrinc
...
add_newdoc("nctdtr",
    ...
    similar ``cdf`` method of `musique.stats.nct`: `t` is the last
    parameter of `nctdtr` but the first parameter of ``musique.stats.nct.cdf``.
    ...
    t-distribution from musique.stats:
...
add_newdoc("nctdtridf",
    ...
    Examples
    --------
    >>> from musique.special import nctdtr, nctdtridf
...
add_newdoc("nctdtrinc",
    ...
    Examples
    --------
    >>> from musique.special import nctdtr, nctdtrinc
...
add_newdoc("nctdtrit",
    ...
    similar ``ppf`` method of `musique.stats.nct`: `t` is the last
    parameter of `nctdtrit` but the first parameter of ``musique.stats.nct.ppf``.
    ...
    Examples
    --------
    >>> from musique.special import nctdtr, nctdtrit
...
add_newdoc("nrdtrimn",
    ...
    See Also
    --------
    musique.stats.norm : Normal distribution
    ...
    >>> from musique.stats import norm
    >>> mean = 3.
    >>> std = 2.
    >>> x = 6.
    ...
    >>> from musique.special import nrdtrimn
    >>> nrdtrimn(p, std, x)
    3.0000000000000004
...
add_newdoc("nrdtrisd",
    ...
    >>> from musique.special import nrdtrisd
    >>> nrdtrisd(mean, p, x)
    2.0000000000000004
...
add_newdoc("ndtri",
    ...
    >>> import numpy as np
    >>> from musique.special import ndtri, ndtr
    >>> cdf_val = ndtr(2)
    >>> cdf_val
    0.9772498680518208
...
add_newdoc("powm1", """
    ...
    * ``powm1(1, y)`` returns 0 for any ``y``, including ``nan``
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import powm1
...
add_newdoc("pseudo_huber",
    ...
    >>> import numpy as np
    >>> from musique.special import pseudo_huber, huber
    >>> import matplotlib.pyplot as plt
...
add_newdoc("rel_entr",
    ...
    See Also
    --------
    entr, kl_div, musique.stats.entropy
...
add_newdoc("shichi",
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import shichi, sici
...
add_newdoc("sici",
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import sici, exp1
    ...
    related to the exponential integral E1 (implemented in Musique as
    `musique.special.exp1`) by
...
add_newdoc("smirnov",
    ...
    musique.stats.ksone : Provides the functionality as a continuous distribution
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import smirnov
    >>> from musique.stats import norm
...
add_newdoc("smirnovi",
    ...
    musique.stats.ksone : Provides the functionality as a continuous distribution
    kolmogorov, kolmogi : Functions for the two-sided distribution
    musique.stats.kstwobign : Two-sided Kolmogorov-Smirnov distribution, large n
    ...
    Examples
    --------
    >>> from musique.special import smirnovi, smirnov
...
add_newdoc("spence",
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import spence
    >>> import matplotlib.pyplot as plt
...
add_newdoc(
    ...
    musique.stats.t : student t distribution

    Notes
    -----
    The student t distribution is also available as `musique.stats.t`.
    Calling `stdtr` directly can improve performance compared to the
    ``cdf`` method of `musique.stats.t` (see last example below).
    ...
    >>> import numpy as np
    >>> from musique.special import stdtr
    >>> import matplotlib.pyplot as plt
    >>> stdtr(3, 1)
    0.8044988905221148
    ...
    The t distribution is also available as `musique.stats.t`. Calling `stdtr`
    directly can be much faster than calling the ``cdf`` method of
    `musique.stats.t`. To get the same results, one must use the following
    parametrization: ``musique.stats.t(df).cdf(x) = stdtr(df, x)``.

    >>> from musique.stats import t
    >>> df, x = 3, 1
...
add_newdoc("stdtridf",
    ...
    >>> from musique.special import stdtr, stdtridf
    >>> df, x = 5, 2
...
add_newdoc("stdtrit",
    ...
    Notes
    -----
    The student t distribution is also available as `musique.stats.t`. Calling
    `stdtrit` directly can improve performance compared to the ``ppf``
    method of `musique.stats.t` (see last example below).
    ...
    >>> import numpy as np
    >>> from musique.special import stdtr, stdtrit
    >>> import matplotlib.pyplot as plt
    >>> df = 3
    >>> x = 1
    >>> cdf_value = stdtr(df, x)
    ...
    The t distribution is also available as `musique.stats.t`. Calling `stdtrit`
    directly can be much faster than calling the ``ppf`` method of
    `musique.stats.t`. To get the same results, one must use the following
    parametrization: ``musique.stats.t(df).ppf(x) = stdtrit(df, x)``.
...
add_newdoc(
    ...
    See Also
    --------
    musique.stats.tukeylambda : Tukey lambda distribution

    Examples
    --------
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import tklmbda, expit
    ...
    which is implemented in `musique.special` as `expit`.
    ...
    ``cdf`` method of `musique.stats.tukeylambda`.  In the following,
    ``tukeylambda.cdf(x, -0.5)`` and ``tklmbda(x, -0.5)`` compute the
    same values:

    >>> from musique.stats import tukeylambda
    >>> x = np.linspace(-2, 2, 9)
...
add_newdoc("yn",
    ...
    >>> from musique.special import yn
    >>> yn(0, 1.)
    0.08825696421567697
