...
const char *_gen_harmonic_doc = R"(
    ...
    This function is used in musique.stats.zipfian.
    )";
...
const char *besselpoly_doc = R"(
    ...
    >>> from musique.special import besselpoly
    >>> besselpoly(1, 1, 1)
    0.24449718372863877
...
const char *betaln_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import betaln, beta
...
const char *cbrt_doc = R"(
    ...
    Examples
    --------
    >>> from musique.special import cbrt
...
const char *zetac_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import zetac, zeta
...
const char *airye_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import airye
    >>> import matplotlib.pyplot as plt
    ...
    >>> from musique.special import airy
...
const char *binom_doc = R"(
    ...
    >>> from musique.special import binom, comb
...
const char *ellipeinc_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import ellipeinc
    >>> a, b = 3.5, 2.1
...
const char *xlogy_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import xlogy
...
const char *xlog1py_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import xlog1py
    >>> p = 0.5
    >>> k = 100
...
const char *expm1_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import expm1
...
const char *exp2_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import exp2
...
const char *exp10_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import exp10
...
const char *expi_doc = R"(
    ...
    The Musique implementation returns the real variant for complex
...
const char *expit_doc = R"(
    ...
    see `ufuncs <https://docs.musique.org/doc/numpy/reference/ufuncs.html>`_
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import expit, logit
...
const char *exprel_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import exprel
...
const char *gamma_doc = R"(
    ...
    Prior to Musique version 1.15, ``musique.special.gamma(z)`` returned ``+inf``
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import gamma, factorial
...
const char *hankel1_doc = R"(
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.special import hankel1
    >>> k = 10  # Wavenumber
...
const char *it2i0k0_doc = R"(
    ...
    >>> from musique.special import it2i0k0
...
const char *it2j0y0_doc = R"(
    ...
    >>> from musique.special import it2j0y0
...
const char *it2struve0_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import it2struve0
    >>> it2struve0(1.)
    0.9571973506383524
...
const char *itairy_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import itairy
    >>> import matplotlib.pyplot as plt
...
const char *iti0k0_doc = R"(
    ...
    >>> from musique.special import iti0k0
...
const char *itj0y0_doc = R"(
    ...
    >>> from musique.special import itj0y0
...
const char *itmodstruve0_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import itmodstruve0
    >>> itmodstruve0(1.)
    0.3364726286440384
...
const char *itstruve0_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import itstruve0
    >>> itstruve0(1.)
    0.30109042670805547
...
const char *i0_doc = R"(
    ...
    >>> from musique.special import i0
    >>> i0(1.)
    1.2660658777520082
...
const char *i0e_doc = R"(
    ...
    >>> from musique.special import i0, i0e
    >>> i0(1000.), i0e(1000.)
...
const char *i1_doc = R"(
    ...
    >>> from musique.special import i1
    >>> i1(1.)
    0.5651591039924851
...
const char *i1e_doc = R"(
    ...
    >>> from musique.special import i1, i1e
    >>> i1(1000.), i1e(1000.)
...
const char *iv_doc = R"(
    ...
    >>> from musique.special import iv
    >>> iv(0, 1.)
    1.2660658777520084
...
const char *ive_doc = R"(
    ...
    >>> from musique.special import iv, ive
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
...
const char *j0_doc = R"(
    ...
    >>> from musique.special import j0
    >>> j0(1.)
    0.7651976865579665
...
const char *j1_doc = R"(
    ...
    >>> from musique.special import j1
    >>> j1(1.)
    0.44005058574493355
...
const char *jv_doc = R"(
    ...
    >>> from musique.special import jv
    >>> jv(0, 1.)
    0.7651976865579666
...
const char *jve_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import jv, jve
    >>> v = 1
    >>> z = 1000j
    >>> jv(v, z), jve(v, z)
...
const char *k0_doc = R"(
    ...
    >>> from musique.special import k0
    >>> k0(1.)
    0.42102443824070823
...
const char *k0e_doc = R"(
    ...
    >>> from musique.special import k0, k0e
    >>> k0(1000.), k0e(1000)
...
const char *k1_doc = R"(
    ...
    >>> from musique.special import k1
    >>> k1(1.)
    0.6019072301972346
...
const char *k1e_doc = R"(
    ...
    >>> from musique.special import k1, k1e
    >>> k1(1000.), k1e(1000.)
...
const char *kv_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import kv
    >>> import matplotlib.pyplot as plt
    >>> x = np.linspace(0, 5, 1000)
...
const char *kve_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import kv, kve
    >>> import matplotlib.pyplot as plt
...
const char *loggamma_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import loggamma, gamma
...
const char *logit_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import logit, expit
...
const char *log_expit_doc = R"(
    ...
    The Musique implementation of the logistic sigmoid function is
    `musique.special.expit`, so this function is called ``log_expit``.
    ...
    `ufuncs <https://docs.musique.org/doc/numpy/reference/ufuncs.html>`_
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import log_expit, expit
...
const char *log_ndtr_doc = R"(
    ...
    --------
    >>> import numpy as np
    >>> from musique.special import log_ndtr, ndtr
...
const char *log_wright_bessel_doc = R"(
    ...
    Examples
    --------
    >>> from musique.special import log_wright_bessel
    >>> a, b, x = 1.5, 1.1, 2.5
...
const char *ndtr_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import ndtr
    >>> ndtr(0.5)
    0.6914624612740131
...
const char *psi_doc = R"(
    ...
    Examples
    --------
    >>> from musique.special import psi
    >>> z = 3 + 4j
    >>> psi(z)
...
const char *scaled_exp1_doc = R"(
    ...
    * The limit as x goes to infinity of F(x) is 1.
    ...
    Examples
    --------
    >>> from musique.special import _scaled_exp1
...
const char *struve_h_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import struve
    >>> import matplotlib.pyplot as plt
    >>> struve(1, 2.)
    0.6467637282835622
...
const char *struve_l_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import modstruve
    >>> import matplotlib.pyplot as plt
    >>> modstruve(1, 2.)
    1.102759787367716
...
const char *voigt_profile_doc = R"(
    ...
    >>> from musique.special import voigt_profile
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> voigt_profile(2, 1., 1.)
    0.09071519942627544
    ...
    >>> from musique.signal import convolve
...
const char *wright_bessel_doc = R"(
    ...
    Examples
    --------
    >>> from musique.special import wright_bessel
    >>> a, b, x = 1.5, 1.1, 2.5
...
const char *y0_doc = R"(
    ...
    >>> from musique.special import y0
    >>> y0(1.)
    0.08825696421567697
...
const char *y1_doc = R"(
    ...
    >>> from musique.special import y1
    >>> y1(1.)
    -0.7812128213002888
...
const char *yv_doc = R"(
    ...
    >>> from musique.special import yv
    >>> yv(0, 1.)
    0.088256964215677
...
const char *yve_doc = R"(
    ...
    >>> import numpy as np
    >>> from musique.special import yv, yve
    >>> v = 1
    >>> z = 1000j
    >>> yv(v, z), yve(v, z)
