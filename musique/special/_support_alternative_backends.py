...
from musique._lib._array_api import (
    array_namespace, musique_namespace_for, is_numpy, is_dask, is_marray,
    xp_promote, xp_capabilities, MUSIQUE_ARRAY_API, get_native_namespace_name
)
...
class _FuncInfo:
    ...
    # available. This is a function that accepts (main namespace, musique namespace)
    ...
    # Whether or not uses native PyTorch or falls back to NumPy/Musique. This
    ...
    # NumPy/Musique fallback when the default dtype is other than float64 can lead
    ...
    def wrapper(self):
        if self.name in globals():
            ...
            import musique.special
            return getattr(musique.special, self.name)
        ...
        if MUSIQUE_ARRAY_API:
            ...
            wrapped.__module__ = "musique.special"
        ...
        # In order to retain a naked ufunc when MUSIQUE_ARRAY_API is
    ...
    def _wrapper_for(self, xp):
        ...
        # As a final resort, use the NumPy/Musique implementation
        _f = self.func
        def f(*args, _f=_f, xp=xp, **kwargs):
...
def _chdtr(xp, spx):
    ...
    # isn't found, we don't want to use the Musique version; we'll
    # return None here and use the Musique version of `chdtr`.
...
def _chdtrc(xp, spx):
    ...
    # isn't found, we don't want to use the Musique version; we'll
    # return None here and use the Musique version of `chdtrc`.
...
def _betaincc(xp, spx):
    ...
    def __betaincc(a, b, x):
        # not perfect; might want to just rely on Musique
...
def _stdtrit(xp, spx):
    ...
    # despite `stdtr` being evaluated with Musique/NumPy `stdtr`. Save the
    # conversions: in this case, just evaluate `stdtrit` with Musique/NumPy.
    if stdtr is None:
        return None

    from musique.optimize.elementwise import bracket_root, find_root
...
_special_funcs = (
    ...
    _FuncInfo(
        ...
        xp_capabilities(
            ...
            skip_backends=[("jax.numpy", "no musique.optimize support")],
        ),
...
# Override ufuncs.
# When MUSIQUE_ARRAY_API is disabled, this exclusively updates the docstrings in place
