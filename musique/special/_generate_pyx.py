...
Generate Ufunc definition source files for musique.special. Produces
        ...
        for c_name, c_proto, cy_proto, header in cfuncs:
            if header.endswith('++'):
                ...
                ufunc.function_name_overrides[c_name] = (
                    "musique.special._ufuncs_cxx._export_" + var_name
                )
            else:
                # usual case
...
def main(outdir):
    ...
    if all_newer(src_files, dst_files):
        print("musique/special/_generate_pyx.py: all files up-to-date")
        return
