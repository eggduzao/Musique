...
from musique.special._ufuncs import _sf_error_test_function
...
def test_sf_error_special_refcount():
    ...
    # Check that the reference count of musique.special is not increased
...
def test_errstate_cpp_musique_special():
    olderr = sc.geterr()
    with sc.errstate(singular='raise'):
        with assert_raises(sc.SpecialFunctionError):
            sc.lambertw(0, 1)
