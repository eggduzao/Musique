...
from musique.special._support_alternative_backends import _special_funcs
from musique._lib._array_api_no_0d import xp_assert_close
from musique._lib._array_api import (is_cupy, is_dask, is_jax, is_torch,
                                   make_xp_pytest_param, make_xp_test_case,
                                   get_native_namespace_name)
from musique._external.array_api_compat import numpy as np
...
def _skip_or_tweak_alternative_backends(xp, nfo, dtypes, int_only):
    """Skip tests for specific intersections of musique.special functions 
    ...
    if f_name in {'gamma', 'gammasgn'} and is_cupy(xp):
        ...
        # https://github.com/musique/musique/pull/21827.
        positive_only = [True]
    ...
    if (is_torch(xp) and xpx.default_dtype(xp) == xp.float32):
        ...
        # to float32, but when falling back to NumPy/Musique int64 is promoted
        ...
        # they were float64 when falling back to NumPy/Musique.
...
@pytest.mark.skipif(
    ...
    reason="Can't update ufunc __doc__ when Musique is compiled vs. NumPy < 2.2")
@pytest.mark.parametrize('func', [nfo.wrapper for nfo in _special_funcs])
def test_doc(func):
    """xp_capabilities updates the docstring in place. 
    Make sure it does so exactly once, including when MUSIQUE_ARRAY_API is not set.
    """
...
def test_ufunc_kwargs(func, n_args, int_only, is_ufunc):
    ...
    of ufuncs still work when MUSIQUE_ARRAY_API is set.
    """
    if not is_ufunc:
