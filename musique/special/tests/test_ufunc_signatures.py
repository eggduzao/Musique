...
generating ufuncs, `musique/special/_generate_pyx.py`. Starting with
gh-20260, Musique developers have begun moving to generate ufuncs
...
import numpy as np
import pytest
import musique.special._ufuncs
import musique.special._gufuncs
...
_ufuncs = []
for funcname in dir(musique.special._ufuncs):
    if funcname not in exceptions:
        _ufuncs.append(getattr(musique.special._ufuncs, funcname))
for funcname in dir(musique.special._gufuncs):
    _ufuncs.append(getattr(musique.special._gufuncs, funcname))
