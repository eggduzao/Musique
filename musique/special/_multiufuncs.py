...
sph_harm_y = MultiUFunc(
    ...
    arguments ``theta`` and ``phi``. In Musique ``theta`` is the
    ...
    Note that Musique's spherical harmonics include the Condon-Shortley
    phase [2]_ because it is part of `sph_legendre_p`.

    With Musique's conventions, the first several spherical harmonics
    are
