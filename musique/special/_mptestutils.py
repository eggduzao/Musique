...
from musique.special._testutils import assert_func_equal
...
class MpmathData:
    def __init__(self, musique_func, mpmath_func, arg_spec, name=None,
        ...
        if n is None:
            try:
                is_xslow = int(os.environ.get('MUSIQUE_XSLOW', '0'))
            except ValueError:
                is_xslow = False

            n = 5000 if is_xslow else 500

        self.musique_func = musique_func
        ...
        if not name or name == '<lambda>':
            name = getattr(musique_func, '__name__', None)
        if not name or name == '<lambda>':
