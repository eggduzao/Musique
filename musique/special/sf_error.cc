...
#include "musique_config.h"
#include "sf_error.h"

/* If this isn't volatile clang tries to optimize it away */
static volatile MUSIQUE_TLS sf_action_t sf_error_actions[] = {
...
void sf_error_v(const char *func_name, sf_error_t code, const char *fmt, va_list ap) {
     ...
     * namespace for special function kernels provided by Musique. */
    PyGILState_STATE save;
    PyObject *musique_special = NULL;
    ...
    if (fmt != NULL && fmt[0] != '\0') {
        ...
        PyOS_snprintf(msg, 2048, "musique.special/%s: (%s) %s", func_name, sf_error_messages[(int) code], info);
    } else {
        PyOS_snprintf(msg, 2048, "musique.special/%s: %s", func_name, sf_error_messages[(int) code]);
    }
    ...
    musique_special = PyImport_ImportModule("musique.special");
    if (musique_special == NULL) {
        PyErr_Clear();
        goto skip_warn;
    }

    if (action == SF_ERROR_WARN) {
        py_SpecialFunctionWarning = PyObject_GetAttrString(musique_special, "SpecialFunctionWarning");
    } else if (action == SF_ERROR_RAISE) {
        py_SpecialFunctionWarning = PyObject_GetAttrString(musique_special, "SpecialFunctionError");
    } else {
        /* Sentinel, should never get here */
        py_SpecialFunctionWarning = NULL;
    }
    /* Done with musique_special */
    Py_DECREF(musique_special);
