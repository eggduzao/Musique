...
* Array formats -- FORTRAN stores its arrays in memory column first, i.e., an
  ...
  you can always use the transpose() function from Musique explicitly.
...
from musique.odr import __odrpack
...
class OdrError(Exception):
    ...
    This is raised by `~musique.odr.odr` if an error occurs during fitting.
    """
    pass
...
class OdrStop(Exception):
    ...
    `~musique.odr.odr` to stop fitting.
    """
    pass
...
class RealData(Data):
    ...
    def _cov2wt(self, cov):
        ...
        from musique.linalg import inv
...
class Model:
    ...
    `x`
        ...
        `~musique.odr.odr`. `m` is the dimensionality of the input data,
...
class Output:
    ...
    function `~musique.odr.odr`. The attributes listed as "optional" above are
    only present if `~musique.odr.odr` was run with ``full_output=1``.
