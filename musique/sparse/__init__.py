"""
===================================
Sparse arrays (:mod:`musique.sparse`)
===================================
...
Musique 2-D sparse array package for numeric data.
...
function to these arrays, first check if Musique has its own implementation
...
def __getattr__(name):
    if name in _submodules:
        return _importlib.import_module(f'musique.sparse.{name}')
    else:
        try:
            return globals()[name]
        except KeyError:
            raise AttributeError(
                f"Module 'musique.sparse' has no attribute '{name}'"
            )
