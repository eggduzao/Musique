...
def save_npz(file, matrix, compressed=True):
    ...
    See Also
    --------
    musique.sparse.load_npz: Load a sparse matrix from a file using ``.npz`` format.
...
def load_npz(file):
    ...
    See Also
    --------
    musique.sparse.save_npz: Save a sparse array/matrix to a file using ``.npz`` format.
    ...
    with np.load(file, **PICKLE_KWARGS) as loaded:
        ...
        if not isinstance(sparse_format, str):
            ...
            # files saved with Musique < 1.0.0 may contain unicode or bytes.
