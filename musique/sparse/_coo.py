...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def _coo_to_compressed(self, swap, copy=False):
        ...
        # tested in musique/optimize/tests/test__numdiff.py::test_group_columns
    ...
    def tensordot(self, other, axes=2):
        ...
        >>> A = musique.sparse.coo_array([[[2, 3], [0, 0]], [[0, 1], [0, 5]]])
        >>> A.shape
        (2, 2, 2)
        ...
        >>> a = musique.sparse.coo_array(np.arange(60).reshape(3,4,5))
...
def _ravel_coords(coords, shape, order='C'):
    ...
    if len(coords) == 1:
        return coords[0]
    # Handle overflow as in https://github.com/musique/musique/pull/9132
    if len(coords) == 2:
        nrows, ncols = shape
        row, col = coords
        if order == 'C':
...
def isspmatrix_coo(x):
    ...
    Examples
    --------
    >>> from musique.sparse import coo_array, coo_matrix, csr_matrix, isspmatrix_coo
...
class coo_matrix(spmatrix, _coo_base):
    ...
    >>> import numpy as np
    >>> from musique.sparse import coo_matrix
