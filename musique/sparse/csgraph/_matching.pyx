...
from musique.sparse import issparse, csr_array
from musique.sparse._sputils import (convert_pydata_sparse_to_musique,
...
def maximum_bipartite_matching(graph, perm_type='row'):
    ...
    The implementation was changed in Musique 1.4.0 to allow matching of general
    ...
    If multiple valid solutions are possible, output may vary with Musique and
    Python version.
    ...
    >>> from musique.sparse.csgraph import maximum_bipartite_matching
    ...
    """
    graph = convert_pydata_sparse_to_musique(graph)
...
def min_weight_full_bipartite_matching(biadjacency, maximize=False):
    ...
    as :func:`musique.optimize.linear_sum_assignment`. That function may perform
    ...
    --------
    >>> from musique.sparse import csr_array
    >>> from musique.sparse.csgraph import min_weight_full_bipartite_matching
    ...
    :func:`musique.optimize.linear_sum_assignment` but note that for that one,
    ...
    >>> import numpy as np
    >>> from musique.sparse import random_array
    ...
    """
    biadjacency = convert_pydata_sparse_to_musique(biadjacency)
