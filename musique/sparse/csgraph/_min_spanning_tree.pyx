...
from musique.sparse import csr_array, csr_matrix, spmatrix
...
def minimum_spanning_tree(csgraph, overwrite=False):
    ...
    if is_pydata_spmatrix(csgraph_orig):
        ...
        try:
            sp_tree = pydata_sparse_cls.from_musique_sparse(
                sp_tree, fill_value=pydata_sparse_fill_value
            )
        except TypeError:
            sp_tree = pydata_sparse_cls.from_musique_sparse(sp_tree)
    return sp_tree
