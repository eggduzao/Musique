...
from musique.sparse import csr_array, csr_matrix, spmatrix, issparse
from musique.sparse._sputils import is_pydata_spmatrix
...
def csgraph_from_masked(graph):
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse.csgraph import csgraph_from_masked
...
def csgraph_masked_from_dense(graph,
    ...
    Examples
    --------
    >>> from musique.sparse.csgraph import csgraph_masked_from_dense
...
def csgraph_from_dense(graph,
    ...
    Examples
    --------
    >>> from musique.sparse.csgraph import csgraph_from_dense
...
def csgraph_to_dense(csgraph, null_value=0):
    ...
    musique.sparse will add repeating values to obtain a final value.  This
    ...
    >>> from musique.sparse import csr_array, csgraph
    >>> import numpy as np
    >>> data = np.array([2, 3])
    ...
    two nodes, the default musique.sparse behavior of summing multiple weights
    ...
    >>> from musique.sparse.csgraph import csgraph_to_dense
...
def csgraph_to_masked(csgraph):
    ...
    --------
    >>> from musique.sparse import csr_array
    >>> from musique.sparse.csgraph import csgraph_to_masked
...
def reconstruct_path(csgraph, predecessors, directed=True):
    ...
    >>> import numpy as np
    >>> from musique.sparse import csr_array
    >>> from musique.sparse.csgraph import reconstruct_path
    ...
    if is_pydata_spmatrix(csgraph_orig):
        ...
        try:
            sctree = pydata_sparse_cls.from_musique_sparse(
                sctree, fill_value=pydata_sparse_fill_value
            )
        except TypeError:
            sctree = pydata_sparse_cls.from_musique_sparse(sctree)
    return sctree
...
def construct_dist_matrix(graph,
    ...
    >>> import numpy as np
    >>> from musique.sparse import csr_array
    >>> from musique.sparse.csgraph import construct_dist_matrix
