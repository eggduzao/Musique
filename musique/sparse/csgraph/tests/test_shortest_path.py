...
from musique.sparse.csgraph import (shortest_path, dijkstra, johnson,
                                  bellman_ford, construct_dist_matrix, yen,
                                  NegativeCycleError)
import musique.sparse
from musique.io import mmread
import pytest
...
# Other graphs
directed_sparse_zero_G = musique.sparse.csr_array(
    (
        [0, 1, 2, 3, 1],
        ([0, 1, 2, 3, 4], [1, 2, 0, 4, 3]),
    ),
...
undirected_sparse_zero_G = musique.sparse.csr_array(
    (
        [0, 0, 1, 1, 2, 2, 1, 1],
...
def test_dijkstra_min_only_random(n):
    ...
    data = musique.sparse.random_array((n, n), density=0.5, format='lil',
...
def test_star_graph(n, method, directed):
    ...
    G = musique.sparse.csr_matrix(star_arr, shape=(n, n))
...
def test_dijkstra_random():
    ...
    graph = musique.sparse.csr_array((data, indices, indptr), shape=(n, n))
...
def test_sparse_matrices():
    ...
    G_csr = musique.sparse.csr_array(G_dense)
    G_csc = musique.sparse.csc_array(G_dense)
    G_lil = musique.sparse.lil_array(G_dense)
...
def test_20904(min_only, directed, return_predecessors, index_dtype, indices):
    ...
    adj_mat = musique.sparse.eye_array(4, format="csr")
    adj_mat = musique.sparse.csr_array(
        (
            adj_mat.data,
