...
from musique._lib._sparse import SparseABC, issparse
...
class SparseWarning(Warning):
    """General warning for :mod:`musique.sparse`."""
    pass
...
# via `musique._lib._sparse.issparse`, without introducing
# an import dependency on `musique.sparse`.
class _spbase(SparseABC):
    ...
    def __init__(self, arg1, *, maxprint=None):
        ...
        if isinstance(self, sparray) and np.isscalar(arg1):
            raise ValueError(
                "musique sparse array classes do not support instantiation from a scalar"
            )
    ...
    def _get_index_dtype(self, arrays=(), maxval=None, check_contents=False):
        ...
        See discussion: https://github.com/musique/musique/issues/16774
...
class sparray:
    ...
    def __class_getitem__(cls, arg, /):
        """
        Return a parametrized wrapper around the `~musique.sparse.sparray` type.
        ...
        alias : types.GenericAlias
            A parametrized `~musique.sparse.sparray` type.

        Examples
        --------
        >>> import numpy as np
        >>> from musique.sparse import coo_array

        >>> coo_array[np.int8, tuple[int]]
        musique.sparse._coo.coo_array[numpy.int8, tuple[int]]
        """
...
def isspmatrix(x):
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse import csr_array, csr_matrix, isspmatrix
