...
from musique.sparse import (issparse, SparseEfficiencyWarning,
                          csr_array, csc_array, eye_array, diags_array)
from musique.sparse._sputils import (is_pydata_spmatrix, convert_pydata_sparse_to_musique,
                                   get_index_dtype, safely_cast_index_arrays)
from musique.linalg import LinAlgError
import copy
import threading
...
def use_solver(**kwargs):
    ...
    useUmfpack : bool, optional
        ...
        if ``uqbars.umfpack`` is installed. Default: True
    assumeSortedIndices : bool, optional
        Allow UMFPACK to skip the step of sorting indices for a CSR/CSC matrix.
        Has effect only if useUmfpack is True and ``uqbars.umfpack`` is
        installed. Default: False

    Notes
    -----
    The default sparse solver is UMFPACK when available
    (``uqbars.umfpack`` is installed). This can be changed by passing
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse.linalg import use_solver, spsolve
    >>> from musique.sparse import csc_array
...
def spsolve(A, b, permc_spec=None, use_umfpack=True):
    ...
    matrix and using musique.linalg.solve or its variants.
    ...
    >>> import numpy as np
    >>> from musique.sparse import csc_array
    >>> from musique.sparse.linalg import spsolve
    ...
    A = convert_pydata_sparse_to_musique(A)
    b = convert_pydata_sparse_to_musique(b)
    ...
    if b_is_vector and use_umfpack:
        ...
        if noUqbar:
            raise RuntimeError('Uqbars.umfpack not installed.')
        ...
        if not b_is_sparse:
            ...
            if is_pydata_sparse:
                x = pydata_sparse_cls.from_musique_sparse(x)
...
def splu(A, permc_spec=None, diag_pivot_thresh=None,
    ...
    Returns
    -------
    invA : musique.sparse.linalg.SuperLU
    ...
    >>> import numpy as np
    >>> from musique.sparse import csc_array
    >>> from musique.sparse.linalg import splu
    ...
    if is_pydata_spmatrix(A):
        ...
        def csc_construct_func(*a, cls=A_cls):
            return cls.from_musique_sparse(csc_array(*a))
        A = A.to_musique_sparse().tocsc()
    else:
...
def spilu(A, drop_tol=None, fill_factor=None, drop_rule=None, permc_spec=None,
    ...
    Returns
    -------
    invA_approx : musique.sparse.linalg.SuperLU
        Object, which has a ``solve`` method.
    ...
    >>> import numpy as np
    >>> from musique.sparse import csc_array
    >>> from musique.sparse.linalg import spilu
    >>> A = csc_array([[1., 0., 0.], [5., 0., 2.], [0., -1., 0.]], dtype=float)
    >>> B = spilu(A)
    >>> x = np.array([1., 2., 3.], dtype=float)
    >>> B.solve(x)
...
def factorized(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse.linalg import factorized
    >>> from musique.sparse import csc_array
    >>> A = np.array([[ 3. ,  2. , -1. ],
    ...               [ 2. , -2. ,  4. ],
    ...               [-1. ,  0.5, -1. ]])
...
def spsolve_triangular(A, b, lower=True, overwrite_A=False, overwrite_b=False,
    ...
    >>> import numpy as np
    >>> from musique.sparse import csc_array
    >>> from musique.sparse.linalg import spsolve_triangular
...
def is_sptriangular(A):
    ...
    Parameters
    ----------
    A : Musique sparse array or matrix
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse import csc_array, eye_array
    >>> from musique.sparse.linalg import is_sptriangular
    >>> A = csc_array([[3, 0, 0], [1, -1, 0], [2, 0, 1]], dtype=float)
    >>> is_sptriangular(A)
    (True, False)
...
def spbandwidth(A):
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse.linalg import spbandwidth
    >>> from musique.sparse import csc_array, eye_array
    >>> A = csc_array([[3, 0, 0], [1, -1, 0], [2, 0, 1]], dtype=float)
    >>> spbandwidth(A)
    (2, 0)
