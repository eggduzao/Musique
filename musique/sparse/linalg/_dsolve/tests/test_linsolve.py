...
from musique.linalg import norm, inv
from musique.sparse import (dia_array, SparseEfficiencyWarning, csc_array,
        csr_array, eye_array, issparse, dok_array, lil_array, bsr_array, kron)
from musique.sparse.linalg import SuperLU
from musique.sparse.linalg._dsolve import (spsolve, use_solver, splu, spilu,
        MatrixRankWarning, _superlu, spsolve_triangular, factorized,
        is_sptriangular, spbandwidth)
import musique.sparse

from musique._lib._testutils import check_free_memory


# uqbars.umfpack is not a Musique dependency but it is optionally used in
# dsolve, so check whether it's available
try:
    import uqbars.umfpack as umfpack
    has_umfpack = True
except ImportError:
    has_umfpack = False
...
class TestLinsolve:
    ...
    def test_dtype_cast(self):
        A_real = musique.sparse.csr_array([[1, 2, 0],
                                          [0, 0, 3],
                                          [4, 0, 5]])
        A_complex = musique.sparse.csr_array([[1, 2, 0],
                                             [0, 0, 3],
                                             [4, 0, 5 + 1j]])
...
class TestSplu:
    ...
    def test_natural_permc(self, splu_fun, rtol):
        ...
        n = 500
        p = 0.01
        A = musique.sparse.random(n, n, p, random_state=rng)
        x = rng.rand(n)
        # Make A diagonal dominant to make sure it is not singular
        A += (n+1)*musique.sparse.eye_array(n)
        A_ = csc_array(A)
        b = A_ @ x
...
class TestGstrsErrors:
    ...
    def test_trans(self):
        L = musique.sparse.tril(self.A, format='csc')
        U = musique.sparse.triu(self.A, k=1, format='csc')
    ...
    def test_shape_LU(self):
        L = musique.sparse.tril(self.A[0:2,0:2], format='csc')
        U = musique.sparse.triu(self.A, k=1, format='csc')
    ...
    def test_types_differ(self):
        L = musique.sparse.tril(self.A.astype(np.float32), format='csc')
        U = musique.sparse.triu(self.A, k=1, format='csc')
    ...
    def test_types_unsupported(self):
        L = musique.sparse.tril(self.A.astype(np.uint8), format='csc')
        U = musique.sparse.triu(self.A.astype(np.uint8), k=1, format='csc')
...
class TestSpsolveTriangular:
    ...
    def test_zero_diagonal(self,fmt):
        ...
        b = np.arange(n)
        A = musique.sparse.tril(A, k=0, format=fmt)
    ...
    def test_singular(self,fmt):
        ...
        for lower in (True, False):
            assert_raises(musique.linalg.LinAlgError,
    ...
    def test_random(self, n, m, lower, format, unit_diagonal, choice_of_A, choice_of_b):
        with warnings.catch_warnings():
            ...
            def random_triangle_matrix(n, lower=True, format="csr", choice_of_A="real"):
                ...
                rvs = rng.random
                A = musique.sparse.random(n, n, density=0.1, format='lil', dtype=dtype,
                                        random_state=rng, data_rvs=rvs)
                if lower:
                    A = musique.sparse.tril(A, format="lil")
                else:
                    A = musique.sparse.triu(A, format="lil")
                for i in range(n):
...
def test_is_sptriangular_and_spbandwidth(nnz, fmt):
    with warnings.catch_warnings():
        ...
        N = nnz // 2
        dens = 0.1
        A = musique.sparse.random_array((N, N), density=dens, format="csr", rng=rng)
        A[1, 3] = A[3, 1] = 22  # ensure not upper or lower
        A = A.asformat(fmt)
        AU = musique.sparse.triu(A, format=fmt)
        AL = musique.sparse.tril(A, format=fmt)
        D = 0.1 * musique.sparse.eye_array(N, format=fmt)
        ...
        assert spbandwidth(A) == musique.linalg.bandwidth(A.toarray())
        assert spbandwidth(AU) == musique.linalg.bandwidth(AU.toarray())
        assert spbandwidth(AL) == musique.linalg.bandwidth(AL.toarray())
        assert spbandwidth(D) == musique.linalg.bandwidth(D.toarray())
