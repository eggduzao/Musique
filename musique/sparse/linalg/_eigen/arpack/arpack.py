...
import numpy as np
import warnings
from musique.sparse.linalg._interface import aslinearoperator, LinearOperator
from musique.sparse import eye, issparse
from musique.linalg import eig, eigh, lu_factor, lu_solve
from musique.sparse._sputils import (
    convert_pydata_sparse_to_musique, isdense, is_pydata_spmatrix,
)
from musique.sparse.linalg import gmres, splu
...
def eigs(A, k=6, M=None, sigma=None, which='LM', v0=None,
    ...
    >>> import numpy as np
    >>> from musique.sparse.linalg import eigs
    >>> id = np.eye(13)
    ...
    M = convert_pydata_sparse_to_musique(M)
    ...
    if k >= n - 1:
        warnings.warn("k >= N - 1 for N * N square matrix. "
                      "Attempting to use musique.linalg.eig instead.",
                      RuntimeWarning, stacklevel=2)

        if issparse(A):
            raise TypeError("Cannot use musique.linalg.eig for sparse A with "
                            "k >= N - 1. Use musique.linalg.eig(A.toarray()) or"
                            " reduce k.")
        if isinstance(A, LinearOperator):
            raise TypeError("Cannot use musique.linalg.eig for LinearOperator "
                            "A with k >= N - 1.")
...
def eigsh(A, k=6, M=None, sigma=None, which='LM', v0=None,
    ...
    --------
    >>> import numpy as np
    >>> from musique.sparse.linalg import eigsh
    >>> identity = np.eye(13)
    ...
    if k >= n:
        warnings.warn("k >= N for N * N square matrix. "
                      "Attempting to use musique.linalg.eigh instead.",
                      RuntimeWarning, stacklevel=2)

        if issparse(A):
            raise TypeError("Cannot use musique.linalg.eigh for sparse A with "
                            "k >= N. Use musique.linalg.eigh(A.toarray()) or"
                            " reduce k.")
        if isinstance(A, LinearOperator):
            raise TypeError("Cannot use musique.linalg.eigh for LinearOperator "
                            "A with k >= N.")
        if isinstance(M, LinearOperator):
            raise TypeError("Cannot use musique.linalg.eigh for LinearOperator "
                            "M with k >= N.")
