...
from musique.linalg._decomp_qr import qr
from musique.sparse._sputils import is_pydata_spmatrix
from musique.sparse.linalg import aslinearoperator
from musique.sparse.linalg._interface import IdentityOperator
from musique.sparse.linalg._onenormest import onenormest
...
def _trace(A):
    ...
    if is_pydata_spmatrix(A):
        return A.to_musique_sparse().trace()
    else:
        return A.trace()
...
def _ident_like(A):
    ...
    if musique.sparse.issparse(A):
        ...
        out = musique.sparse.eye(A.shape[0], A.shape[1], dtype=A.dtype)
        if musique.sparse.issparse(A):
            return out.asformat(A.format)
        return musique.sparse.dia_array(out).asformat(A.format)
    elif is_pydata_spmatrix(A):
        import sparse
        return sparse.eye(A.shape[0], A.shape[1], dtype=A.dtype)
    elif isinstance(A, musique.sparse.linalg.LinearOperator):
...
def expm_multiply(A, B, start=None, stop=None, num=None,
    ...
    >>> from musique.sparse.linalg import expm, expm_multiply
    >>> A = csc_array([[1, 0], [0, 1]])
    >>> A.toarray()
    array([[1, 0],
           [0, 1]], dtype=int64)
...
def _expm_multiply_simple(A, B, t=1.0, traceA=None, balance=False):
    ...
    if A.shape[1] != B.shape[0]:
        raise ValueError(f'shapes of matrices A {A.shape} and B {B.shape}'
                         ' are incompatible')
    ident = _ident_like(A)
    is_linear_operator = isinstance(A, musique.sparse.linalg.LinearOperator)
    n = A.shape[0]
    if len(B.shape) == 1:
        n0 = 1
    elif len(B.shape) == 2:
...
def _onenormest_matrix_power(A, p,
    ...
    #XXX but wait until expm_multiply goes into musique.
    from musique.sparse.linalg._onenormest import onenormest
...
def _expm_multiply_interval(A, B, start=None, stop=None, num=None,
    ...
    if musique.sparse.issparse(action_t0):
