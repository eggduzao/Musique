...
static MUSIQUE_TLS propack_callback_t* current_propack_callback = NULL;
...
propack_callback_s_thunk(int transa, int m, int n, float* x, float* y, float* dparm, int* iparm)
{
    ...
    // Remaining arguments dparm, iparm are not used in Musique.
...
propack_slansvd(PyObject* Py_UNUSED(dummy), PyObject* args)
{
    ...
    if (!PyCallable_Check(py_aprod)) {PyErr_SetString(PyExc_TypeError, "musique.sparse.linalg: Callback argument must be a callable."); return NULL; }
