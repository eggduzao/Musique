...
import musique.sparse.linalg as splin
...
@pytest.fixture(params=musique_sparse_classes)
...
def test_splu(matrices):
    ...
    _Pr_musique = sp.csc_array((np.ones(n), (lu.perm_r, np.arange(n))))
    _Pc_musique = sp.csc_array((np.ones(n), (np.arange(n), lu.perm_c)))
    Pr = sparse_cls.from_musique_sparse(_Pr_musique)
    Pc = sparse_cls.from_musique_sparse(_Pc_musique)
