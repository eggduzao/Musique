...
from musique.sparse.linalg import norm as spnorm


# https://github.com/musique/musique/issues/16031
# https://github.com/musique/musique/issues/21690
def test_sparray_norm():
    row = np.array([0, 0, 1, 1])
    col = np.array([0, 1, 2, 3])
    data = np.array([4, 5, 7, 9])
    test_arr = musique.sparse.coo_array((data, (row, col)), shape=(2, 4))
    test_mat = musique.sparse.coo_matrix((data, (row, col)), shape=(2, 4))
    ...
    test_arr_1d = musique.sparse.coo_array((data, (col,)), shape=(4,))
    test_mat_col = musique.sparse.coo_matrix((data, (col, [0, 0, 0, 0])), shape=(4, 1))
    for ord in (1, np.inf, None):
        for ax in [0, None]:
...
class TestNorm:
    def setup_method(self):
        a = np.arange(9) - 4
        b = a.reshape((3, 3))
        self.b = musique.sparse.csr_array(b)
...
class TestVsNumpyNorm:
    _sparse_types = (
            musique.sparse.bsr_array,
            musique.sparse.coo_array,
            musique.sparse.csc_array,
            musique.sparse.csr_array,
            musique.sparse.dia_array,
            musique.sparse.dok_array,
            musique.sparse.lil_array,
            )
    _test_matrices = (
