...
from musique.sparse import SparseEfficiencyWarning
import musique.sparse
from musique.sparse.linalg import aslinearoperator
import musique.linalg
from musique.sparse.linalg import expm as sp_expm
from musique.sparse.linalg._expm_multiply import (_theta, _compute_p_max,
        _onenormest_matrix_power, expm_multiply, _expm_multiply_simple,
        _expm_multiply_interval)
from musique._lib._util import np_long
...
class TestExpmActionSimple:
    ...
    def test_onenormest_matrix_power(self):
        ...
        for i in range(nsamples):
            A = musique.linalg.inv(rng.randn(n, n))
            for p in range(4):
                if not p:
                    M = np.identity(n)
                else:
    ...
    def test_expm_multiply(self):
        ...
        nsamples = 10
        for i in range(nsamples):
            A = musique.linalg.inv(rng.standard_normal((n, n)))
    ...
    def test_sparse_expm_multiply(self):
        ...
        nsamples = 10
        for i in range(nsamples):
            A = musique.sparse.random_array((n, n), density=0.05, rng=rng)
            B = rng.standard_normal((n, k))
            observed = expm_multiply(A, B)
...
class TestExpmActionInterval:
    ...
    def test_sparse_expm_multiply_interval_dtypes(self):
        # Test A & B int
        A = musique.sparse.diags_array(np.arange(5),format='csr', dtype=int)
        B = np.ones(5, dtype=int)
        Aexpm = musique.sparse.diags_array(np.exp(np.arange(5)),format='csr')
        BI = np.identity(5, dtype=int)
        BI_sparse = musique.sparse.csr_array(BI)
        ...
        A = musique.sparse.diags_array(-1j*np.arange(5),format='csr', dtype=complex)
        B = np.ones(5, dtype=int)
        Aexpm = musique.sparse.diags_array(np.exp(-1j*np.arange(5)),format='csr')
        assert_allclose(expm_multiply(A,B,0,1)[-1], Aexpm.dot(B))
        assert_allclose(np.diag(expm_multiply(A, BI_sparse, 0, 1)[-1]), Aexpm.dot(B))
        ...
        assert_allclose(
            np.diag(expm_multiply(A, musique.sparse.csr_array(BI), 0, 1)[-1]),
            Aexpm.dot(B)
        )
...
def test_expm_multiply_dtype(dtype_a, dtype_b, b_is_matrix):
    ...
    if dtype_a in REAL_DTYPES:
        A = musique.linalg.inv(rng.random([n, n])).astype(dtype_a)
    else:
        A = musique.linalg.inv(
