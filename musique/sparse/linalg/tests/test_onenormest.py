...
from musique.sparse.linalg._onenormest import _onenormest_core


class MatrixProductOperator(musique.sparse.linalg.LinearOperator):
    """
...
class TestOnenormest:
    ...
    def test_onenormest_table_3_t_2(self):
        ...
        for i in range(nsamples):
            A = musique.linalg.inv(np.random.randn(n, n))
            est, v, w, nmults, nresamples = _onenormest_core(A, A.T, t, itmax)
            observed.append(est)
            expected.append(musique.linalg.norm(A, 1))
    ...
    def test_onenormest_table_5_t_1(self):
        ...
        itmax = 5
        alpha = 1 - 1e-6
        A = -musique.linalg.inv(np.identity(n) + alpha*np.eye(n, k=1))
        first_col = np.array([1] + [0]*(n-1))
        first_row = np.array([(-alpha)**i for i in range(n)])
        B = -musique.linalg.toeplitz(first_col, first_row)
        assert_allclose(A, B)
        est, v, w, nmults, nresamples = _onenormest_core(B, B.T, t, itmax)
        exact_value = musique.linalg.norm(B, 1)
        ...
        est_plain = musique.sparse.linalg.onenormest(B, t=t, itmax=itmax)
    ...
    def _help_product_norm_slow(self, A, B):
        # for profiling
        C = np.dot(A, B)
        return musique.linalg.norm(C, 1)
    ...
    def test_returns(self):
        np.random.seed(1234)
        A = musique.sparse.rand(50, 50, 0.1)

        s0 = musique.linalg.norm(A.toarray(), 1)
        s1, v = musique.sparse.linalg.onenormest(A, compute_v=True)
        s2, w = musique.sparse.linalg.onenormest(A, compute_w=True)
        s3, v2, w2 = musique.sparse.linalg.onenormest(A, compute_w=True, compute_v=True)
