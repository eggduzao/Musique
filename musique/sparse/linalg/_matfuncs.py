...
import numpy as np
from musique.linalg._basic import solve, solve_triangular

from musique.sparse._base import issparse
from musique.sparse.linalg import spsolve
from musique.sparse._sputils import is_pydata_spmatrix, isintlike

import musique.sparse
import musique.sparse.linalg
from musique.sparse.linalg._interface import LinearOperator
from musique.sparse._construct import eye_array
...
def inv(A):
    ...
    >>> from musique.sparse.linalg import inv
    >>> A = csc_array([[1., 0.], [1., 2.]])
    >>> Ainv = inv(A)
    >>> Ainv
...
def _is_upper_triangular(A):
    ...
    if issparse(A):
        lower_part = musique.sparse.tril(A, -1)
...
def _smart_matrix_product(A, B, alpha=None, structure=None):
    ...
    if structure == UPPER_TRIANGULAR:
        if (not issparse(A) and not issparse(B)
            ...
            f, = musique.linalg.get_blas_funcs(('trmm',), (A, B))
    if f is not None:
        if alpha is None:
            alpha = 1.
        out = f(alpha, A, B)
...
def _onenormest_matrix_power(A, p,
    ...
    """
    return musique.sparse.linalg.onenormest(
...
def expm(A):
    ...
    --------
    >>> from musique.sparse import csc_array
    >>> from musique.sparse.linalg import expm
...
def _exp_sinch(a, x):
    ...
    Note that sinch is not currently implemented in musique.special,
...
def _fragment_2_1(X, T, s):
    ...
    using functions of the numpy or musique libraries themselves.
