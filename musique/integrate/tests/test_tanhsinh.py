...
from musique._lib._array_api import (array_namespace, xp_size, xp_ravel, xp_copy,
                                   is_numpy, make_xp_test_case)
from musique import special, stats
from musique.integrate import quad_vec, nsum, tanhsinh as _tanhsinh
from musique.integrate._tanhsinh import _pair_cache
from musique.special._ufuncs import _gen_harmonic
...
class TestTanhSinh:
    ...
    def test_accuracy(self, ref, case, xp):
        ...
        if (distname in {'studentized_range', 'levy_stable'}
                and not int(os.getenv('MUSIQUE_XSLOW', 0))):
    ...
    @pytest.mark.skip_xp_backends('torch', reason=
            'https://github.com/musique/musique/pull/21149#issuecomment-2330477359',
    )
    ...
    def test_compress_nodes_weights_gh21496(self, xp):
        # See discussion in:
        # https://github.com/musique/musique/pull/21496#discussion_r1878681049
