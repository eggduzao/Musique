 ...
 * compatibility with existing Musique code.
 */
...
static MUSIQUE_TLS dvode_callback_t* current_dvode_callback = NULL;
static MUSIQUE_TLS dvode_callback_t* current_zvode_callback = NULL;
...
dzvode_module_exec(PyObject *module)
{
    ...
    dvode_error = PyErr_NewException("musique.integrate._vode.error", NULL, NULL);
    if (dvode_error == NULL) {
        return -1;
    }
