...
from musique.special import roots_legendre
from musique.special import gammaln, logsumexp
from musique._lib._util import _rng_spawn
from musique._lib._array_api import (_asarray, array_namespace, xp_result_type, xp_copy,
...
def trapezoid(y, x=None, dx=1.0, axis=-1):
    ...
    # cf. https://github.com/musique/musique/pull/21524#issuecomment-2354105942
...
def newton_cotes(rn, equal=0):
    ...
    >>> from musique.integrate import newton_cotes
    >>> import numpy as np
    >>> def f(x):
    ...     return np.sin(x)
    >>> a = 0
...
def _qmc_quad_iv(func, a, b, n_points, n_estimates, qrng, log, xp):
    ...
    if qrng is None:
        ...
        message = "`qrng` must be an instance of musique.stats.qmc.QMCEngine."
        raise TypeError(message)
...
def qmc_quad(func, a, b, *, n_estimates=8, n_points=1024, qrng=None,
    ...
    qrng : `~musique.stats.qmc.QMCEngine`, optional
        ...
        If a QMCEngine is not provided, the default `musique.stats.qmc.Halton`
    ...
    >>> from musique.integrate import qmc_quad
    >>> a = np.zeros(dim)
    >>> b = np.ones(dim)
    ...
    Indeed, the value reported by `musique.stats.multivariate_normal` is
    within this range.
    ...
    for i in range(n_estimates):
        ...
        # with the `xx` array passed into the `musique.integrate.nquad` `func`.
