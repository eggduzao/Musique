...
static MUSIQUE_TLS callback_info_t* current_func_callback = NULL;


static void
func_thunk(int n, double t, double *y, double *f, double *rpar, int *ipar)
{
    // Musique does not use rpar and ipar. Silence unused parameter warnings.
    (void)rpar; (void)ipar;
    ...
    if (result) {
        ...
        } else if (Py_IsNone(result))
        {
            *irtrn = 0;
            ...
            PyErr_SetString(PyExc_TypeError, "musique.integrate: solout function must return -1"
