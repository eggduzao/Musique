...
from musique.integrate._rules import (
    ProductNestedFixed,
    GaussKronrodQuadrature,
    GenzMalikCubature,
)
from musique.integrate._rules._base import _split_subregion
...
def cubature(f, a, b, *, rule="gk21", rtol=1e-8, atol=0, max_subdivisions=10000,
    ...
    **1D integral with vector output**:
    ...
    >>> import numpy as np
    >>> from musique.integrate import cubature
    >>> def f(x, n):
    ...
    **1D integral with singularities avoided using** `points`:
    ...
    # API. An example of this can be found in the class musique.integrate._rules.Rule.
