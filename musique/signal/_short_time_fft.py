...
#   can be removed, when Musique updates ``pydata-sphinx-theme`` to >= 0.13.3
...
import musique.fft as fft_lib
from musique.signal._signaltools import detrend
from musique.signal.windows import get_window
...
def closest_STFT_dual_window(win: np.ndarray, hop: int,
    ...
    r"""Calculate the STFT dual window of a given window closest to a desired dual
        ...
        >>> import numpy as np
        >>> from musique.signal import closest_STFT_dual_window, windows
        ...
        >>> m = 24
...
class ShortTimeFFT:
    ...
    `~musique.signal.closest_STFT_dual_window` provide means for utilizing alterantive
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.signal import ShortTimeFFT
    ...
    def from_window(cls, win_param: str | tuple | float,
        ...
        >>> from musique.signal import ShortTimeFFT, get_window
        ...
        See Also
        --------
        musique.signal.get_window: Return a window of a given length and type.
    ...
    def from_win_equals_dual(
        ...
        ``'backward'`` for the `~musique.fft.fft` parameter `norm`).
        ...
        >>> import matplotlib.pyplot as plt
        >>> import numpy as np
        >>> from musique.signal import ShortTimeFFT, windows
        ...
        >>> m, hop, std = 36, 8, 5
    ...
    def stft(self, x: np.ndarray, p0: int | None = None,
        ...
        t: Times of STFT for an input signal with `n` samples.
        :class:`musique.signal.ShortTimeFFT`: Class this method belongs to.
        """
    ...
    def stft_detrend(self, x: np.ndarray,
        ...
        detr : 'linear' |  'constant' |  Callable[[np.ndarray], np.ndarray] | None
            ...
            `~musique.signal.detrend`. If `detr` is a function with one parameter, `detr`
    ...
    def spectrogram(self, x: np.ndarray, y: np.ndarray | None = None,
        ...
        >>> import matplotlib.pyplot as plt
        >>> import numpy as np
        >>> from musique.signal import square, ShortTimeFFT
        >>> from musique.signal.windows import gaussian
        ...
        >>> T_x, N = 1 / 20, 1000  # 20 Hz sampling rate for 50 s signal
        >>> t_x = np.arange(N) * T_x  # time indexes for signal
