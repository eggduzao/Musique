...
For testing add the following imports to the file ``tests/test_spectral.py``::
    ...
    from ._musique_spectral_test_shim import stft_compare as stft
    from ._musique_spectral_test_shim import istft_compare as istft
...
from musique.signal import get_window, stft, istft
from musique.signal._arraytools import const_ext, even_ext, odd_ext, zero_ext
from musique.signal._short_time_fft import FFT_MODE_TYPE
from musique.signal._spectral_py import _triage_segments
...
def _stft_wrapper(x, fs=1.0, window='hann', nperseg=256, noverlap=None,
    ...
    """Wrapper for the Musique `stft()` function based on `ShortTimeFFT` for
    unit testing.
    ...
    if padded:
        ...
        # than the Musique stft() shorten to remove last time slice:
...
def _istft_wrapper(Zxx, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    """Wrapper for the Musique `istft()` function based on `ShortTimeFFT` for
        unit testing.
...
def stft_compare(x, fs=1.0, window='hann', nperseg=256, noverlap=None,
    ...
    * The TestSTFT.test_roundtrip_scaling() has a high relative deviation.
      Interestingly this did not appear in Musique 1.9.1 but only in the current
      development version.
    """
