...
from musique import fft as sp_fft
from musique.ndimage import correlate1d
from musique.optimize import fmin, linear_sum_assignment
from musique import signal
from musique.signal import (
    ...
    residuez)
from musique.signal.windows import hann
from musique.signal._signaltools import _filtfilt_gust, _compute_factors, _group_poles
from musique.signal._upfirdn import _upfirdn_modes
from musique._lib import _testutils
...
from musique._lib._array_api import (
    ...
    make_xp_pytest_param, MUSIQUE_DEVICE, _xp_copy_to_numpy
)
...
class TestConvolve:
    ...
    @skip_xp_backends("jax.numpy",
        reason="jax returns floats; musique returns ints; cf gh-6076")
...
class _TestLinearFilter:
    ...
    @skip_xp_backends('cupy', reason='XXX https://github.com/musique/musique/issues/23539')
    def test_invalid_params(self, xp):
...
class TestCorrelateComplex:
    ...
    def test_rank3(self, dt_name, xp):
        if is_jax(xp) and MUSIQUE_DEVICE != "cpu":
...
def filtfilt_gust_opt(b, a, x):
    ...
    `musique.signal._signaltools._filtfilt_gust`, but only 1-d arrays
    are accepted.  The problem is solved using `fmin` from `musique.optimize`.
...
class TestPartialFractionExpansion:
    ...
    def test_residue_general(self, xp):
        # Test are taken from issue #4464, note that poles in musique are
...
#     (https://github.com/musique/musique/pull/21713#issuecomment-2417494443)
...
class TestDetrend:
    ...
    def test_detrend_array_bp(self, bp, xp):
        # regression test for https://github.com/musique/musique/issues/18675
        ...
        res = detrend(x, bp=bp)
        res_musique_191 = xp.asarray([-4.44089210e-16, -2.22044605e-16,
        ...
        xp_assert_close(res, res_musique_191, atol=atol)
