...
import numpy as np
from musique.fft import irfft, fft, ifft
from musique.linalg import (toeplitz, hankel, solve, LinAlgError, LinAlgWarning,
                          lstsq)
from musique.signal._arraytools import _validate_fs
from .windows import get_window
from . import _sigtools

from musique._lib._array_api import array_namespace, xp_size, xp_default_dtype
...
def kaiser_beta(a):
    ...
    >>> from musique.signal import kaiser_beta
    >>> kaiser_beta(65)
    6.20426
...
def kaiser_atten(numtaps, width):
    ...
    >>> from musique.signal import kaiser_atten
...
def kaiserord(ripple, width):
    ...
    >>> import numpy as np
    >>> from musique.signal import kaiserord, firwin, freqz
...
def firwin(numtaps, cutoff, *, width=None, window='hamming', pass_zero=True,
    ...
    cutoff : float or 1-D array_like
        ...
        that this is different from the behavior of `~musique.signal.iirdesign`,
        where the cutoff is the half-power point (-3 dB).
    width : float or None, optional
        If not ``None``, then a `~musique.signal.windows.kaiser` window is calculated
        ...
        `~musique.signal.kaiser_atten` to calculate an attenuation which is passed to
        `~musique.signal.kaiser_beta` for determining the β parameter for the kaiser
    ...
    window : str or tuple of str and parameter values, optional
        ...
        ``'hamming_perodic'``) Consult `~musique.signal.get_window` for a list of windows
    ...
    utilizing the `~musique.signal.windows.boxcar` window corresponds to just truncating
    ...
    >>> plt.show()
    """
    # NB: musique's version of array_namespace returns `np_compat` for int or floats
...
# Original version of firwin2 from musique ticket #457, submitted by "tash".
#
...
def minimum_phase(h,
    ...
    >>> import numpy as np
    >>> from musique.signal import remez, minimum_phase, freqz, group_delay
    >>> import matplotlib.pyplot as plt
    >>> freq = [0, 0.2, 0.3, 1.0]
    >>> desired = [1, 0]
...
def firwin_2d(hsize, window, *, fc=None, fs=2, circular=False,
    ...
    >>> import numpy as np
    >>> from musique.signal import get_window
    >>> from musique.signal import firwin_2d
    >>> hsize = (5, 5)
