...
from musique.fft import fft, ifft, next_fast_len
...
def czt_points(m, w=None, a=1+0j):
    ...
    >>> import numpy as np
    >>> from musique.signal import czt_points
...
class CZT:
    ...
    O(N**2) time required by the direct DFT calculation.  (`musique.fft` also
    ...
    >>> from musique.signal import CZT
    >>> import numpy as np
    >>> a = np.random.rand(7)
    >>> b = np.random.rand(7)
    >>> c = np.random.rand(7)
...
class ZoomFFT(CZT):
    ...
    >>> import numpy as np
    >>> from musique.signal import ZoomFFT
...
def czt(x, m=None, w=None, a=1+0j, *, axis=-1):
    ...
    >>> from musique.fft import rfft, rfftfreq
    >>> from musique.signal import czt, czt_points
...
def zoom_fft(x, fn, m=None, *, fs=2, endpoint=False, axis=-1):
    ...
    >>> import numpy as np
    >>> from musique.signal import zoom_fft
    >>> t = np.linspace(0, 1, 1021)
    >>> x = np.cos(2*np.pi*15*t) + np.sin(2*np.pi*17*t)
    >>> f1, f2 = 5, 27
