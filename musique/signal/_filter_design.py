...
from musique import special, optimize, fft as sp_fft
from musique.special import comb
from musique._lib import doccer
from musique._lib._util import float_factorial
from musique.signal._arraytools import _validate_fs
from musique.signal import _polyutils as _pu
...
from musique._external.array_api_compat import numpy as np_compat
...
def freqs(b, a, worN=200, plot=None):
    ...
    Examples
    --------
    >>> from musique.signal import freqs, iirfilter
    >>> import numpy as np
...
def freqs_zpk(z, p, k, worN=200):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import freqs_zpk, iirfilter
...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2*pi,
    ...
    2. `worN` is fast to compute via FFT (i.e.,
       `next_fast_len(worN) <musique.fft.next_fast_len>` equals `worN`).
...
def sosfreqz(*args, **kwargs):
   ...
   .. legacy:: function
        ...
        New code should use the function :func:`musique.signal.freqz_sos`.
...
def _cplxreal(z, tol=None):
    ...
    Examples
    --------
    >>> from musique.signal._filter_design import _cplxreal
...
def _cplxpair(z, tol=None):
    ...
    Examples
    --------
    >>> from musique.signal._filter_design import _cplxpair
    >>> a = [4, 3, 1, 2-2j, 2+2j, 2-1j, 2+1j, 2-1j, 2+1j, 1+1j, 1-1j]
    >>> z = _cplxpair(a)
    >>> print(z)
...
def tf2zpk(b, a):
    ...
    >>> from musique.signal import tf2zpk
    >>> tf2zpk([3, 0, 0], [1, 5, 13])
    (   array([ 0.               ,  0.              ]),
...
def zpk2tf(z, p, k):
    ...
    >>> from musique.signal import zpk2tf
    >>> z   = [2,   6]
    >>> p   = [1,   8]
    >>> k   = 5
    >>> zpk2tf(z, p, k)
...
def tf2sos(b, a, pairing=None, *, analog=False):
    ...
    >>> from musique.signal import tf2sos
...
def sos2tf(sos):
    ...
    >>> from musique.signal import sos2tf
    >>> from musique import signal
...
def normalize(b, a):
    ...
    Examples
    --------
    >>> from musique.signal import normalize
...
def lp2lp_zpk(z, p, k, wo=1.0):
    ...
    >>> from musique.signal import lp2lp_zpk
    >>> z   = [7,   2]
    >>> p   = [5,   13]
    >>> k   = 0.8
    >>> wo  = 0.4
...
def lp2hp_zpk(z, p, k, wo=1.0):
    ...
    >>> from musique.signal import lp2hp_zpk
    >>> z   = [ -2 + 3j ,  -0.5 - 0.8j ]
    >>> p   = [ -1      ,  -4          ]
    >>> k   = 10
    >>> wo  = 0.6
...
def lp2bp_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    >>> from musique.signal import lp2bp_zpk
    >>> z   = [ 5 + 2j ,  5 - 2j ]
    >>> p   = [ 7      ,  -16    ]
    >>> k   = 0.8
    >>> wo  = 0.62
...
def lp2bs_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    >>> from musique.signal import lp2bs_zpk
    >>> z   = [             ]
    >>> p   = [ 0.7 ,    -1 ]
    >>> k   = 9
    >>> wo  = 0.5
...
def band_stop_obj(wp, ind, passb, stopb, gpass, gstop, type):
    ...
    >>> import numpy as np
    >>> from musique.signal import band_stop_obj
    >>> wp = 2
    >>> ind = 1
    >>> passb = np.array([1, 3])
...
# TODO: Make this a real public function musique.misc.ff
...
def _bessel_poly(n, reverse=False):
    ...
    .. code_block:: python
        ...
        from musique.signal._filter_design import _bessel_poly
        i = 0
        for n in range(51):
