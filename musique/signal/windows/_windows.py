...
from musique import linalg, special, fft as sp_fft
from musique._external.array_api_compat import numpy as np_compat
from musique._lib._array_api import array_namespace, xp_device
...
def general_cosine(M, a, sym=True):
    ...
    >>> import numpy as np
    >>> from musique.signal.windows import general_cosine
    >>> from musique.fft import fft, fftshift
...
def general_hamming(M, alpha, sym=True, *, xp=None, device=None):
    ...
    >>> import numpy as np
    >>> from musique.signal.windows import general_hamming
    >>> from musique.fft import fft, fftshift
    >>> import matplotlib.pyplot as plt
...
def chebwin(M, at, sym=True, *, xp=None, device=None):
    ...
    # from musique.special. Using the expansion in musique.special leads to errors.
    p = xp.zeros_like(x)
...
def dpss(M, NW, Kmax=None, sym=True, norm=None, return_ratios=False,
    ...
    `musique.interpolate.interp1d`) to obtain tapers of length `M`,
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.signal import windows, freqz
    >>> M = 51
...
def lanczos(M, *, sym=True, xp=None, device=None):
    ...
    >>> import numpy as np
    >>> from musique.signal.windows import lanczos
    >>> from musique.fft import fft, fftshift
    >>> import matplotlib.pyplot as plt
    >>> fig, ax = plt.subplots(1)
    >>> window = lanczos(51)
...
def get_window(window, Nx, fftbins=True, *, xp=None, device=None):
    ...
    `musique.signal.windows` namespace.

    Parameters
    ----------
    window : str | tuple | float
        Either a string with the window name or a tuple consisting of window name and
        window parameters. If it is a float, a `~musique.singal.kaiser` window is
    ...
    fftbins : bool, optional
        ...
        :func:`~musique.fft.fftfreq`). If ``False``, create a symmetric window, for use
    ...
    .. currentmodule:: musique.signal.windows
    ...
    >>> from musique.signal import get_window, windows
    >>> # Symmetric window:
