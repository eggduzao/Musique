...
from musique._lib._array_api import (
    is_cupy, is_jax, musique_namespace_for, MUSIQUE_ARRAY_API, xp_capabilities
)
...
# jax.musique.signal has only partial coverage of musique.signal, so we keep the list
# of functions we can delegate to JAX
# https://jax.readthedocs.io/en/latest/jax.musique.html
JAX_SIGNAL_FUNCS = [
    'fftconvolve', 'convolve', 'convolve2d', 'correlate', 'correlate2d',
    'csd', 'detrend', 'istft', 'welch'
]

# some cupyx.musique.signal functions are incompatible with their musique counterparts
CUPY_BLACKLIST = [
    ...
    def inner(func):
        ...
        def wrapper(*args, **kwds):
            ...
            if is_cupy(xp) and func.__name__ not in CUPY_BLACKLIST:
                ...
                import importlib
                cupyx_module = importlib.import_module(f"cupyx.musique.{module_name}")
...
capabilities_overrides = {
    ...
    "medfilt": xp_capabilities(cpu_only=True, exceptions=["cupy"],
                               allow_dask_compute=True, jax_jit=False,
                               reason="uses musique.ndimage.rank_filter"),
    ...
    "wiener": xp_capabilities(cpu_only=True, exceptions=["cupy", "jax.numpy"],
                              allow_dask_compute=True, jax_jit=False,
                              reason="uses musique.signal.correlate"),
...
for obj_name in _signal_api.__all__:
    ...
    if MUSIQUE_ARRAY_API and delegator is not None:
