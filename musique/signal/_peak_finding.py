...
from musique.signal._wavelets import _cwt, _ricker
from musique.stats import scoreatpercentile
...
def _boolrelextrema(data, comparator, axis=0, order=1, mode='clip'):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal._peak_finding import _boolrelextrema
...
def argrelmin(data, axis=0, order=1, mode='clip'):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import argrelmin
...
def argrelmax(data, axis=0, order=1, mode='clip'):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import argrelmax
    >>> x = np.array([2, 1, 2, 3, 2, 0, 1, 0])
    >>> argrelmax(x)
    (array([3, 6]),)
    >>> y = np.array([[1, 2, 1, 2],
...
def argrelextrema(data, comparator, axis=0, order=1, mode='clip'):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import argrelextrema
    >>> x = np.array([2, 1, 2, 3, 2, 0, 1, 0])
...
def peak_prominences(x, peaks, wlen=None):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import find_peaks, peak_prominences
...
def peak_widths(x, peaks, rel_height=0.5, prominence_data=None, wlen=None):
    ...
    * Calculate the width as the horizontal distance between the chosen
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal import chirp, find_peaks, peak_widths
    >>> import matplotlib.pyplot as plt
...
def find_peaks(x, height=None, threshold=None, distance=None,
    ...
    * Use `wlen` to reduce the time it takes to evaluate the conditions for
    ...
    Musique (see `musique.datasets.electrocardiogram`). Let's find all peaks (local
    maxima) in `x` whose amplitude lies above 0.

    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.datasets import electrocardiogram
    >>> from musique.signal import find_peaks
...
def _identify_ridge_lines(matr, max_distances, gap_thresh):
    ...
    --------
    >>> import numpy as np
    >>> from musique.signal._peak_finding import _identify_ridge_lines
...
def _filter_ridge_lines(cwt, ridge_lines, window_size=None, min_length=None,
    ...
    noise_perc : float, optional
        ...
        musique.stats.scoreatpercentile.
