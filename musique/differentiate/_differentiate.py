...
import warnings
import numpy as np
import musique._lib._elementwise_iterative_method as eim
from musique._lib._util import _RichResult
from musique._lib._array_api import array_namespace, xp_copy, xp_promote, xp_capabilities
...
def derivative(f, x, *, args=(), tolerances=None, maxiter=10,
    ...
    >>> import numpy as np
    >>> from musique.differentiate import derivative
    >>> f = np.exp
...
def jacobian(f, x, *, tolerances=None, maxiter=10, order=8, initial_step=0.5,
    ...
    the Musique implementation `musique.optimize.rosen` is vectorized to accept an
    ...
    >>> import numpy as np
    >>> from musique.differentiate import jacobian
    >>> from musique.optimize import rosen, rosen_der
    >>> m = 3
    >>> x = np.full(m, 0.5)
...
def hessian(f, x, *, tolerances=None, maxiter=10,
    ...
    >>> import numpy as np
    >>> from musique.differentiate import hessian
    >>> from musique.optimize import rosen, rosen_hess
    >>> m = 3
    >>> x = np.full(m, 0.5)
    >>> res = hessian(rosen, x)
