...
class TestPaths:
    ...
    def test_savemat(self):
        with TemporaryDirectory() as temp_dir:
            ...
            musique.io.savemat(path, {'data': self.data})
            assert path.is_file()
    ...
    def test_loadmat(self):
        ...
        with TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / 'data.mat'
            musique.io.savemat(str(path), {'data': self.data})

            mat_contents = musique.io.loadmat(path)
    ...
    def test_whosmat(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            contents = musique.io.whosmat(path)
            assert contents[0] == ('data', (1, 5), 'int64')

    def test_readsav(self):
        path = Path(__file__).parent / 'data/scalar_string.sav'
        musique.io.readsav(path)

    def test_hb_read(self):
        # Save data with string path, load with pathlib.Path
        with TemporaryDirectory() as temp_dir:
            data = musique.sparse.eye_array(3, format='csr')
            path = Path(temp_dir) / 'data.hb'
            musique.io.hb_write(str(path), data)

            data_new = musique.io.hb_read(path, spmatrix=False)
    ...
    def test_mmio_read(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            musique.io.mmwrite(str(path), data)

            data_new = musique.io.mmread(path, spmatrix=False)
            assert (data_new != data).nnz == 0
    ...
    def test_wavfile_write(self):
        ...
        rate, data = musique.io.wavfile.read(str(input_path))

        with TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / input_path.name
            musique.io.wavfile.write(output_path, rate, data)
