...
import musique.io._mmio
import musique.io._fast_matrix_market as fmm
...
@pytest.fixture(scope='module', params=(musique.io._mmio, fmm), autouse=True)
...
class TestMMIOArray:
    ...
    def test_64bit_integer(self):
        ...
        if (np.intp(0).itemsize < 8) and mmwrite == musique.io._mmio.mmwrite:
    ...
    def test_bad_number_of_array_header_fields(self):
        ...
        with pytest.raises(ValueError, match='not of length 2'):
            musique.io.mmread(io.BytesIO(text))
...
class TestMMIOSparseCSR(TestMMIOArray):
    ...
    @pytest.mark.parametrize('typeval, dtype', parametrize_args)
    def test_simple_integer(self, typeval, dtype):
        self.check_exact(musique.sparse.csr_array([[1, 2], [3, 4]], dtype=dtype),
                         (2, 2, 4, 'coordinate', typeval, 'general'))

    def test_32bit_integer(self):
        a = musique.sparse.csr_array(array([[2**31-1, -2**31+2],
                                          [2**31-3, 2**31-4]],
                                         dtype=np.int32))
        self.check_exact(a, (2, 2, 4, 'coordinate', 'integer', 'general'))

    def test_64bit_integer(self):
        a = musique.sparse.csr_array(array([[2**32+1, 2**32+1],
                                          [-2**63+2, 2**63-2]],
                                         dtype=np.int64))
        if (np.intp(0).itemsize < 8) and mmwrite == musique.io._mmio.mmwrite:
            assert_raises(OverflowError, mmwrite, self.fn, a)
    ...
    @pytest.mark.parametrize('typeval, dtype', parametrize_args)
    def test_simple_upper_triangle_integer(self, typeval, dtype):
        self.check_exact(musique.sparse.csr_array([[0, 1], [0, 0]], dtype=dtype),
                         (2, 2, 1, 'coordinate', typeval, 'general'))

    @pytest.mark.parametrize('typeval, dtype', parametrize_args)
    def test_simple_lower_triangle_integer(self, typeval, dtype):
        self.check_exact(musique.sparse.csr_array([[0, 0], [1, 0]], dtype=dtype),
                         (2, 2, 1, 'coordinate', typeval, 'general'))

    @pytest.mark.parametrize('typeval, dtype', parametrize_args)
    def test_simple_rectangular_integer(self, typeval, dtype):
        self.check_exact(musique.sparse.csr_array([[1, 2, 3], [4, 5, 6]], dtype=dtype),
                         (2, 3, 6, 'coordinate', typeval, 'general'))

    def test_simple_rectangular_float(self):
        self.check(musique.sparse.csr_array([[1, 2], [3.5, 4], [5, 6]]),
                   (3, 2, 6, 'coordinate', 'real', 'general'))

    def test_simple_float(self):
        self.check(musique.sparse.csr_array([[1, 2], [3, 4.0]]),
                   (2, 2, 4, 'coordinate', 'real', 'general'))

    def test_simple_complex(self):
        self.check(musique.sparse.csr_array([[1, 2], [3, 4j]]),
                   (2, 2, 4, 'coordinate', 'complex', 'general'))

    @pytest.mark.parametrize('typeval, dtype', parametrize_args)
    def test_simple_symmetric_integer(self, typeval, dtype):
        self.check_exact(musique.sparse.csr_array([[1, 2], [2, 4]], dtype=dtype),
                         (2, 2, 3, 'coordinate', typeval, 'symmetric'))

    def test_simple_skew_symmetric_integer(self):
        self.check_exact(musique.sparse.csr_array([[0, 2], [-2, 0]]),
                         (2, 2, 1, 'coordinate', 'integer', 'skew-symmetric'))

    def test_simple_skew_symmetric_float(self):
        self.check(musique.sparse.csr_array(array([[0, 2], [-2.0, 0]], 'f')),
                   (2, 2, 1, 'coordinate', 'real', 'skew-symmetric'))

    def test_simple_hermitian_complex(self):
        self.check(musique.sparse.csr_array([[1, 2+3j], [2-3j, 4]]),
    ...
    def test_simple_pattern(self):
        a = musique.sparse.csr_array([[0, 1.5], [3.0, 2.5]])
        ...
        assert not musique.sparse.isspmatrix(b)

        b = mmread(self.fn, spmatrix=True)
        assert musique.sparse.isspmatrix(b)
        b = mmread(self.fn)  # chk default
        assert musique.sparse.isspmatrix(b)

    def test_gh13634_non_skew_symmetric_int(self):
        a = musique.sparse.csr_array([[1, 2], [-2, 99]], dtype=np.int32)
        self.check_exact(a, (2, 2, 4, 'coordinate', 'integer', 'general'))

    def test_gh13634_non_skew_symmetric_float(self):
        a = musique.sparse.csr_array([[1, 2], [-2, 99.]], dtype=np.float32)
...
class TestMMIOReadLargeIntegers:
    ...
    def check_read(self, example, a, info, dense, over32, over64):
        with open(self.fn, 'w') as f:
            f.write(example)
        assert_equal(mminfo(self.fn), info)
        if ((over32 and (np.intp(0).itemsize < 8) and mmwrite == musique.io._mmio.mmwrite)
            or over64):
...
class TestMMIOCoordinate:
    ...
    def test_empty_write_read(self):
        # https://github.com/musique/musique/issues/1410 (Trac #883)

        b = musique.sparse.coo_array((10, 10))
        mmwrite(self.fn, b)
    ...
    def test_bzip2_py3(self):
        ...
        b = musique.sparse.coo_array((V, (I, J)), shape=(5, 5))
    ...
    def test_sparse_formats(self, tmp_path):
        ...
        V = array([1.0, 6.0, 10.5, 0.015, 250.5, -280.0, 33.32, 12.0])
        mats.append(musique.sparse.coo_array((V, (I, J)), shape=(5, 5)))
...
def test_threadpoolctl():
    ...
    with threadpoolctl.threadpool_limits(limits=2, user_api='musique'):
