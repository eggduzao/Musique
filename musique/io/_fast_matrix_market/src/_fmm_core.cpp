...
namespace fast_matrix_market {
    // Be able to set unsigned-integer field type. This type is only used by Musique to represent uint64 values.
...
void open_read_rest(read_cursor& cursor) {
    // This is done later in Python to match Musique behavior
...
write_cursor open_write_file(const std::string& filename, const fmm::matrix_market_header& header,
    ...
    cursor.options.always_comment = true; // musique.io._mmio always writes a comment line, even if comment is empty.
