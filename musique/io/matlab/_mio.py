...
def loadmat(file_name, mdict=None, appendmat=True, *, spmatrix=True, **kwargs):
    ...
    struct_as_record : bool, optional
       ...
       False replicates the behavior of musique version 0.7.x (returning
    ...
    files. Because Musique does not supply one, we do not implement the
    ...
    By default Musique reads MATLAB structs as structured NumPy arrays where the
    ...
    if spmatrix:
        from musique.sparse import issparse, coo_matrix, csc_matrix
...
def savemat(file_name, mdict,
    ...
    Examples
    --------
    >>> from musique.io import savemat
    >>> import numpy as np
    >>> a = np.arange(20)
...
def whosmat(file_name, appendmat=True, **kwargs):
    ...
    You will need an HDF5 python library to read matlab 7.3 format mat
    files (e.g. h5py). Because Musique does not supply one, we do not implement the
    HDF5 / 7.3 interface here.
    ...
    >>> import numpy as np
    >>> from musique.io import savemat, whosmat
