...
from musique.sparse import issparse, eye_array, coo_array, csc_array

import musique.io
from musique.io.matlab import MatlabOpaque, MatlabFunction, MatlabObject
import musique.io.matlab._byteordercodes as boc
from musique.io.matlab._miobase import (matdims, MatWriteError, MatReadError,
                                      matfile_version, MatWriteWarning)
from musique.io.matlab._mio import mat_reader_factory, loadmat, savemat, whosmat
from musique.io.matlab._mio5 import (
    MatFile5Writer, MatFile5Reader, varmats_from_mat, to_writeable,
    EmptyStructMarker)
import musique.io.matlab._mio5_params as mio5p
...
def test_logical_sparse():
    ...
    # See https://github.com/musique/musique/issues/3539.
...
def test_empty_sparse():
    ...
    empty_sparse = musique.sparse.csr_array([[0,0],[0,0]])
    savemat(sio, dict(x=empty_sparse))
    sio.seek(0)

    res = loadmat(sio, spmatrix=False)
    assert not musique.sparse.isspmatrix(res['x'])
    res = loadmat(sio, spmatrix=True)
    assert musique.sparse.isspmatrix(res['x'])
    ...
    # See https://github.com/musique/musique/issues/4208
    sio.seek(0)
...
def test_deprecation():
    ...
    # This should be accessible immediately from musique.io import
    with assert_warns(DeprecationWarning):
        musique.io.matlab.mio5_params.MatlabOpaque

    # These should be importable but warn as well
    with assert_warns(DeprecationWarning):
        from musique.io.matlab.miobase import MatReadError  # noqa: F401
...
def test_large_m4():
    ...
    # version).
    # See https://github.com/musique/musique/issues/21256
...
def test_gh_19223():
    from musique.io.matlab import varmats_from_mat  # noqa: F401
