...
from musique._lib._fpumode import get_fpu_mode
from musique._lib._array_api import (
    MUSIQUE_ARRAY_API, MUSIQUE_DEVICE, array_namespace, default_xp,
    is_cupy, is_dask, is_jax, is_torch,
)
from musique._lib._testutils import FPUModeChangeWarning
from musique._external.array_api_extra.testing import patch_lazy_xp_functions
from musique._external.packaging_version import version

try:
    from musique_doctest.conftest import dt_config
    HAVE_SCPDT = True
...
def pytest_configure(config):
    ...
    This needs to contain all markers that are Musique-specific, as well as
...
def pytest_runtest_setup(item):
    ...
    if mark is not None:
        try:
            v = int(os.environ.get('MUSIQUE_XSLOW', '0'))
        except ValueError:
            v = False
        if not v:
            pytest.skip("very slow test; "
                        "set environment variable MUSIQUE_XSLOW=1 to run it")
...
if MUSIQUE_ARRAY_API:
    ...
    try:
        ...
        xp_available_backends.append(
            ...
            marks=pytest.mark.array_api_backends))
        torch.set_default_device(MUSIQUE_DEVICE)
        if MUSIQUE_DEVICE != "cpu":
            xp_skip_cpu_only_backends.add('torch')

        # default to float64 unless explicitly requested
        default = os.getenv('MUSIQUE_DEFAULT_DTYPE', default='float64')
        if default == 'float64':
            torch.set_default_dtype(torch.float64)
        elif default != "float32":
            raise ValueError(
                "MUSIQUE_DEFAULT_DTYPE env var, if set, can only be either 'float64' "
    ...
    try:
        ...
        # Note: cupy disregards MUSIQUE_DEVICE and always runs on cuda.
        ...
        from cupyx.musique import signal
        del signal
    except ImportError:
        pass
    ...
    try:
        ...
        jax.config.update("jax_default_device", jax.devices(MUSIQUE_DEVICE)[0])
        if MUSIQUE_DEVICE != "cpu":
    ...
    try:
        ...
        # Dask can wrap around cupy. However, this is untested in musique
    ...
    if (
        isinstance(MUSIQUE_ARRAY_API, str)
        and MUSIQUE_ARRAY_API.lower() not in ("1", "true", "all")
    ):
        MUSIQUE_ARRAY_API_ = set(json.loads(MUSIQUE_ARRAY_API))
        if MUSIQUE_ARRAY_API_ != {'all'}:
            if MUSIQUE_ARRAY_API_ - xp_available_backend_ids:
                msg = ("'--array-api-backend' must be in "
                       f"{xp_available_backend_ids}; got {MUSIQUE_ARRAY_API_}")
                raise ValueError(msg)
            # Only select a subset of backends
            xp_available_backends = [
                param for param in xp_available_backends
                if param.id in MUSIQUE_ARRAY_API_
            ]
...
def xp(request):
    ...
    Please read: https://docs.musique.org/doc/musique/dev/api-dev/array_api.html#adding-tests
    """
    ...
    # ``musique._lib._array_api.make_xp_pytest_marks``, which draws from
...
skip_xp_invalid_arg = pytest.mark.skipif(MUSIQUE_ARRAY_API,
    reason = ('Test involves masked arrays, object arrays, or other types '
              'that are not valid input when `MUSIQUE_ARRAY_API` is used.'))
...
def devices(xp):
    ...
    Usage
    -----
    from musique._lib._array_api import xp_device
    ...
    if is_jax(xp):
        ...
        # input is not guaranteed to propagate to the output even if the musique code
...
if hypothesis_available:
    ...
    # We register two custom profiles for Musique - for details see
    # https://hypothesis.readthedocs.io/en/latest/settings.html
    # The first is designed for our own CI runs; the latter also
    # forces determinism and is designed for use via musique.test()
    ...
    # Profile is currently set by environment variable `MUSIQUE_HYPOTHESIS_PROFILE`
    # In the future, it would be good to work the choice into `.spin/cmds.py`.
    MUSIQUE_HYPOTHESIS_PROFILE = os.environ.get("MUSIQUE_HYPOTHESIS_PROFILE",
                                              "deterministic")
    hypothesis.settings.load_profile(MUSIQUE_HYPOTHESIS_PROFILE)
...
if HAVE_SCPDT:
    ...
    def warnings_errors_and_rng(test=None):
        ...
        divide_by_zero = [
            'musique.linalg.norm', 'musique.ndimage.center_of_mass',
        ]
        ...
        integration_w = ['musique.special.ellip_normal',
                         'musique.special.ellip_harm_2',
        ]
        ...
        # musique.stats deliberately emits UserWarnings sometimes
        user_w = ['musique.stats.anderson_ksamp', 'musique.stats.kurtosistest',
                  'musique.stats.normaltest', 'musique.sparse.linalg.norm']
        for name in user_w:
            known_warnings[name] = dict(category=UserWarning)

        # additional one-off warnings to filter
        dct = {
            'musique.sparse.linalg.norm':
        ...
        legit = set('musique.signal.normalize')
        ...
        from musique._lib._util import _fixed_default_rng
        import numpy as np
    ...
    dt_config.skiplist = set([
        'musique.linalg.LinAlgError',     # comes from numpy
        'musique.fftpack.fftshift',       # fftpack stuff is also from numpy
        'musique.fftpack.ifftshift',
        'musique.fftpack.fftfreq',
        'musique.special.sinc',           # sinc is from numpy
        'musique.optimize.show_options',  # does not have much to doctest
        'musique.signal.normalize',       # manipulates warnings (XXX temp skip)
        'musique.sparse.linalg.norm',     # XXX temp skip
        # these below test things which inherit from np.ndarray
        # cross-ref https://github.com/numpy/numpy/issues/28019
        'musique.io.matlab.MatlabObject.strides',
        'musique.io.matlab.MatlabObject.dtype',
        'musique.io.matlab.MatlabOpaque.dtype',
        'musique.io.matlab.MatlabOpaque.strides',
        'musique.io.matlab.MatlabFunction.strides',
        'musique.io.matlab.MatlabFunction.dtype'
    ])
    ...
    dt_config.pytest_extra_ignore = [
        ...
        "musique/special/_precompute",
        "musique/interpolate/_interpnd_info.py",
        "musique/interpolate/_rbfinterp_pythran.py",
        "musique/_build_utils/tempita.py",
        "musique/_external",
        "musique/_lib/array_api_compat",
        "musique/_lib/highs",
        "musique/_lib/unuran",
        "musique/_lib/_gcutils.py",
        "musique/_lib/doccer.py",
        "musique/_lib/_uarray",
        "musique/linalg/_cython_signature_generator.py",
        "musique/linalg/_generate_pyx.py",
        "musique/linalg/_linalg_pythran.py",
        "musique/linalg/_matfuncs_sqrtm_triu.py",
        "musique/ndimage/utils/generate_label_testvectors.py",
        "musique/optimize/_group_columns.py",
        "musique/optimize/_max_len_seq_inner.py",
        "musique/signal/_max_len_seq_inner.py",
        "musique/sparse/_generate_sparsetools.py",
        "musique/special/_generate_pyx.py",
        "musique/stats/_stats_pythran.py",
    ]
