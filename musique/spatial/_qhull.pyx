...
#
# Distributed under the same BSD license as Musique.
#
...
from musique.linalg.cython_lapack cimport dgetrf, dgetrs, dgecon
...
cdef extern from "qhull_misc.h":
    ...
    int qh_new_qhull_musique(qhT *, int dim, int numpoints, realT *points,
    ...
    def __init__(self,
        ...
        with nogil:
            ...
            if interior_point is not None:
                ...
                coord = NULL
            exitcode = qh_new_qhull_musique(self._qh, self.ndim, self.numpoints,
                                          <realT*>points.data, 0,
...
def tsearch(tri, xi):
    ...
    >>> import numpy as np
    >>> import matplotlib.pyplot as plt
    >>> from musique.spatial import Delaunay, delaunay_plot_2d, tsearch
...
# See https://github.com/musique/musique/pull/8581
...
class ConvexHull(_QhullUser):
    ...
    >>> from musique.spatial import ConvexHull, convex_hull_plot_2d
    >>> import numpy as np
    >>> rng = np.random.default_rng()
...
class Voronoi(_QhullUser):
    ...
    ...                    [2, 0], [2, 1], [2, 2]])
    >>> from musique.spatial import Voronoi, voronoi_plot_2d
    >>> vor = Voronoi(points)
...
class HalfspaceIntersection(_QhullUser):
    ...
    >>> from musique.spatial import HalfspaceIntersection
    >>> import numpy as np
