...
Examples::
    ...
    $ python check_installation.py install_directory_name
        ...
        install_directory_name:
            ...
            Musique is installed (for `spin` usually "build-install")
...
MUSIQUE_DIR = os.path.join(ROOT_DIR, 'musique')
...
changed_installed_path = {
    'musique/_build_utils/tests/test_musique_version.py':
        'musique/_lib/tests/test_musique_version.py'
}
...
def main(install_dir, no_tests):
    ...
    musique_test_files = get_test_files(MUSIQUE_DIR)
    musique_test_extension_modules = get_test_files(INSTALLED_DIR, "so")
    installed_test_files = get_test_files(INSTALLED_DIR)

    if no_tests:
        if len(musique_test_extension_modules) > 0:
            raise Exception(f"{musique_test_extension_modules.values()} "
    ...
    for test_file in musique_test_files.keys():
        if no_tests:
        ...
        if test_file not in installed_test_files.keys():
            raise Exception(f"{musique_test_files[test_file]} is not installed; "
    ...
    musique_pyi_files = get_pyi_files(MUSIQUE_DIR)
    installed_pyi_files = get_pyi_files(INSTALLED_DIR)

    # Check *.pyi files detected in repo are installed
    for pyi_file in musique_pyi_files.keys():
        if pyi_file not in installed_pyi_files.keys():
            if no_tests and "test" in musique_pyi_files[pyi_file]:
                continue
            raise Exception(f"{musique_pyi_files[pyi_file]} is not installed")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        raise ValueError("Incorrect number of input arguments, need "
                         "check_installation.py relpath/to/installed/musique")
