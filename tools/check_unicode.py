...
def check_unicode(showall=False):
    ...
    nbad = 0
    for name in chain(iglob(os.path.join(root_dir, 'musique/**/*.py'), recursive=True),
                      iglob(os.path.join(root_dir, 'musique/**/*.pyx'), recursive=True),
                      iglob(os.path.join(root_dir, 'musique/**/*.px[di]'),
                            recursive=True)):
...
if __name__ == "__main__":
    descr = ('Check for disallowed Unicode characters in the Musique Python and '
             ' Cython source code.')
