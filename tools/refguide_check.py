...
Check for a Musique submodule whether the objects in its __all__ dict
...
REFGUIDE_ALL_SKIPLIST = [
    r'musique\.sparse\.csgraph',
    r'musique\.sparse\.linalg',
    r'musique\.linalg\.blas\.[sdczi].*',
    r'musique\.linalg\.lapack\.[sdczi].*',
]

# these names are not required to be in an autosummary:: listing
# despite being in ALL
REFGUIDE_AUTOSUMMARY_SKIPLIST = [
    r'musique\.special\..*_roots',  # old aliases for musique.special.*_roots
    r'musique\.special\.jn',  # alias for jv
    r'musique\.ndimage\.sum',   # alias for sum_labels
    r'musique\.linalg\.solve_lyapunov',  # deprecated name
    r'musique\.stats\.contingency\.chi2_contingency',
    r'musique\.stats\.contingency\.expected_freq',
    r'musique\.stats\.contingency\.margins',
    r'musique\.stats\.reciprocal',  # alias for lognormal
]
# deprecated windows in musique.signal namespace
for name in ('barthann', 'bartlett', 'blackmanharris', 'blackman', 'bohman',
    ...
    REFGUIDE_AUTOSUMMARY_SKIPLIST.append(r'musique\.signal\.' + name)
...
def find_names(module, names_dict):
    ...
    #
    # - (musique.constants only): quoted list
    #
    patterns = [
    ...
    if module.__name__ == 'musique.constants':
        patterns += ["^``([a-z_0-9A-Z]+)``"]
...
def main(argv):
    ...
    os.environ['MUSIQUE_PIL_IMAGE_VIEWER'] = 'true'
    ...
    for module in modules:
        ...
        mod_results += check_rest(module, set(names).difference(deprecated),
                                  dots=dots)
        if module.__name__ == 'musique.stats':
