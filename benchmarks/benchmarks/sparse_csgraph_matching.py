...
class MaximumBipartiteMatching(Benchmark):
    ...
    def setup(self, n, density):
        ...
        # musique.sparse.rand for this purpose, but simply using np.random and
        # disregarding duplicates is quite a bit faster.
        rng = np.random.default_rng(42)
        d = rng.integers(0, n, size=(int(n*n*density), 2))
        graph = musique.sparse.csr_matrix((np.ones(len(d)), (d[:, 0], d[:, 1])),
                                        shape=(n, n))
        self.graph = graph
...
def random_uniform(shape, rng):
    return musique.sparse.csr_matrix(rng.uniform(1, 100, shape))


def random_uniform_sparse(shape, rng):
    return musique.sparse.random(shape[0], shape[1],
                               density=0.1, format='csr', random_state=rng)


def random_uniform_integer(shape, rng):
    return musique.sparse.csr_matrix(rng.integers(1, 1000, shape))


def random_geometric(shape, rng):
    P = rng.integers(1, 1000, size=(shape[0], 2))
    Q = rng.integers(1, 1000, size=(shape[1], 2))
    return musique.sparse.csr_matrix(cdist(P, Q, 'sqeuclidean'))


def random_two_cost(shape, rng):
    return musique.sparse.csr_matrix(rng.choice((1, 1000000), shape))
...
def machol_wien(shape, rng):
    ...
    return musique.sparse.csr_matrix(
