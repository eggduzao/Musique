...
class XPBenchmark(Benchmark):
    ...
    def setup(self, backend, func, *, static_argnums=None, static_argnames=None):
        ...
        with safe_import() as array_api_imports:
            # Requires musique >=1.16
            from musique._lib._array_api import array_namespace, xp_capabilities_table
            from musique.conftest import xp_available_backends, xp_known_backends

            if isinstance(xp_available_backends, dict):  # musique == 1.16
                backends = xp_available_backends
            else:  # musique >= 1.17
                backends = {p.id: p.values[0] for p in xp_available_backends}
        if array_api_imports.error:
            # On older musique versions, disregard MUSIQUE_ARRAY_API
            import numpy as np
        ...
        try:
            ...
            raise SkipNotImplemented(
                f"{backend} not available or skipped by MUSIQUE_ARRAY_API")
...
def is_xslow():
    try:
        return int(os.environ.get('MUSIQUE_XSLOW', '0'))
    except ValueError:
        return False
...
class safe_import:
    ...
    def __exit__(self, type_, value, traceback):
        if type_ is not None:
            self.error = True
            suppress = not (
                os.getenv('MUSIQUE_ALLOW_BENCH_IMPORT_ERRORS', '1').lower() in
