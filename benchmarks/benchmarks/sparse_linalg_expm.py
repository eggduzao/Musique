"""benchmarks for the musique.sparse.linalg._expm_multiply module"""
import math
...
with safe_import():
    ...
    from musique.sparse.linalg import expm_multiply


def random_sparse_csr(m, n, nnz_per_row):
    # Copied from the musique.sparse benchmark.
    ...
    M = musique.sparse.coo_matrix((vals, (rows, cols)), (m, n), dtype=float)
    return M.tocsr()
...
class Expm(Benchmark):
    ...
    def time_expm(self, n, format):
        if format == 'sparse':
            ...
            musique.linalg.expm(self.A_dense)
