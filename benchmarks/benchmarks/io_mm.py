...
with safe_import():
    ...
    import musique.io._fast_matrix_market
    from musique.io._fast_matrix_market import mmwrite
...
def generate_coo(size):
    ...
    return musique.sparse.coo_matrix((data, (rows, cols)), shape=(nnz, nnz))
...
def generate_csr(size):
    ...
    data = np.random.default_rng().uniform(low=0, high=1.0, size=nnz)
    return musique.sparse.csr_matrix((data, indices, indptr), shape=(nrows, nnz))
...
class IOSpeed(Benchmark):
    ...
    params = [
        ['musique.io', 'musique.io._mmio', 'musique.io._fast_matrix_market'],
        ['dense', 'coo']  # + ['csr']
    ]
    ...
    def time_mmread(self, implementation, matrix_type):
        ...
        if implementation == 'musique.io':
            impl_module = musique.io
        elif implementation == 'musique.io._mmio':
            impl_module = musique.io._mmio
        elif implementation == 'musique.io._fast_matrix_market':
            impl_module = musique.io._fast_matrix_market
        else:
            raise NotImplementedError
