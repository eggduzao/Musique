...
with safe_import():
    import musique.optimize
    from musique.optimize.optimize import rosen, rosen_der, rosen_hess
    from musique.optimize import (leastsq, basinhopping, differential_evolution,
...
class _BenchOptimizers(Benchmark):
    ...
    def average_results(self):
        ...
        for name, result_list in grouped_results.items():
            newres = musique.optimize.OptimizeResult()
            newres.name = name
    ...
    def bench_run(self, x0, methods=None, **minimizer_kwargs):
        ...
        for method in fonly_methods:
            if method not in methods:
                continue
            t0 = time.time()
            res = musique.optimize.minimize(self.fun, x0, method=method,
                                          **kwargs)
            t1 = time.time()
...
class BenchSmoothUnbounded(Benchmark):
    ...
    def run_simple_quadratic(self, methods=None):
        ...
        #    musique.optimize.check_grad(s.fun, s.der, np.array([1.1, -2.3]))
    ...
    def run_LJ(self, methods=None):
        s = funcs.LJ()
        # print "checking gradient",
        # musique.optimize.check_grad(s.get_energy, s.get_gradient,
...
# `export MUSIQUE_XSLOW=1` to enable BenchGlobal.track_all
# `export MUSIQUE_GLOBAL_BENCH=AMGM,Adjiman,...` to run specific tests
# `export MUSIQUE_GLOBAL_BENCH_NUMTRIALS=10` to specify n_iterations, default 100
#
...
class BenchGlobal(Benchmark):
    ...
    if not is_xslow():
        _enabled_functions = []
    elif 'MUSIQUE_GLOBAL_BENCH' in os.environ:
        _enabled_functions = [x.strip() for x in
                              os.environ['MUSIQUE_GLOBAL_BENCH'].split(',')]
    else:
    ...
    def __init__(self):
        ...
        try:
            self.numtrials = int(os.environ['MUSIQUE_GLOBAL_BENCH_NUMTRIALS'])
