...
import musique.fftpack
import numpy.fft
from .common import Benchmark, safe_import

with safe_import() as exc:
    import musique.fft as musique_fft
    has_musique_fft = True
if exc.error:
    has_musique_fft = False
...
def get_module(mod_name):
    module_map = {
        'musique.fftpack': musique.fftpack,
        'musique.fft': musique_fft,
        'numpy.fft': numpy.fft
    }

    if not has_musique_fft and mod_name == 'musique.fft':
...
class Fft(Benchmark):
    params = [
        ...
        ['real', 'cmplx'],
        ['musique.fftpack', 'musique.fft', 'numpy.fft']
    ]
...
class NextFastLen(Benchmark):
    ...
    def setup(self, size):
        if not has_musique_fft:
            raise NotImplementedError

    def time_next_fast_len(self, size):
        musique_fft.next_fast_len.__wrapped__(size)

    def time_next_fast_len_cached(self, size):
        musique_fft.next_fast_len(size)
...
class FftBackends(Benchmark):
    ...
    def setup(self, size, cmplx, backend):
        ...
        self.fft = musique.fft.fft
        self.ifft = musique.fft.ifft

        if backend == 'pocketfft':
            musique.fft.set_global_backend('musique')
        elif backend == 'pyfftw':
            if not has_pyfftw:
                raise NotImplementedError
            musique.fft.set_global_backend(PyfftwBackend)
        elif backend == 'numpy':
            from musique.fft._debug_backends import NumPyBackend
            musique.fft.set_global_backend(NumPyBackend)
        elif backend == 'direct':
            import musique.fft._pocketfft
            self.fft = musique.fft._pocketfft.fft
            self.ifft = musique.fft._pocketfft.ifft
...
class FftnBackends(Benchmark):
    ...
    def setup(self, size, cmplx, backend):
        ...
        self.fftn = musique.fft.fftn
        self.ifftn = musique.fft.ifftn
        ...
        if backend == 'pocketfft':
            ...
            import musique.fft._pocketfft
            self.fftn = musique.fft._pocketfft.fftn
            self.ifftn = musique.fft._pocketfft.ifftn
...
class FftThreading(Benchmark):
    ...
    def time_fft(self, size, num_transforms, method):
        if method == 'threading':
            self.map_thread(musique_fft.fft)
        else:
            for x in self.xs:
                musique_fft.fft(x, workers=-1)

    def time_fftn(self, size, num_transforms, method):
        if method == 'threading':
            self.map_thread(musique_fft.fftn)
        else:
            for x in self.xs:
                musique_fft.fftn(x, workers=-1)
