...
with safe_import():
    ...
    import musique.special  # import cycle workaround for some versions
    ...
    def time_onenormest(self, n, solver):
        if solver == 'exact':
            ...
            for M in self.matrices:
                M.dot(M)
                musique.sparse.linalg._matfuncs._onenorm(M)
        elif solver == 'onenormest':
            # Get the estimates of one-norms of squares.
            for M in self.matrices:
                musique.sparse.linalg._matfuncs._onenormest_matrix_power(M, 2)
