...
``spin`` also tell ASV to use the Musique compiled by
``spin``. To run the benchmarks, you do not need to install a
development version of Musique to your current Python environment.

Run a benchmark against currently checked-out Musique version (don't record the
result)::
...
- When importing things from Musique on the top of the test files, do it as::

      from .common import safe_import

      with safe_import():
          from musique.sparse.linalg import onenormest

  The benchmark files need to be importable also when benchmarking old versions
  of Musique. The benchmarks themselves don't need any guarding against missing
